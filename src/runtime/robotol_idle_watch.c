...
void robotol_idle_watch_on_handler_register(uint32_t plat_code, uint32_t family, uint32_t handler) {
    const char *role = "?";
    if (!robotol_idle_watch_enabled()) return;
    if (plat_code == 0x10140u) role = "lifecycle_period";
    else if (plat_code == 0x10165u) role = "enqueue_event";
    else if (plat_code == 0x10162u) role = "alloc_or_enqueue";
    else if (plat_code == 0x10102u) role = "family_register";
...
void robotol_idle_watch_try_10165_probe(void *uc) {
    ...
    int ok;
    if (!robotol_idle_watch_enabled() || !g_e8c.e8d_probe || g_e8c.probe_done) return;
    if (!uc) uc = g_e8c.uc;
    if (!uc) return;
    handler = platform_handler_registry_get(0x10165u);
    if (!handler) return;
    family = platform_handler_registry_family(0x10165u);
    cand = getenv("JJFB_E8D_10165_CANDIDATE");
    if (!cand || !cand[0]) cand = "ZERO_ARGS";
    /* Only ZERO_ARGS until ABI proven — still label hypothesis. */
    ...
    printf("[JJFB_E8D_10165_FIRE] candidate=%s handler=0x%X family=0x%X r9=0x%X "
           "note=observe_only_hypothesis evidence=HYPOTHESIS+docs/06_enqueue\n",
           cand, handler, family, r9_run);
    fflush(stdout);
    g_e8c.probe_done = 1;
