...
void platform_send_app_event_classify(const GwyPlatCall *call, GwyPlatCallResult *out) {
    ...
    /*
     * 0x10112: read file into guest buffer (TARGET_OBSERVED gamelist.ext @+0x7B9C).
     *   sendAppEvent(0x10112, ctx, path, &out_buf, &out_len, 0, 0)
     * Bare names (cfg.bin) → active MRP member; gwy/... → shared VFS root.
     * Must NOT alias MRP_MEMBER cfg.bin onto shared gwy/cfg.bin.
     */
    if (call->code == 0x10112u && call->arg2) {
        out->kind = GWY_PLAT_KIND_READ_FILE;
        out->status_ret = 0u;
        out->fill_buf = call->arg2;
        out->read_out_buf = call->arg3;
        out->fill_type = call->arg4;
        out->read_ctx = call->app;
        out->name = "plat_10112_read_file";
        out->evidence = "TARGET_OBSERVED";
        return;
    }
}
