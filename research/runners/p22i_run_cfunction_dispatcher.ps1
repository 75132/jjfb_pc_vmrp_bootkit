...
$runId = ('p22i_{0:yyyyMMdd_HHmmss}_{1}' -f (Get-Date), (Get-Random -Maximum 99999))
$outDir = Join-Path $Root "out\p22i\$runId"
$reportDir = Join-Path $Root "reports\p22i\$runId"
$logDir = Join-Path $Root "logs\p22i\$runId"
...
$identity = Join-Path $outDir 'p22i_build_identity.txt'
$verdict = Join-Path $reportDir 'p22i_dispatcher_verdict.md'
...
gate=P22I_CLEAN_cfunction_dispatcher
NATURAL_ONLY=yes
Lane=A
research_assisted=0
product_valid=1
...
$vmLog = Join-Path $logDir 'p22i_vmrp.txt'
$stderr = Join-Path $logDir 'p22i_stderr.txt'
...
$wl = [ordered]@{
  ...
  JJFB_E10A31_TIMER_CSV = (Join-Path $reportDir 'p22i_e10a31_timer.csv')
  JJFB_E10A31B_MODE = '1'
  JJFB_E10A31B_PUB_CSV = (Join-Path $reportDir 'p22i_e10a31b_pub.csv')
  JJFB_E10A31_CFG_GATE = '1'
  JJFB_E10A31_PARAM_TRACE = '1'
  JJFB_E10A31_PARAM_CSV = (Join-Path $reportDir 'p22i_e10a31_param.csv')
  JJFB_E10A31_CFG_GATE_CSV = (Join-Path $reportDir 'p22i_e10a31_cfg_gate.csv')
  JJFB_P19_HANDOFF = '1'
  JJFB_P19_OUT_DIR = $outDir
  GWY_P19_PARENT_CHILD_HANDOFF = '1'
  JJFB_P20_CLEAN = '1'
  JJFB_P22I_STACK_CSV = (Join-Path $reportDir 'p22i_helper_call_stack.csv')
  JJFB_P22I_RETURNS_CSV = (Join-Path $reportDir 'p22i_helper_returns.csv')
  JJFB_P22I_PROV_CSV = (Join-Path $reportDir 'p22i_method_value_provenance.csv')
  JJFB_P22I_R9_CSV = (Join-Path $reportDir 'p22i_r9_owner_timeline.csv')
  JJFB_P22I_POST_CSV = (Join-Path $reportDir 'p22i_post_init_timeline.csv')
  JJFB_P22I_XREF_CSV = (Join-Path $reportDir 'p22i_cfunction_caller_xrefs.csv')
  JJFB_P22I_MATRIX_CSV = (Join-Path $reportDir 'p22i_method_dispatch_matrix.csv')
  JJFB_P22I_CALLSITE = (Join-Path $reportDir 'p22i_cfunction_helper_callsite.txt')
  JJFB_P22I_BRANCH_MD = (Join-Path $reportDir 'p22i_return_branch_chain.md')
  JJFB_P22I_VERDICT = $verdict
  JJFB_P22I_SUMMARY = (Join-Path $outDir 'p22i_runtime_summary.txt')
...
Write-Host "=== P22I-CLEAN Lane A run_id=$runId seconds=$Seconds ==="
...
$summaryPath = Join-Path $outDir 'p22i_runtime_summary.txt'
$summary = @{}
...
Copy-Item $vmLog (Join-Path $outDir 'p22i_vmrp.txt') -Force -EA SilentlyContinue
Copy-Item $stderr (Join-Path $outDir 'p22i_stderr.txt') -Force -EA SilentlyContinue
if (Test-Path $identity) { Copy-Item $identity (Join-Path $reportDir 'p22i_build_identity.txt') -Force -EA SilentlyContinue }

@(
  'p22i_helper_call_stack.csv',
  'p22i_helper_returns.csv',
  'p22i_method_value_provenance.csv',
  'p22i_r9_owner_timeline.csv',
  'p22i_post_init_timeline.csv',
  'p22i_cfunction_caller_xrefs.csv',
  'p22i_method_dispatch_matrix.csv',
  'p22i_cfunction_helper_callsite.txt',
  'p22i_return_branch_chain.md'
) | ForEach-Object {
...
See out/p22i/$runId/ and reports/p22i/$runId/.
...
Write-Host "=== P22I-CLEAN done class=$($r.class) helper=$($r.helper) nat601=$($r.nat) freeze=$(if($freezeOk){'PASS'}else{'FAIL'}) fire=$($r.fire_ext_n) ==="
