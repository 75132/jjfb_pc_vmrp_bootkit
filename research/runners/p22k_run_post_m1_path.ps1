# P22K: observe post-method=1 ARM path at cfunction continuation (Class B next_fix).
# Reuses P22I observer + P22J R9 fix. NATURAL_ONLY.
param(
  [int]$Seconds = 240,
  [int]$HoldSec = 8,
  [switch]$SkipBuild
...
$runId = ('p22k_{0:yyyyMMdd_HHmmss}_{1}' -f (Get-Date), (Get-Random -Maximum 99999))
$outDir = Join-Path $Root "out\p22k\$runId"
$reportDir = Join-Path $Root "reports\p22k\$runId"
$logDir = Join-Path $Root "logs\p22k\$runId"
...
$identity = Join-Path $outDir 'p22k_build_identity.txt'
$verdict = Join-Path $reportDir 'p22k_post_m1_verdict.md'
...
$vmLog = Join-Path $logDir 'p22k_vmrp.txt'
$stderr = Join-Path $logDir 'p22k_stderr.txt'
...
$wl = [ordered]@{
  ...
  JJFB_E10A31_TIMER_CSV = (Join-Path $reportDir 'p22k_e10a31_timer.csv')
  JJFB_E10A31B_MODE = '1'
  JJFB_E10A31B_PUB_CSV = (Join-Path $reportDir 'p22k_e10a31b_pub.csv')
  JJFB_E10A31_CFG_GATE = '1'
  JJFB_E10A31_PARAM_TRACE = '1'
  JJFB_E10A31_PARAM_CSV = (Join-Path $reportDir 'p22k_e10a31_param.csv')
  JJFB_P19_HANDOFF = '1'
  JJFB_P19_OUT_DIR = $outDir
  GWY_P19_PARENT_CHILD_HANDOFF = '1'
  JJFB_P20_CLEAN = '1'
  JJFB_P22I_STACK_CSV = (Join-Path $reportDir 'p22k_helper_call_stack.csv')
  JJFB_P22I_RETURNS_CSV = (Join-Path $reportDir 'p22k_helper_returns.csv')
  JJFB_P22I_PROV_CSV = (Join-Path $reportDir 'p22k_method_value_provenance.csv')
  JJFB_P22I_R9_CSV = (Join-Path $reportDir 'p22k_r9_owner_timeline.csv')
  JJFB_P22I_POST_CSV = (Join-Path $reportDir 'p22k_post_init_timeline.csv')
  JJFB_P22I_XREF_CSV = (Join-Path $reportDir 'p22k_cfunction_caller_xrefs.csv')
  JJFB_P22I_MATRIX_CSV = (Join-Path $reportDir 'p22k_method_dispatch_matrix.csv')
  JJFB_P22I_CALLSITE = (Join-Path $reportDir 'p22k_cfunction_helper_callsite.txt')
  JJFB_P22I_BRANCH_MD = (Join-Path $reportDir 'p22k_return_branch_chain.md')
  JJFB_P22I_VERDICT = (Join-Path $reportDir 'p22k_p22i_dispatcher_verdict.md')
  JJFB_P22I_SUMMARY = (Join-Path $outDir 'p22k_p22i_runtime_summary.txt')
  JJFB_P22I_IDENTITY = $identity
  JJFB_P22K_INSN_CSV = (Join-Path $reportDir 'p22k_post_m1_insn.csv')
  JJFB_P22K_VERDICT = $verdict
  JJFB_P22K_SUMMARY = (Join-Path $outDir 'p22k_runtime_summary.txt')
}
...
Write-Host "=== P22K post-m1 path run_id=$runId seconds=$Seconds ==="
...
$summaryPath = Join-Path $outDir 'p22k_runtime_summary.txt'
$summary = @{}
...
Copy-Item $vmLog (Join-Path $outDir 'p22k_vmrp.txt') -Force -EA SilentlyContinue
Copy-Item $stderr (Join-Path $outDir 'p22k_stderr.txt') -Force -EA SilentlyContinue
if (Test-Path $identity) { Copy-Item $identity (Join-Path $reportDir 'p22k_build_identity.txt') -Force -EA SilentlyContinue }

$r = [ordered]@{
  run_id = $runId
  class = if ($summary['class']) { $summary['class'] } else { '?' }
  cont = if ($summary['cont']) { $summary['cont'] } else { '?' }
  saw_cont = if ($summary['saw_cont']) { $summary['saw_cont'] } else { Hit 'JJFB_P22K\] pc=0x89BF0' }
  sp64 = if ($summary['sp64']) { $summary['sp64'] } else { '?' }
  mov_r0_2 = if ($summary['mov_r0_2']) { $summary['mov_r0_2'] } else { Hit 'epilogue_r0_2' }
  ldmfd = if ($summary['ldmfd']) { $summary['ldmfd'] } else { Hit 'function_epilogue' }
  return_pc = if ($summary['return_pc']) { $summary['return_pc'] } else { '?' }
  lock = if ($summary['sole_lock']) { $summary['sole_lock'] } else { '?' }
  next = if ($summary['next_fix']) { $summary['next_fix'] } else { '?' }
  p22k_final = FirstLine '\[JJFB_P22K_FINAL\][^\n]+'
  p22i_final = FirstLine '\[JJFB_P22I_FINAL\][^\n]+'
  insn_n = Hit '\[JJFB_P22K\] pc='
}

if (-not (Test-Path $verdict)) {
  @"
# P22K post-m1 path verdict (runner fallback)

**Class: $($r.class)**

$($r.lock)

- cont=$($r.cont) saw_cont=$($r.saw_cont) sp64=$($r.sp64)
- mov_r0_2=$($r.mov_r0_2) ldmfd=$($r.ldmfd) return_pc=$($r.return_pc)
- insn_hits=$($r.insn_n)
...
Write-Host "=== P22K done class=$($r.class) cont=$($r.cont) saw_cont=$($r.saw_cont) mov_r0_2=$($r.mov_r0_2) ldmfd=$($r.ldmfd) ret=$($r.return_pc) insn=$($r.insn_n) ==="
