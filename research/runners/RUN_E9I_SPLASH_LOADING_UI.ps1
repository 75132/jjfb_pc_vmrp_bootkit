# Stage E9I: splash loading UI completion — NOT product success.
# Natural coords via R9+0x830/0x834 seed; sibling binds; reduce COORD_ASSIST.
...
param(
  ...
  [ValidateSet('natural','coordassist','trace')]
  [string]$Mode = 'natural',
  [switch]$SkipBuild,
  [switch]$UiModeAssist
)
...
$bpSpec = @(
  ...
  'p:0x2FC418','p:0x2EFA33','p:0x2EFA46','p:0x2EFA56','p:0x2EFA5C','p:0x2EFA7C','p:0x2EFA9A',
  'p:0x2EC6B8','p:0x2EFA9E','p:0x2EFAF2','p:0x2EFAFA'
) -join ','
...
function Clear-E9Modes {
  @(
    ...
    'JJFB_E9F_MODE','JJFB_E9G_MODE','JJFB_E9H_MODE','JJFB_E9I_MODE',
...
function Analyze-E9I([string]$log) {
  $t = Get-Content $log -Raw -ErrorAction SilentlyContinue
  if (-not $t) { return [pscustomobject]@{ class = 'NO_LOG'; evidence = 'EMPTY' } }
  $ui = $t -match 'JJFB_E9I_CLASS\] class=SPLASH_LOADING_UI_VISIBLE'
  $nat = $t -match 'SPLASH_LOADINGBAR_NATURAL_COORD_VISIBLE'
  $tb = $t -match 'SPLASH_TEXTBAR_DRAWN_NO_REWRITE'
  $top = $t -match 'SPLASH_TOP_DRAWN_NO_REWRITE'
  $lb = $t -match 'SPLASH_LOADINGBAR_DRAWN_NO_REWRITE|2EC6B8_BLIT\][^\r\n]*loadingbar'
  $coordA = $t -match 'JJFB_SPLASH_COORD_ASSIST\]|JJFB_E9H_COORD_ASSIST\]'
  $seed = $t -match 'JJFB_E9I_SCR_DIM_SEED\]'
  $r4src = $t -match 'SPLASH_R4_SOURCE_FOUND_NEXT_GAP'
  $sibIncomplete = $t -match 'SPLASH_R4_REQUIRES_SIBLING_BIND'
  $r4z = $t -match 'SPLASH_BLOCKED_BY_R4_ZERO|JJFB_E9I_R4_CMP\][^\r\n]*r4_zero'
  $siblings = $t -match 'KEEP_SIBLINGS|keep=bar_bind'
  $rewrite = $t -match 'JJFB_E9F_REQUEST_REWRITE\]'
  $cls = if ($ui) { 'SPLASH_LOADING_UI_VISIBLE' }
    elseif ($nat -and $lb -and -not $coordA) { 'SPLASH_LOADINGBAR_NATURAL_COORD_VISIBLE' }
    elseif ($tb) { 'SPLASH_TEXTBAR_DRAWN_NO_REWRITE' }
    elseif ($top) { 'SPLASH_TOP_DRAWN_NO_REWRITE' }
    elseif ($lb -and $coordA) { 'SPLASH_COORD_ASSIST_STILL_REQUIRED' }
    elseif ($lb -and $seed) { 'SPLASH_COORD_SOURCE_FOUND_NEXT_GAP' }
    elseif ($r4src) { 'SPLASH_R4_SOURCE_FOUND_NEXT_GAP' }
    elseif ($sibIncomplete) { 'SPLASH_R4_REQUIRES_SIBLING_BIND' }
    elseif ($r4z) { 'SPLASH_BLOCKED_BY_R4_ZERO' }
    elseif ($rewrite) { 'REWRITE_USED_NOT_PRODUCT' }
    else { 'PRODUCT_STILL_NEEDS_STATE_MACHINE_NATURALIZATION' }
  return [pscustomobject]@{
    class = $cls
    evidence = if ($ui -or $nat -or $tb -or $top -or $lb) { 'OBSERVED' } else { 'HYPOTHESIS' }
    loadingbar = [bool]$lb
    textbar = [bool]$tb
    top = [bool]$top
    natural_coord = [bool]($nat -and -not $coordA)
    coord_assist = [bool]$coordA
    r4_zero = [bool]$r4z
...
if (-not $SkipBuild) {
  Write-Host '=== E9I force-rebuild launcher_core + vmrp gwy ==='
...
$coordCsv = Join-Path $reportDir 'e9i_coord_trace.csv'
$r4Csv = Join-Path $reportDir 'e9i_r4_after_blit_trace.csv'
$seqCsv = Join-Path $reportDir 'e9i_splash_resource_sequence.csv'
$verdictMd = Join-Path $reportDir 'stage_e9i_splash_loading_verdict.md'
$shotBmp = Join-Path $shotDir 'e9i_splash_loading_ui.bmp'
$hwndBmp = Join-Path $shotDir 'e9i_actual_window_capture.bmp'
$hwndPng = Join-Path $shotDir 'e9i_actual_window_capture.png'
$uiPng = Join-Path $shotDir 'e9i_splash_loading_ui.png'
$caseLog = Join-Path $logDir 'e9i_splash_loading_stdout.txt'
$caseErr = Join-Path $logDir 'e9i_splash_loading_stderr.txt'
...
$env:JJFB_E9I_COORD_CSV = $coordCsv
$env:JJFB_E9I_R4_CSV = $r4Csv
$env:JJFB_E9I_SEQ_CSV = $seqCsv
$env:JJFB_E9H_R4_CSV = $r4Csv
$env:JJFB_E9H_SEQ_CSV = $seqCsv
$env:JJFB_E9D_COMPARE_CSV = (Join-Path $reportDir 'e9i_name_compare_debug.csv')
$env:JJFB_E9D_REQUEST_CSV = (Join-Path $reportDir 'e9i_e9d_resource_request.csv')
$env:JJFB_E9G_REQUEST_CSV = (Join-Path $reportDir 'e9i_game_ui_request_trace.csv')
$env:JJFB_E9G_UIMODE_CSV = (Join-Path $reportDir 'e9i_ui_mode_trace.csv')
$env:JJFB_E9F_REQUEST_CSV = (Join-Path $reportDir 'e9i_e9f_request.csv')
$env:JJFB_E8Z_SCREENSHOT = $shotBmp
$env:JJFB_E8Z_SCREENSHOT_BEFORE = (Join-Path $shotDir 'e9i_before.bmp')
$env:JJFB_E8Z_SCREENSHOT_AFTER = (Join-Path $shotDir 'e9i_after.bmp')
...
switch ($Mode) {
  'natural' {
    ...
    Remove-Item Env:JJFB_SPLASH_COORD_ASSIST -EA SilentlyContinue
  }
  'coordassist' {
    $env:JJFB_FAST_SPLASH_CALL = '1'
    $env:JJFB_E9G_UI_MODE_ASSIST = '1'
    $env:JJFB_SPLASH_COORD_ASSIST = '1'
    $env:JJFB_E9I_SKIP_SIBLING = '0'
  }
  'trace' {
    $env:JJFB_FAST_SPLASH_CALL = '1'
...
Write-Host "=== E9I Mode=$Mode timeout=${CASE_TIMEOUT_SEC}s hold=${HoldSec}s NOT_PRODUCT ==="
[Console]::Out.Flush()
...
if (-not $p.WaitForExit($OUTER_KILL_SEC * 1000)) {
  Write-Host "E9I outer kill after ${OUTER_KILL_SEC}s"
  try { Stop-Process -Id $p.Id -Force -EA SilentlyContinue } catch {}
  Stop-E9Children
}
$src = Join-Path $logDir 'stage_e_product_robotol_stdout.txt'
if ((Test-Path $src) -and (Select-String -Path $src -Pattern 'JJFB_E9I_|JJFB_E9H_BP\]|JJFB_E9I_SCR_DIM' -Quiet -EA SilentlyContinue)) {
  Copy-Item -Force $src $caseLog
}
if ($p.ExitCode -ne 0 -and $p.ExitCode -ne $null) {
  Write-Host "E9I child exit=$($p.ExitCode) — see $caseErr"
...
- **Textbar**: $($an.textbar)
- **Top**: $($an.top)
- **Natural coord**: $($an.natural_coord)
- **Coord assist used**: $($an.coord_assist)
- **R4 zero**: $($an.r4_zero)
- **Rewrite**: $($an.rewrite)
- **Log**: ``$caseLog``
- **Coord CSV**: ``$coordCsv``
...
## Natural coordinate formula (decoded)
- R9+0x830 via 0x2F9970 = screen W; x=(W-bmp_w)/2 into R6
- R9+0x834 via 0x2F9964 = Y base; y=Ybase-100 into R5
- E9H (-100,-100) was W=0 / Ybase=0 (unseeded)
- E9I seeds those fields for natural xy without COORD_ASSIST
- Call path: 0x2EFA36 loadingbar -> 0x2EFA46 bar!16!18 -> 0x2EFA56 textbar -> 0x2EFA5C y-calc -> 0x2EFA9A -> 0x2EC6B8
- 0x2EFA9E is after blit (progress loop), not the blit target

## Splash object / r4 (decoded)
- Splash UI object = R9+0xBA0
- Sibling bind slots: +0x28 loadingbar, +0x20 bar!16!18, +0x24 textbar!120!30
- r4 = *(R9+0xBD0) = splash_obj+0x30 (loaded at 0x2EF886) -- NOT filled by sibling binds
- Gate 0x2EFAF2 peeks *(R9+0xB6C); if zero then cmp r4,#0 at 0x2EFAFA

## Rules
- NOT product success. No rewrite / no bridge success.
- COORD_ASSIST is diagnostic only.
"@
Set-Content -Path $verdictMd -Value $md -Encoding UTF8

Write-Host "E9I class=$($an.class) evidence=$($an.evidence) elapsed=${elapsed}s"
