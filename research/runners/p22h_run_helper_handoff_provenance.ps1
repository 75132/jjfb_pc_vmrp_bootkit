...
# Lane A freeze. NATURAL_ONLY. No headless / FAST 6-8-0 / Host helper invoke.
param(
  [int]$Seconds = 240,
  [int]$HoldSec = 8,
  [switch]$SkipBuild
...
$runId = ('p22h_{0:yyyyMMdd_HHmmss}_{1}' -f (Get-Date), (Get-Random -Maximum 99999))
$outDir = Join-Path $Root "out\p22h\$runId"
$reportDir = Join-Path $Root "reports\p22h\$runId"
$logDir = Join-Path $Root "logs\p22h\$runId"
...
$identity = Join-Path $outDir 'p22h_build_identity.txt'
$verdict = Join-Path $reportDir 'p22h_helper_handoff_verdict.md'
...
gate=P22H_CLEAN_helper_handoff_provenance
NATURAL_ONLY=yes
Lane=A
research_assisted=0
product_valid=1
...
$vmLog = Join-Path $logDir 'p22h_vmrp.txt'
$stderr = Join-Path $logDir 'p22h_stderr.txt'
...
$wl = [ordered]@{
  ...
  JJFB_P22H_RUN_ID = $runId
  JJFB_P22H_CLEAN = '1'
  JJFB_P22H_SOURCE_COMMIT = $sourceCommit
  JJFB_P22H_MAIN_SHA = $mainSha
  JJFB_P22H_RAW_EXT_SHA = $rawExtSha
  JJFB_P21_MODE = '1'
  JJFB_E10A_MODE = '1'
  ...
  JJFB_E10A31_TIMER_CSV = (Join-Path $reportDir 'p22h_e10a31_timer.csv')
  JJFB_E10A31B_MODE = '1'
  JJFB_E10A31B_PUB_CSV = (Join-Path $reportDir 'p22h_e10a31b_pub.csv')
  JJFB_E10A31_CFG_GATE = '1'
  JJFB_E10A31_PARAM_TRACE = '1'
  JJFB_E10A31_PARAM_CSV = (Join-Path $reportDir 'p22h_e10a31_param.csv')
  JJFB_E10A31_CFG_GATE_CSV = (Join-Path $reportDir 'p22h_e10a31_cfg_gate.csv')
  JJFB_P19_HANDOFF = '1'
  JJFB_P19_OUT_DIR = $outDir
  GWY_P19_PARENT_CHILD_HANDOFF = '1'
  JJFB_P20_CLEAN = '1'
  JJFB_P22H_PROV_CSV = (Join-Path $reportDir 'p22h_method1_call_provenance.csv')
  JJFB_P22H_BOUND_CSV = (Join-Path $reportDir 'p22h_helper_call_boundaries.csv')
  JJFB_P22H_PTRW_CSV = (Join-Path $reportDir 'p22h_helper_pointer_writes.csv')
  JJFB_P22H_PTRR_CSV = (Join-Path $reportDir 'p22h_helper_pointer_reads.csv')
  JJFB_P22H_HANDOFF_CSV = (Join-Path $reportDir 'p22h_cfunction_return_handoff.csv')
  JJFB_P22H_SLICE_CSV = (Join-Path $reportDir 'p22h_parent_dispatch_slice.csv')
  JJFB_P22H_MATRIX_CSV = (Join-Path $reportDir 'p22h_method_dispatch_matrix.csv')
  JJFB_P22H_DISASM = (Join-Path $reportDir 'p22h_parent_dispatch_disasm.txt')
  JJFB_P22H_VERDICT = $verdict
  JJFB_P22H_SUMMARY = (Join-Path $outDir 'p22h_runtime_summary.txt')
  JJFB_P22H_IDENTITY = $identity
}
...
@(
  ...
  'JJFB_P22F_CLEAN','JJFB_P22G_CLEAN','JJFB_FAST_REAL_GAMELIST_INIT_SEQUENCE',
  'JJFB_ROBOTOL_RETRY_AFTER_CONTEXT_RESTORE'
) | ForEach-Object { Remove-Item -Path ("Env:{0}" -f $_) -EA SilentlyContinue }

Write-Host "=== P22H-CLEAN Lane A run_id=$runId seconds=$Seconds ==="
...
$summaryPath = Join-Path $outDir 'p22h_runtime_summary.txt'
$summary = @{}
...
sole_lock=process killed before p22h_finalize
stop_reason=runner_timeout
guest_state_written=0
events_injected=0
headless=0
fast_init=0
p22h_final=$(FirstLine '\[JJFB_P22H_FINAL\][^\n]+')
...
$r = [ordered]@{
  ...
  gamelist = [int]((Hit 'GAMELIST_STARTED|SHELL_PHASE_GAMELIST_LOAD|JJFB_P22H\] gamelist_started') -gt 0)
  ...
  m1 = if ($summary['method1_seen']) { $summary['method1_seen'] } else { '?' }
  lock = if ($summary['sole_lock']) { $summary['sole_lock'] } else { '?' }
}

Copy-Item $vmLog (Join-Path $outDir 'p22h_vmrp.txt') -Force -EA SilentlyContinue
Copy-Item $stderr (Join-Path $outDir 'p22h_stderr.txt') -Force -EA SilentlyContinue
if (Test-Path $identity) { Copy-Item $identity (Join-Path $reportDir 'p22h_build_identity.txt') -Force -EA SilentlyContinue }

@(
  'p22h_method1_call_provenance.csv',
  'p22h_helper_call_boundaries.csv',
  'p22h_helper_pointer_writes.csv',
  'p22h_helper_pointer_reads.csv',
  'p22h_cfunction_return_handoff.csv',
  'p22h_parent_dispatch_slice.csv',
  'p22h_method_dispatch_matrix.csv',
  'p22h_parent_dispatch_disasm.txt'
) | ForEach-Object {
...
if (-not (Test-Path $verdict)) {
  @"
# P22H-CLEAN helper handoff verdict

## Bottom line

**Class: $($r.class)** (runner fallback)

$($r.lock)

$($r.p22h_final)

See out/p22h/$runId/ and reports/p22h/$runId/.
...
Write-Host "=== P22H-CLEAN done class=$($r.class) helper=$($r.helper) m1=$($r.m1) freeze=$(if($freezeOk){'PASS'}else{'FAIL'}) fire=$($r.fire_ext_n) ==="
