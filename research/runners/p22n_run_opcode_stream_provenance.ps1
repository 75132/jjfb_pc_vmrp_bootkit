# P22N-CLEAN: cfunction opcode stream + UI/init record provenance. NATURAL_ONLY.
# Corrects P22M: 0x9C41C = NORMAL_OPCODE_DISPATCH (not a lock). No [object+0x30]|=0x0C.
param(
  [int]$Seconds = 360,
  [int]$HoldSec = 8,
  [switch]$SkipBuild
...
$runId = ('p22n_{0:yyyyMMdd_HHmmss}_{1}' -f (Get-Date), (Get-Random -Maximum 99999))
$outDir = Join-Path $Root "out\p22n\$runId"
$reportDir = Join-Path $Root "reports\p22n\$runId"
$logDir = Join-Path $Root "logs\p22n\$runId"
New-Item -ItemType Directory -Force -Path $outDir, $reportDir, $logDir | Out-Null

$identity = Join-Path $outDir 'p22n_build_identity.txt'
$verdict = Join-Path $reportDir 'p22n_opcode_producer_verdict.md'
...
# Seed identity WITHOUT a cfunction SHA — runtime overwrites with final dump SHA only.
@"
run_id=$runId
...
$vmLog = Join-Path $logDir 'p22n_vmrp.txt'
$stderr = Join-Path $logDir 'p22n_stderr.txt'
...
$wl = [ordered]@{
  ...
  JJFB_E10A31_TIMER_CSV = (Join-Path $reportDir 'p22n_e10a31_timer.csv')
  JJFB_E10A31B_MODE = '1'
  JJFB_E10A31B_PUB_CSV = (Join-Path $reportDir 'p22n_e10a31b_pub.csv')
  JJFB_E10A31_CFG_GATE = '1'
  JJFB_E10A31_PARAM_TRACE = '1'
  JJFB_E10A31_PARAM_CSV = (Join-Path $reportDir 'p22n_e10a31_param.csv')
  JJFB_P19_HANDOFF = '1'
  JJFB_P19_OUT_DIR = $outDir
  GWY_P19_PARENT_CHILD_HANDOFF = '1'
  JJFB_P20_CLEAN = '1'
  JJFB_P22I_STACK_CSV = (Join-Path $reportDir 'p22n_helper_call_stack.csv')
  JJFB_P22I_RETURNS_CSV = (Join-Path $reportDir 'p22n_p22i_helper_returns.csv')
  JJFB_P22I_PROV_CSV = (Join-Path $reportDir 'p22n_method_value_provenance.csv')
  JJFB_P22I_R9_CSV = (Join-Path $reportDir 'p22n_r9_owner_timeline.csv')
  JJFB_P22I_POST_CSV = (Join-Path $reportDir 'p22n_p22i_post_init_timeline.csv')
  JJFB_P22I_XREF_CSV = (Join-Path $reportDir 'p22n_p22i_xrefs.csv')
  JJFB_P22I_MATRIX_CSV = (Join-Path $reportDir 'p22n_method_dispatch_matrix.csv')
  JJFB_P22I_CALLSITE = (Join-Path $reportDir 'p22n_cfunction_helper_callsite.txt')
  JJFB_P22I_BRANCH_MD = (Join-Path $reportDir 'p22n_p22i_return_branch_chain.md')
  JJFB_P22I_VERDICT = (Join-Path $reportDir 'p22n_p22i_dispatcher_verdict.md')
  JJFB_P22I_SUMMARY = (Join-Path $outDir 'p22n_p22i_runtime_summary.txt')
  JJFB_P22I_IDENTITY = $identity
  JJFB_P22N_IDENTITY = $identity
  JJFB_P22N_CF_BIN = $cfBin
  JJFB_P22N_CF_SHA = $cfSha
  JJFB_P22N_STREAM_CSV = (Join-Path $reportDir 'p22n_opcode_stream.csv')
  JJFB_P22N_JT_CSV = (Join-Path $reportDir 'p22n_opcode_jump_table.csv')
  JJFB_P22N_WRITES_CSV = (Join-Path $reportDir 'p22n_record_writes.csv')
  JJFB_P22N_PROV_CSV = (Join-Path $reportDir 'p22n_record_provenance.csv')
  JJFB_P22N_OP14_CSV = (Join-Path $reportDir 'p22n_opcode14_slice.csv')
  JJFB_P22N_OP1B_CSV = (Join-Path $reportDir 'p22n_opcode1b_slice.csv')
  JJFB_P22N_DIV_MD = (Join-Path $reportDir 'p22n_first_divergence.md')
  JJFB_P22N_VERDICT = $verdict
  JJFB_P22N_SUMMARY = (Join-Path $outDir 'p22n_runtime_summary.txt')
}
...
@(
  ...
  'JJFB_P22L_CLEAN','JJFB_P22M_CLEAN',
...
Write-Host "=== P22N opcode stream provenance run_id=$runId seconds=$Seconds ==="
...
# Enrich jump-table tags from exported bin (static)
$jtCsv = Join-Path $reportDir 'p22n_opcode_jump_table.csv'
if ((Test-Path $cfBin) -and (Test-Path $jtCsv)) {
  & python (Join-Path $Root 'tools\p22n_enrich_jump_table.py') `
    --bin $cfBin --base 0x80000 `
    --jt-csv $jtCsv `
    --out $jtCsv
} elseif (Test-Path $cfBin) {
  Write-Host "[P22N] jump-table csv missing; skip enrich (finalize may not have run)"
}

$summaryPath = Join-Path $outDir 'p22n_runtime_summary.txt'
$summary = @{}
...
Copy-Item $vmLog (Join-Path $outDir 'p22n_vmrp.txt') -Force -EA SilentlyContinue
Copy-Item $stderr (Join-Path $outDir 'p22n_stderr.txt') -Force -EA SilentlyContinue
if (Test-Path $identity) { Copy-Item $identity (Join-Path $reportDir 'p22n_build_identity.txt') -Force -EA SilentlyContinue }

# Ensure identity has ONLY final dump SHA (runtime should have rewritten; enforce here too)
...
if (Test-Path $identity) {
  ...
  Copy-Item $identity (Join-Path $reportDir 'p22n_build_identity.txt') -Force
}
...
$r = [ordered]@{
  ...
  op14 = if ($summary['saw_op14']) { $summary['saw_op14'] } else { '?' }
  op1b = if ($summary['saw_op1b']) { $summary['saw_op1b'] } else { '?' }
...
if (-not (Test-Path $verdict)) {
  @"
# P22N verdict (runner fallback)

$($r.lock)

- sha=$($r.sha) opcodes=$($r.opcodes) stream=$($r.stream_n) fire2=$($r.fire2)
- op14=$($r.op14) op1b=$($r.op1b)
- 0x9C41C=NORMAL_OPCODE_DISPATCH
"@ | Set-Content $verdict -Encoding utf8
}

Write-Host "=== P22N done sha=$($r.sha) opcodes=$($r.opcodes) stream=$($r.stream_n) fire2=$($r.fire2) op14=$($r.op14) op1b=$($r.op1b) ==="
