# Stage E9T: lift BD0 init to upstream caller + assist reduction — NOT product success.
# Modes: upstream | reduce | trace
# Prefer JJFB_FAST_BD0_CALLER_2FC03C over bare FAST_BD0_INIT_CALL.
param(
  [int]$Seconds = 90,
  [int]$HoldSec = 12,
  [int]$Zoom = 2,
  [string]$Target = 'gwy/jjfb.mrp',
  [ValidateSet('upstream','reduce','trace')]
...
function Analyze-E9T([string]$log, [string]$CaseMode) {
  $t = Get-Content $log -Raw -ErrorAction SilentlyContinue
  if (-not $t) { return [pscustomobject]@{ class = 'NO_LOG'; evidence = 'EMPTY' } }
  $c03 = $t -match 'BD0_UPSTREAM_CALLER_2FC03C_WORKS'
  $c05 = $t -match 'BD0_UPSTREAM_CALLER_2FC05E_WORKS'
  $c30 = $t -match 'BD0_UPSTREAM_CALLER_30EE8A_WORKS'
  $needObj = $t -match 'BD0_UPSTREAM_CALLER_REQUIRES_OBJECT'
  $upVis = $t -match 'SPLASH_VISIBLE_WITH_UPSTREAM_BD0_CALLER'
  $cnt = $t -match 'PROGRESS_COUNT_REAL_CALLER_RESTORED'
  $fb418 = $t -match 'BD0_REAL_INIT_CALL_WRITES_BD0|E9T_FALLBACK'
  $stateOff = $t -match 'STATE_ASSIST_REMOVED_SPLASH_VISIBLE'
  $c9dOff = $t -match 'C9D_ASSIST_REMOVED_SPLASH_VISIBLE'
  $c9dBlock = $t -match 'C9D_GATE_STILL_BLOCKS|DISPLAY_FIRST_BRANCH_ASSIST\] gate=C9D'
  $noC9d = $t -match 'E9T_REDUCE\] C9D_ASSIST=0'
  ...
  $poke = $t -match 'JJFB_FAST_SPLASH_PROGRESS_OBJECT_ASSIST\]'
  $cls = 'PRODUCT_STILL_NEEDS_C9D_STATE_CLEANUP'
  if ($c03) { $cls = 'BD0_UPSTREAM_CALLER_2FC03C_WORKS' }
  elseif ($c30) { $cls = 'BD0_UPSTREAM_CALLER_30EE8A_WORKS' }
  elseif ($c05) { $cls = 'BD0_UPSTREAM_CALLER_2FC05E_WORKS' }
  elseif ($needObj -and ($upVis -or $fb418)) { $cls = 'SPLASH_VISIBLE_WITH_UPSTREAM_BD0_CALLER' }
  if ($cnt) { $cls = 'PROGRESS_COUNT_REAL_CALLER_RESTORED' }
  if ($upVis -and ($draw -or $ui)) { $cls = 'SPLASH_VISIBLE_WITH_UPSTREAM_BD0_CALLER' }
  if ($CaseMode -match 'no_ui|case2') {
    if ($stateOff -or ($noC9d -eq $false -and $splash -and ($draw -or $ui))) {
      $cls = 'STATE_ASSIST_REMOVED_SPLASH_VISIBLE'
    }
  }
  if ($CaseMode -match 'no_c9d|case3') {
    if ($splash -and ($draw -or $ui) -and $noC9d) {
      $cls = 'C9D_ASSIST_REMOVED_SPLASH_VISIBLE'
    } elseif ($noC9d -and -not $splash) {
      $cls = 'C9D_GATE_STILL_BLOCKS_SPLASH'
    } elseif ($noC9d -and $c9dBlock -and -not ($draw -or $ui)) {
      $cls = 'C9D_GATE_STILL_BLOCKS_SPLASH'
    } elseif ($noC9d -and -not ($draw -or $ui)) {
      $cls = 'DISPLAYFIRST_STILL_REQUIRED'
    }
  }
  return [pscustomobject]@{
    class = $cls
    evidence = if ($c03 -or $upVis -or $draw -or $c9dBlock) { 'OBSERVED' } else { 'HYPOTHESIS' }
    upstream_2fc03c = [bool]$c03
    count_restored = [bool]$cnt
...
function Invoke-E9TCase([string]$CaseName, [hashtable]$Opts) {
  $upCsv = Join-Path $reportDir 'e9t_bd0_upstream_trace.csv'
  $cntCsv = Join-Path $reportDir 'e9t_count_writer_trace.csv'
  $ladCsv = Join-Path $reportDir 'e9t_assist_reduction_ladder.csv'
  $verdictMd = Join-Path $reportDir 'stage_e9t_fast_cleanup_verdict.md'
  $shotBmp = Join-Path $shotDir 'e9t_splash_visible.bmp'
  $hwndBmp = Join-Path $shotDir 'e9t_actual_window_capture.bmp'
  $hwndPng = Join-Path $shotDir 'e9t_actual_window_capture.png'
  $uiPng = Join-Path $shotDir 'e9t_splash_visible.png'
  $caseLog = Join-Path $logDir 'e9t_fast_cleanup_stdout.txt'
  $caseErr = Join-Path $logDir 'e9t_fast_cleanup_stderr.txt'
  ...
  $env:JJFB_E9T_UPSTREAM_CSV = $upCsv
  $env:JJFB_E9T_COUNT_CSV = $cntCsv
  $env:JJFB_E9T_LADDER_CSV = $ladCsv
  $env:JJFB_E9S_WRITER_CSV = (Join-Path $reportDir 'e9t_e9s_bd0_compat.csv')
  $env:JJFB_E9S_FN_CSV = (Join-Path $reportDir 'e9t_e9s_fn_compat.csv')
  $env:JJFB_E8Z_SCREENSHOT = $shotBmp
  $env:JJFB_E8Z_SCREENSHOT_BEFORE = (Join-Path $shotDir 'e9t_before.bmp')
  $env:JJFB_E8Z_SCREENSHOT_AFTER = (Join-Path $shotDir 'e9t_after.bmp')
  ...
  if ($Opts.ContainsKey('Caller') -and $Opts.Caller -eq '30EE8A') {
    Remove-Item Env:JJFB_FAST_BD0_CALLER_2FC03C -EA SilentlyContinue
    $env:JJFB_FAST_BD0_CALLER_30EE8A = '1'
  }
  if ($Opts.ContainsKey('NoUiAssist') -and $Opts.NoUiAssist) {
    $env:JJFB_E9T_NO_UI_MODE_ASSIST = '1'
    Remove-Item Env:JJFB_E9G_UI_MODE_ASSIST -EA SilentlyContinue
    $env:JJFB_E9G_UI_MODE_ASSIST = '0'
  }
  if ($Opts.ContainsKey('NoC9d') -and $Opts.NoC9d) {
    $env:JJFB_E9T_NO_C9D_ASSIST = '1'
    Remove-Item Env:JJFB_BYPASS_C9D_GATE -EA SilentlyContinue
    $env:JJFB_BYPASS_C9D_GATE = '0'
  }
  if ($Opts.ContainsKey('TraceOnly') -and $Opts.TraceOnly) {
    Remove-Item Env:JJFB_FAST_BD0_CALLER_2FC03C -EA SilentlyContinue
    Remove-Item Env:JJFB_FAST_BD0_CALLER_30EE8A -EA SilentlyContinue
    $env:JJFB_FAST_BD0_INIT_CALL = '0'
  }
  ...
  Write-Host "=== E9T case=$CaseName timeout=${CASE_TIMEOUT_SEC}s NOT_PRODUCT ==="
  [Console]::Out.Flush()
  $t0 = Get-Date
  $argList = @(
  ...
  if (-not $p.WaitForExit($OUTER_KILL_SEC * 1000)) {
    Write-Host "E9T outer kill after ${OUTER_KILL_SEC}s"
    try { Stop-Process -Id $p.Id -Force -EA SilentlyContinue } catch {}
    Stop-E9Children
  }
  $src = Join-Path $logDir 'stage_e_product_robotol_stdout.txt'
  if ((Test-Path $src) -and (Select-String -Path $src -Pattern 'JJFB_E9T_|FAST_BD0_CALLER|2FC03C|BD0_' -Quiet -EA SilentlyContinue)) {
  ...
  $an = Analyze-E9T $caseLog $CaseName
...
- **Elapsed**: ${elapsed}s
- **Upstream 2FC03C works**: $($an.upstream_2fc03c)
- **Progress count restored**: $($an.count_restored)
- **PROGRESS_OBJECT poke**: $($an.poke_assist)
- **Splash / loading / progress**: $($an.splash) / $($an.loading_ui) / $($an.progress)
- **0x11F00 / transparent**: $($an.draw11) / $($an.transparent)
- **Log**: ``$caseLog``
- **Upstream CSV**: ``$upCsv``
- **Count CSV**: ``$cntCsv``
- **Ladder CSV**: ``$ladCsv``
...
## Notes
- Prefer ``0x2FC03C`` (loads string, BL ``0x2FC418``); does not write ``BA0+0x2C`` in static decode
- No direct BD0 / count poke as success
- Remaining: DisplayFirst/C9D if still assisted; progress count natural writer
"@
  [System.IO.File]::WriteAllText($verdictMd, $md, [System.Text.UTF8Encoding]::new($false))
  Write-Host "== E9T_CASE_DONE name=$CaseName verdict=$($an.class) elapsed=${elapsed}s =="
  return $an
}

if (-not $SkipBuild) {
  Write-Host '=== E9T rebuild launcher_core + vmrp gwy ==='
...
$final = $null
if ($Mode -eq 'upstream' -or $Mode -eq 'trace') {
  $final = Invoke-E9TCase -CaseName 'upstream_2fc03c' -Opts @{ Caller = '2FC03C' }
} elseif ($Mode -eq 'reduce') {
  $c1 = Invoke-E9TCase -CaseName 'case1_upstream' -Opts @{ Caller = '2FC03C' }
  $final = $c1
  if ($c1.upstream_2fc03c -or $c1.draw11 -or $c1.loading_ui) {
    $c2 = Invoke-E9TCase -CaseName 'case2_no_ui' -Opts @{ Caller = '2FC03C'; NoUiAssist = $true }
    $final = $c2
    if ($c2.splash -or $c2.draw11 -or $c2.loading_ui) {
      $c3 = Invoke-E9TCase -CaseName 'case3_no_c9d' -Opts @{ Caller = '2FC03C'; NoUiAssist = $true; NoC9d = $true }
      $final = $c3
    }
  }
}

Write-Host "verdict=$(Join-Path $reportDir 'stage_e9t_fast_cleanup_verdict.md')"
if ($final -and $final.class -match 'NO_LOG') { exit 1 }
exit 0
