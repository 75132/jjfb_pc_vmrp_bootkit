...
$summary = [ordered]@{
  ...
  hwnd_capture = if (Test-Path $hwndPng) { 'evidence/screenshots/e9b_actual_window_capture.png' } else { $null }
  hwnd_nonwhite = $hits.hwnd_nonwhite
  internal_surface = if (Test-Path $internalPng) { 'evidence/screenshots/e9b_internal_surface.png' } else { $null }
  bmp=$hits.bmp
  other=$hits.other
