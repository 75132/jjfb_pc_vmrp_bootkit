# Stage E9F: natural postmatch UI sequence (slogo/loadingbar/…) — NOT product success.
param(
  [int]$Seconds = 90,
  [int]$HoldSec = 20,
  [int]$Zoom = 2,
  [string]$Target = 'gwy/jjfb.mrp',
  [ValidateSet('logo','loading','auto','compare')]
  [string]$Mode = 'logo',
  [switch]$SkipBuild
)
...
function Clear-E9Modes {
  @(
    ...
    'JJFB_E9A_MODE','JJFB_E9B_MODE','JJFB_E9C_MODE','JJFB_E9D_MODE','JJFB_E9E_MODE','JJFB_E9F_MODE',
...
function Analyze-E9F([string]$log) {
  $h = @{
    natural_match=$false; postmatch=$false; bridge=$false; ret=$false
    first_frame=$false; presented=$false; hwnd_nw=''; name=''; rewrite=$false
    slogo=$false; loading=$false; only_wy=$false; names=@()
  }
  ...
  foreach ($m in $ms) {
    if ($m.Line -match 'name="([^"]+)"') {
      ...
      $h.names += $n
      $h.name = $n
      if ($n -match 'slogo') { $h.slogo = $true }
      if ($n -match 'loadingbar') { $h.loading = $true }
      if ($n -match 'wy_jiao1') { $h.only_wy = $true }
    }
  }
  # Auto gap: DisplayFirst request alone (match may be killed mid-scan under E9B hold wait).
  ...
  $hasSplashReq = [bool]($reqNames | Where-Object { $_ -match 'slogo|loadingbar|textbar|top!' })
  $hasWyReq = [bool]($reqNames | Where-Object { $_ -match 'wy_jiao1' })
  if ($hasWyReq -and -not $hasSplashReq -and -not $h.slogo -and -not $h.loading) {
    $h.only_wy = $true
  }
  if ($h.slogo -or $h.loading) { $h.only_wy = $false }
  return $h
}

function Case-Verdict([hashtable]$h, [string]$mode) {
  if ($h.bridge) { return 'BRIDGE_FALLBACK_STILL_REQUIRED' }
  if ($h.postmatch -and $h.natural_match -and $h.first_frame -and (($h.hwnd_nw -as [int]) -gt 0)) {
    if ($h.slogo) { return 'NATURAL_POSTMATCH_LOGO_FRAME' }
    if ($h.loading) { return 'NATURAL_POSTMATCH_LOADINGBAR_FRAME' }
    if ($h.name -match 'textbar|top!') { return 'NATURAL_UI_SEQUENCE_VISIBLE' }
    if ($mode -eq 'auto' -and $h.only_wy) { return 'GAME_REQUEST_TRACE_FOUND_NEXT_GAP' }
    return 'NATURAL_POSTMATCH_GENERALIZED'
  }
  if ($h.postmatch -and $h.natural_match -and $h.ret) { return 'POSTMATCH_HANDLE_OK_DRAW_FAILED' }
  if ($mode -eq 'auto' -and $h.only_wy -and -not $h.slogo -and -not $h.loading) {
    return 'GAME_REQUEST_TRACE_FOUND_NEXT_GAP'
  }
  if ($mode -eq 'auto' -and $h.natural_match -and -not $h.slogo -and -not $h.loading) {
    return 'GAME_REQUEST_TRACE_FOUND_NEXT_GAP'
  }
  if ($h.postmatch) { return 'NATURAL_POSTMATCH_RESOURCE_FAILED' }
  return 'PRODUCT_STILL_NEEDS_UI_STATE_MACHINE'
}

function Invoke-E9FCase([string]$caseName, [hashtable]$opts) {
  Clear-E9Modes
  Stop-E9Children
  $env:JJFB_E9F_MODE = '1'
  $env:JJFB_E9E_MODE = '1'
  ...
  if ($opts.Prefer) {
    $env:JJFB_E9F_PREFER = [string]$opts.Prefer
  }
  if ($opts.Rewrite) {
    $env:JJFB_E9F_REWRITE_REQUEST = '1'
  }
  $env:JJFB_E9D_COMPARE_CSV = (Join-Path $reportDir 'e9f_name_compare_debug.csv')
  $env:JJFB_E9D_REQUEST_CSV = (Join-Path $reportDir 'e9f_e9d_resource_request.csv')
  $env:JJFB_E9F_REQUEST_CSV = (Join-Path $reportDir 'e9f_natural_resource_request_trace.csv')
  $env:JJFB_E9F_RESULTS_JSONL = (Join-Path $reportDir 'e9f_resource_results.jsonl')
  $shotBmp = Join-Path $shotDir ([string]$opts.ShotBmp)
  $hwndBmp = Join-Path $shotDir 'e9f_actual_window_capture.bmp'
  $env:JJFB_E8Z_SCREENSHOT = $shotBmp
  $env:JJFB_E8Z_SCREENSHOT_BEFORE = (Join-Path $shotDir 'e9f_before.bmp')
  $env:JJFB_E8Z_SCREENSHOT_AFTER = (Join-Path $shotDir 'e9f_after.bmp')
  ...
  $caseLog = Join-Path $logDir 'e9f_natural_ui_sequence_stdout.txt'
  $caseErr = Join-Path $logDir 'e9f_natural_ui_sequence_stderr.txt'
  $t0 = Get-Date
  ...
  if ((Test-Path $src) -and (Select-String -Path $src -Pattern 'JJFB_E9F_|JJFB_E9E_|JJFB_E9D_NATURAL_MATCH' -Quiet -EA SilentlyContinue)) {
    Copy-Item -Force $src $caseLog
  }
  $hits = Analyze-E9F $caseLog
  $elapsed = ((Get-Date) - $t0).TotalSeconds
  $verdict = Case-Verdict $hits $caseName
  $png = [IO.Path]::ChangeExtension($shotBmp, '.png')
  [void](Convert-BmpToPng $shotBmp $png)
  [void](Convert-BmpToPng $hwndBmp (Join-Path $shotDir 'e9f_actual_window_capture.png'))
  Write-Host "== E9F_CASE_DONE name=$caseName verdict=$verdict elapsed=$([Math]::Round($elapsed,1))"
  return @{
...
Write-Host "E9F Mode=$Mode zoom=$Zoom hold=${HoldSec}s timeout=${CASE_TIMEOUT_SEC}s NOT_PRODUCT_SUCCESS"
[Console]::Out.Flush()

$cases = @()
if ($Mode -eq 'logo') {
  $cases += @{ Name='logo'; Prefer='slogo!157!58.bmp'; Rewrite=$true; ShotBmp='e9f_logo_natural.bmp' }
} elseif ($Mode -eq 'loading') {
  $cases += @{ Name='loading'; Prefer='loadingbar!201!29.bmp'; Rewrite=$true; ShotBmp='e9f_loading_sequence.bmp' }
} elseif ($Mode -eq 'compare') {
  $cases += @{ Name='logo'; Prefer='slogo!157!58.bmp'; Rewrite=$true; ShotBmp='e9f_logo_natural.bmp' }
  $cases += @{ Name='loading'; Prefer='loadingbar!201!29.bmp'; Rewrite=$true; ShotBmp='e9f_loading_sequence.bmp' }
  $cases += @{ Name='auto'; Prefer=''; Rewrite=$false; ShotBmp='e9f_auto_natural.bmp' }
} else {
  # auto: no rewrite — observe natural request sequence
  $cases += @{ Name='auto'; Prefer=''; Rewrite=$false; ShotBmp='e9f_auto_natural.bmp' }
}
...
foreach ($c in $cases) {
  ...
  $r = Invoke-E9FCase $c.Name $c
  $results += $r
  if ($r.verdict -match 'LOGO_FRAME|LOADINGBAR_FRAME|UI_SEQUENCE_VISIBLE|GAME_REQUESTED_UI') {
    $stop = $true
  }
}
...
# Nearest blocker note for auto / no-slogo
$blocker = 'none'
if ($verdict -eq 'GAME_REQUEST_TRACE_FOUND_NEXT_GAP') {
  $blocker = 'DisplayFirst/C9D path only requests wy_jiao1; splash UI_MODE=0x45 / 0x2EF86C (slogo/loadingbar) not entered under current assists'
} elseif ($hits.name -match 'loadingbar' -and $Mode -eq 'logo') {
  $blocker = 'slogo!157!58.bmp absent from guest 0x304BF0 index (present in jjfb.mrp); logo mode falls back to loadingbar'
}

$flow = [ordered]@{
  stage='E9F'; mode=$Mode; verdict=$verdict; elapsed_sec=$best.elapsed
  natural_match=[bool]$hits.natural_match; postmatch=[bool]$hits.postmatch
  bridge=[bool]$hits.bridge; rewrite=[bool]$hits.rewrite
  first_frame=[bool]$hits.first_frame; hwnd_nonwhite=$hits.hwnd_nw
  member=$hits.name; members=$hits.names; nearest_blocker=$blocker
  note='NOT_PRODUCT_SUCCESS; natural postmatch UI sequence'
}
($flow | ConvertTo-Json -Depth 6) | Set-Content (Join-Path $reportDir 'e9f_natural_resource_flow.json') -Encoding utf8
...
$md = @(
  ...
  '- Draw dim fix: guest sometimes passes square ``w×w``; host overrides from ``JJFB_E9E_LAST_WH`` (postmatch member size).',
  '',
  '## Result',
  '',
  ...
  '',
  '## Why only wy_jiao first (auto)',
  '',
  'Current DisplayFirst / C9D / F6C/F74 assist path loads a small sprite first.',
  'Legacy splash chrome (slogo/loadingbar/textbar/top) is gated by UI_MODE ``0x45`` / ``0x2EF86C`` — not reached under this assist stack.',
  '``slogo!157!58.bmp`` is in the MRP package but **not** in the guest ``0x304BF0`` index.',
  '',
  '## Artifacts',
  '',
...
[System.IO.File]::WriteAllText((Join-Path $reportDir 'stage_e9f_natural_ui_sequence_verdict.md'), (($md -join "`n") + "`n"), [System.Text.UTF8Encoding]::new($false))
