...
$summary = [ordered]@{
  ...
  zoom = $Zoom
  hold_sec = $HoldSec
  hwnd_capture = if (Test-Path $hwndPng) { 'evidence/screenshots/e9c_actual_window_capture.png' } else { $null }
  hwnd_nonwhite = $hits.hwnd_nonwhite
  internal_surface = if (Test-Path $internalPng) { 'evidence/screenshots/e9c_internal_surface.png' } else { $null }
  other = $hits.other
