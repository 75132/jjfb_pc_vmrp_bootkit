# Stage E9Q: formal platform text-measure API 0x12340 — NOT product success.
# Modes: trace | api | compare
# Success path: JJFB_PLATFORM_TEXT_MEASURE_12340=1, NO FAST_TEXT_MEASURE_SHIM.
param(
  [int]$Seconds = 90,
  [int]$HoldSec = 12,
  [int]$Zoom = 2,
...
function Clear-E9Modes {
  @(
    ...
    'JJFB_E9P_MODE','JJFB_E9Q_MODE','JJFB_VISIBLE_WINDOW',
...
function Analyze-E9Q([string]$log) {
  $t = Get-Content $log -Raw -ErrorAction SilentlyContinue
  if (-not $t) { return [pscustomobject]@{ class = 'NO_LOG'; evidence = 'EMPTY' } }
  $apiOk = $t -match 'JJFB_E9Q_CLASS\] class=PLATFORM_TEXT_MEASURE_12340_RENDERED[^_]'
  $apiFb = $t -match 'PLATFORM_TEXT_MEASURE_12340_RENDERED_WITH_FONT_FALLBACK'
  $shimGone = $t -match 'TEXT_MEASURE_SHIM_REMOVED_SUCCESSFULLY'
  $shimUsed = $t -match 'JJFB_TEXT_MEASURE_SHIM\]'
  $abiWrong = $t -match 'PLATFORM_TEXT_MEASURE_12340_ABI_WRONG|BLOCKED_BY_OUTPUT_POINTER'
  $encBlock = $t -match 'PLATFORM_TEXT_MEASURE_12340_BLOCKED_BY_ENCODING'
  ...
  $cls = if (($apiOk -or $apiFb) -and $shimGone -and -not $shimUsed) { 'PLATFORM_TEXT_MEASURE_12340_RENDERED' }
    elseif ($apiFb) { 'PLATFORM_TEXT_MEASURE_12340_RENDERED_WITH_FONT_FALLBACK' }
    elseif ($apiOk -and $shimGone) { 'TEXT_MEASURE_SHIM_REMOVED_SUCCESSFULLY' }
    elseif ($shimUsed -and ($Mode -eq 'api')) { 'TEXT_MEASURE_API_REGRESSED' }
    elseif ($abiWrong) { 'PLATFORM_TEXT_MEASURE_12340_ABI_WRONG' }
    elseif ($encBlock) { 'PLATFORM_TEXT_MEASURE_12340_BLOCKED_BY_ENCODING' }
    elseif ($draw -and $trans) { 'PRODUCT_STILL_NEEDS_TEXTCTX_NATURALIZATION' }
    else { 'PRODUCT_STILL_NEEDS_TEXTCTX_NATURALIZATION' }
  if ($apiOk -and $shimGone -and -not $shimUsed -and $apiFb) {
    $cls = 'PLATFORM_TEXT_MEASURE_12340_RENDERED_WITH_FONT_FALLBACK'
  } elseif ($apiOk -and $shimGone -and -not $shimUsed) {
    $cls = 'PLATFORM_TEXT_MEASURE_12340_RENDERED'
  }
  return [pscustomobject]@{
    class = $cls
    evidence = if ($apiOk -or $apiFb -or $shimGone) { 'OBSERVED' } else { 'HYPOTHESIS' }
    api = [bool]($apiOk -or $apiFb)
    shim_removed = [bool]$shimGone
    shim_used = [bool]$shimUsed
...
function Invoke-E9QCase([string]$CaseName, [string]$CaseMode) {
  $measCsv = Join-Path $reportDir 'e9q_platform_12340_measure_calls.csv'
  $metricsCsv = Join-Path $reportDir 'e9q_measure_vs_draw_metrics.csv'
  $verdictMd = Join-Path $reportDir 'stage_e9q_text_measure_api_verdict.md'
  $shotBmp = Join-Path $shotDir 'e9q_text_measure_api_window.bmp'
  $hwndBmp = Join-Path $shotDir 'e9q_actual_window_capture.bmp'
  $hwndPng = Join-Path $shotDir 'e9q_actual_window_capture.png'
  $uiPng = Join-Path $shotDir 'e9q_text_measure_api_window.png'
  $caseLog = Join-Path $logDir 'e9q_text_measure_api_stdout.txt'
  $caseErr = Join-Path $logDir 'e9q_text_measure_api_stderr.txt'
  ...
  $env:JJFB_E9O_11F00_CSV = (Join-Path $reportDir 'e9q_e9o_11f00_compat.csv')
  $env:JJFB_E9O_DRAW_CSV = (Join-Path $reportDir 'e9q_e9o_draw_compat.csv')
  $env:JJFB_E9Q_12340_CSV = $measCsv
  $env:JJFB_E9Q_METRICS_CSV = $metricsCsv
  $env:JJFB_E8Z_SCREENSHOT = $shotBmp
  $env:JJFB_E8Z_SCREENSHOT_BEFORE = (Join-Path $shotDir 'e9q_before.bmp')
  $env:JJFB_E8Z_SCREENSHOT_AFTER = (Join-Path $shotDir 'e9q_after.bmp')
  ...
  switch ($CaseMode) {
    'trace' {
      Remove-Item Env:JJFB_PLATFORM_TEXT_MEASURE_12340 -EA SilentlyContinue
      Remove-Item Env:JJFB_FAST_TEXT_MEASURE_SHIM -EA SilentlyContinue
      # force no shim for trace proof of garbage
      $env:JJFB_FAST_TEXT_MEASURE_SHIM = '0'
    }
    'api' {
    ...
    'compare' {
      # old E9M shim path for baseline compare
      Remove-Item Env:JJFB_PLATFORM_TEXT_MEASURE_12340 -EA SilentlyContinue
  ...
  Write-Host "=== E9Q case=$CaseName mode=$CaseMode timeout=${CASE_TIMEOUT_SEC}s NOT_PRODUCT ==="
  [Console]::Out.Flush()
  ...
  if (-not $p.WaitForExit($OUTER_KILL_SEC * 1000)) {
    Write-Host "E9Q outer kill after ${OUTER_KILL_SEC}s"
    try { Stop-Process -Id $p.Id -Force -EA SilentlyContinue } catch {}
    Stop-E9Children
  }
  $src = Join-Path $logDir 'stage_e_product_robotol_stdout.txt'
  if ((Test-Path $src) -and (Select-String -Path $src -Pattern 'JJFB_E9Q_|JJFB_PLATFORM_TEXT_MEASURE_12340|JJFB_E9M_MEASURE|0x12340' -Quiet -EA SilentlyContinue)) {
...
- **Elapsed**: ${elapsed}s
- **Platform 0x12340 API**: $($an.api)
- **Measure shim removed**: $($an.shim_removed)
- **FAST_TEXT_MEASURE_SHIM used**: $($an.shim_used)
...
"@
  ...
  Write-Host "== E9Q_CASE_DONE name=$CaseName verdict=$($an.class) elapsed=${elapsed}s =="
  return $an
}

if (-not $SkipBuild) {
  Write-Host '=== E9Q rebuild launcher_core + vmrp gwy ==='
...
$result = Invoke-E9QCase -CaseName $Mode -CaseMode $Mode
Write-Host "verdict=$(Join-Path $reportDir 'stage_e9q_text_measure_api_verdict.md')"
exit 0
