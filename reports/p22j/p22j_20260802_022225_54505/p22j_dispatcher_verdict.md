...
调用指令：BLX r12
target register：r12
R12 producer：UNKNOWN_NOT_EXPOSED
caller function：0x80000 .. 0x89DEC
...
是否需要 thunk 内部切换：UNKNOWN_NOT_EXPOSED
ABI 是否正确：OBSERVED_AS_IS
...
是否注入事件：NO
是否启用 FAST：NO
当前唯一门锁：natural contract is 6→0→1 with child R9; historical 6→8→0 unproven
下一处最小通用修复：trace first blocking branch after method=1 return (not patch method8)
...
- DSM_TO_MRP_HELPER hits: 3
- DSM_RETURN_FROM_MRP hits: 3
- helper_enter 6/0/1 with child R9 0x682B8C: 3
- helper_enter 6/0/1 with parent R9 0x280400: 0
- r9_fix_ok: YES
