...
int32_t my_open(const char *filename, uint32_t mode) {
    ...
    /* Thin hook: when VmFileService is bound, never fall back to host cwd open. */
    if (gwy_vm_file_is_bound()) {
        return gwy_vm_file_open(filename, mode);
    }
...
int32_t my_close(int32_t f) {
    uIntMap *obj;
    if (gwy_vm_file_is_bound()) {
        return gwy_vm_file_close(f);
    }
    obj = uIntMap_delete(&filef_map, f);
    if (obj == NULL) {
        return MR_FAILED;
    }
    if (f == filef_count) {
...
int32_t my_seek(int32_t f, int32_t pos, int method) {
    uIntMap *obj;
    if (gwy_vm_file_is_bound()) {
        return gwy_vm_file_seek(f, pos, method);
    }
    obj = uIntMap_search(&filef_map, f);
    if (obj == NULL) {
        return MR_FAILED;
    }
...
int32_t my_write(int32_t f, void *p, uint32_t l) {
    uIntMap *obj;
    if (gwy_vm_file_is_bound()) {
        return gwy_vm_file_write(f, p, l);
    }
    obj = uIntMap_search(&filef_map, f);
    if (obj == NULL) {
        return MR_FAILED;
...
int32_t my_rename(const char *oldname, const char *newname) {
    int ret;
    if (gwy_vm_file_is_bound()) {
        /* Follow-up: not yet routed through GuestVFS; fail closed. */
        (void)oldname;
        (void)newname;
        return MR_FAILED;
    }
...
int32_t my_getLen(const char *filename) {
    ...
    int ret;
    if (gwy_vm_file_is_bound()) {
        return gwy_vm_file_get_len(filename);
    }
...
int32_t my_info(const char *filename) {
    ...
    int ret;
    if (gwy_vm_file_is_bound()) {
        return gwy_vm_file_info(filename);
    }
    ret = stat(filename, &s1);
