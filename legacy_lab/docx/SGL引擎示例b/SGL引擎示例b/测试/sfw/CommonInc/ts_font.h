 ...
 * 遇到换行符立即返回
 */
int32 tsf_drawText(uint8 *pText, int16 x, int16 y,  mr_colourSt colorst, uint16 flag);
...
/**
  ...
  文件头：128字节 
		...
		6~7：预留
		8~11：字库文件总字节数 
		12~15：字库索引表偏移 16~19：字库索引表长度 
		20~23：字库点阵偏移 24~27：字库点阵长度
		28：GB字符宽度 29：ASIC字符宽度 30：字符高度（相同） 31：预留
		40~59：字库名
		60~127：预留
 */
#endif