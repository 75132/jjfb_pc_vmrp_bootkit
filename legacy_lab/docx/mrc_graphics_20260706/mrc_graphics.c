...
BITMAP_565 *readBitmap565FromAssets(const char *filename)
{
  ...
  if (buf == NULL)
  {
      mrc_printf("read file %s error !", filename);
      return NULL;
  }
  head = (char *)buf;
  if (len > 0)
...
int32 drawBitmapTransform(BITMAP_565 *bmp, int32 x, int32 y, int32 w, int32 h, int32 sx, int32 sy)
{
  if(!bmp)
      return MR_FAILED;
  if(bmp->color_bit == 16){
...
int32 drawBitmapFlip(BITMAP_565 *p, int32 x, int32 y, int32 w, int32 h, int32 rop, int32 sx, int32 sy) 
{
    ...
    int16 targetW, targetH;
    int16 i, j;
    int16 tx, ty;     
    int16 srcX, srcY; 
    int16 absSrcX, absSrcY; 
    uint16 *screenBuffer;
    uint16 pixelColor, transcolor;
    int32 sindex, pindex;
    uint32 *bmp32;

    if (!p || !p->bitmap) return MR_FAILED;
    
    screenBuffer = w_getScreenBuffer();
    transcolor = (uint16)p->transcolor;

    /* 【核心修复】：放弃 !(rop >> 2) 这种位运算判断，规避老旧编译器的逻辑误判 Bug 
     * 明确列举出需要互换宽高的 4 种旋转模式 */
    if (rop == TRANS_ROT90 || rop == TRANS_ROT270 || 
        rop == TRANS_MIRROR_ROT90 || rop == TRANS_MIRROR_ROT270) {
        targetW = h;
        targetH = w;
    } else {
        targetW = w;
    ...
    startX = MAX(x, 0);
    startY = MAX(y, 0);
    endX = MIN(x + targetW, SCRW);
    endY = MIN(y + targetH, SCRH);

    if (startX >= endX || startY >= endY) return MR_SUCCESS;

    /* 逐行扫描屏幕目标区域，反向映射回源图片提取像素 */
    for (i = startY; i < endY; i++) {
        ty = i - y; 
        for (j = startX; j < endX; j++) {
            tx = j - x; 
            
            /* 矩阵映射：根据翻转模式，将目标的 (tx, ty) 还原为源图的 (srcX, srcY) */
            switch (rop) {
                case TRANS_NONE:          srcX = tx;           srcY = ty;           break;
                case TRANS_MIRROR_ROT180: srcX = tx;           srcY = h - 1 - ty;   break;
                case TRANS_MIRROR:        srcX = w - 1 - tx;   srcY = ty;           break;
                case TRANS_ROT180:        srcX = w - 1 - tx;   srcY = h - 1 - ty;   break;
                case TRANS_MIRROR_ROT270: srcX = ty;           srcY = tx;           break;
                case TRANS_ROT90:         srcX = ty;           srcY = h - 1 - tx;   break;
                case TRANS_ROT270:        srcX = w - 1 - ty;   srcY = tx;           break;
                case TRANS_MIRROR_ROT90:  srcX = w - 1 - ty;   srcY = h - 1 - tx;   break;
                default:                  srcX = tx;           srcY = ty;           break;
            }
            ...
            /* 绝对安全检查：防止源图片读取越界脏内存 */
            if (absSrcX >= 0 && absSrcX < p->width && absSrcY >= 0 && absSrcY < p->height) {
                /* 【完全放弃游走指针 sindex++】每次都使用绝对坐标重新计算，彻底免疫优化 BUG */
                sindex = i * SCRW + j;
                pindex = absSrcY * p->width + absSrcX;
                
                /* 16位 RGB565 图片支持 (透明色剔除) */
                if (p->color_bit == 16) {
                    pixelColor = p->bitmap[pindex];
                    if (p->mode == BM_COPY || pixelColor != transcolor) {
                        screenBuffer[sindex] = pixelColor;
                    }
                } 
                /* 32位 ARGB8888 图片支持 (Alpha通道混合) */
                else if (p->color_bit == 32) {
                    bmp32 = (uint32 *)p->bitmap;
                    if ((bmp32[pindex] >> 24) != 0) {
                        screenBuffer[sindex] = blendColor888(screenBuffer[sindex], bmp32[pindex]);
                    }
                }
            }
        }
...
void drawBitmapRegion(BITMAP_565 *src, int x_src, int y_src, int width, int height, int transform, int x_dest, int y_dest, int anchor) {
  ...
  int16 i,j;
    ...
    // 计算绘制区域的实际边界
    endX = (startX + MIN(width, src->width - x_src) > SCRW) ? SCRW : startX + MIN(width, src->width - x_src);
...
将位图绕指定中心旋转并绘制到屏幕
b        BITMAP_565 位图对象
centerX  屏幕上的旋转中心 X 坐标（像素）
centerY  屏幕上的旋转中心 Y 坐标（像素）
bx       位图上旋转中心的 X 偏移（相对于位图左上角，像素）
by       位图上旋转中心的 Y 偏移（相对于位图左上角，像素）
r        旋转角度（单位：1/4096 弧度，正值表示逆时针旋转）
 */
void drawBitmapRotate(BITMAP_565 *b, int32 centerX, int32 centerY, int32 bx, int32 by, int32 r)
{
    ...
    /* 计算旋转角度的正余弦值（定点数，放大 4096 倍） */
    angle = r;
    cosR = mycos(angle);
    sinR = mysin(angle);
    
    /* 位图的四个角点（相对于旋转中心在图片上的偏移）*/
    vertices[0][0] = -bx;              /* 左上角 */
    vertices[0][1] = -by;
    vertices[1][0] = b->width - bx;    /* 右上角 */
    vertices[1][1] = -by;
    vertices[2][0] = b->width - bx;    /* 右下角 */
    vertices[2][1] = b->height - by;
    vertices[3][0] = -bx;              /* 左下角 */
    vertices[3][1] = b->height - by;
    
    /* 旋转四个顶点，计算旋转后位图在屏幕上的包围盒 */
    for (i = 0; i < 4; i++) {
        x = vertices[i][0];
        y = vertices[i][1];
        vertices[i][0] = centerX + (x * cosR - y * sinR) / 4096;
        vertices[i][1] = centerY + (x * sinR + y * cosR) / 4096;
    ...
    minX = MAX(0, minX);
    minY = MAX(0, minY);
    maxX = MIN(maxX, SCRW - 1);
    maxY = MIN(maxY, SCRH - 1);
    
    /* 逆向映射：遍历屏幕上的每个裁剪后像素，反算其在原图中的位置 */
    if (b->color_bit == 16) {
        if (b->mode == BM_TRANSPARENT) {
            for (y = minY; y <= maxY; y++) {
                for (x = minX; x <= maxX; x++) {
                    ...
                    /* 逆旋转（旋转矩阵的逆） */
                    newX = bx + (dx * cosR + dy * sinR) / 4096;
                    newY = by + (-dx * sinR + dy * cosR) / 4096;
                    
                    if (newX >= 0 && newX < b->width && newY >= 0 && newY < b->height) {
                        color = b->bitmap[newY * b->width + newX];
        ...
        for (y = minY; y <= maxY; y++) {
            for (x = minX; x <= maxX; x++) {
                ...
                if (newX >= 0 && newX < b->width && newY >= 0 && newY < b->height) {
                    color32 = bitmap32[newY * b->width + newX];
                         ...
                         uint16 transcolor)
{
  ...
  int16 endX = (x + MIN(w, mw-sx) > SCRW) ? SCRW : x + MIN(w, mw-sx); // 截取结束绘制的X边界
...
void _DrawBitmapEx(BITMAP_565 *srcbmp, uint16 dx, uint16 dy, uint16 sx, uint16 sy, uint16 w, uint16 h, mr_transMatrixSt *trans)
{
  ...
  dstbmp.bitmap = w_getScreenBuffer();
  dstbmp.width = SCRW;
  dstbmp.height = SCRH;
  dstbmp.color_bit = 16;
  dstbmp.mode = BM_COPY;
...
void gl_drawRotatedHollowRect(int16 centerX, int16 centerY, int16 width, int16 height, int32 bx, int32 by, float angle, uint32 color)
{
  ...
  float cosR = mycos(radian);
  float sinR = mysin(radian);
  int16 x, y;
  int i;
  // 计算矩形的四个顶点
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    vertices[i][0] = centerX + x * cosR/4096 - y * sinR/4096;
    vertices[i][1] = centerY + x * sinR/4096 + y * cosR/4096;
  }
