...
# Runtime may have written zeros into downVersion during a prior run; force
# restore the canonical BE version (0x3EE) the game compares via downVersion.v.
$srcDownVer = Join-Path $resourceSrc "gwy\jjfbol\downVersion"
$dstDownVer = Join-Path $gwyRoot "jjfbol\downVersion"
if (Test-Path $srcDownVer) {
  New-Item -ItemType Directory -Force -Path (Split-Path $dstDownVer) | Out-Null
  Copy-Item -Force $srcDownVer $dstDownVer
  Write-Host "[JJFB_DOWNVERSION] restored from game_files ($((Get-FileHash -Algorithm SHA256 $dstDownVer).Hash.Substring(0,12))...)"
}
# Stale empty download caches confuse open/remove loops.
@("0@s0.map", "0@s0.map.v") | ForEach-Object {
  $stale = Join-Path $gwyRoot ("jjfbol\" + $_)
  if (Test-Path $stale) {
    $len = (Get-Item $stale).Length
    if ($len -eq 0) { Remove-Item -Force $stale; Write-Host "[JJFB_AT_CACHE] removed empty $_" }
  }
}
...
# GWY platform lifecycle (not game ui_mode force):
#  - V64: one call into game-registered 0x10165 enqueue handler
#  - C0: family app=0xC0 via registered 1E200 (like APP2 timer) after B71
#  - 101AB: plat fill from local downVersion (default); no V74 second inject
$env:JJFB_V64_ENQUEUE_ONCE = "1"
$env:JJFB_FAMILY_C0_AFTER_B71 = "1"
$env:JJFB_V74_B58_SECOND = "0"
$env:JJFB_IMEI = $Imei
...
# TEMPORARY PROBE — opt-in only (not official). Uncomment to enable:
# $env:JJFB_SKIP_NET_LOGIN = "1"
# $env:JJFB_SKIP_NET_LOGIN_MS = "2000"
Remove-Item Env:JJFB_SKIP_NET_LOGIN -ErrorAction SilentlyContinue
Remove-Item Env:JJFB_SKIP_NET_LOGIN_MS -ErrorAction SilentlyContinue
...
$pattern = "JJFB_GWY_LAUNCH|JJFB_GWY_ROOT|JJFB_LOADER|JJFB_ROBOTOL|JJFB_PROBE_SKIP_NET|JJFB_V73_10170|JJFB_FORCE_2EFC_TAIL|JJFB_V72_10110|JJFB_NET|JJFB_FILEOPEN|FILEOPEN_MISS|FILEOPEN_AT|BMP_LOAD.*FAIL|my_malloc no memory|UC_ERR|mr_exit"
