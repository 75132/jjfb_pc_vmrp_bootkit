# Phase 6N: Restore Legitimate ExtChunk Publication
param(
  [int]$Seconds = 55,
  [switch]$SkipBuild,
  [switch]$SkipWxjwq
...
$banned = Select-String -Path (Join-Path $Root 'src\runtime\ext_chunk_provider.c') `
...
$unit = Join-Path $Root 'build-i686\test_ext_chunk_provider.exe'
if (-not (Test-Path $unit)) {
  # Ensure unit target exists after cmake refresh
  & cmake --build (Join-Path $Root 'build-i686') --target test_ext_chunk_provider
...
function Invoke-6NLive([string]$outLog, [string]$errLog, [string]$nmrpname, [int]$secs) {
  ...
  $env:GWY_MODULE_SNAPSHOT = (Join-Path $logDir 'module_registry_phase6n.json')
  ...
  while (-not $p.HasExited -and (Get-Date) -lt $deadline) {
    ...
    if (Select-String -Path $outLog -Pattern '\[JJFB_6N_SUMMARY\]|mythroad exit|UC_MEM_READ_UNMAPPED' -Quiet) {
...
Write-Host "== Phase 6N Restore ExtChunk Publication Seconds=$Seconds =="

$stdoutJj = Join-Path $logDir 'phase6n_extchunk_stdout.txt'
$stderrJj = Join-Path $logDir 'phase6n_extchunk_stderr.txt'
Invoke-6NLive $stdoutJj $stderrJj 'gwy/jjfb.mrp' $Seconds

$mid = $false
if (Test-Path $stdoutJj) {
  $txt = Get-Content -Raw $stdoutJj
  $hasPub = $txt -match '\[JJFB_EXTCHUNK_PUBLISH\].*new=0x(?!0+\b)[0-9A-Fa-f]+'
  $fault28 = $txt -match 'fault_addr=0x28\b|address\s*=\s*0x28\b|UNMAPPED.*\b0x28\b'
  $mid = $hasPub -and -not $fault28
}

$stdoutWx = Join-Path $logDir 'phase6n_extchunk_wxjwq_stdout.txt'
$stderrWx = Join-Path $logDir 'phase6n_extchunk_wxjwq_stderr.txt'
if ($mid -and -not $SkipWxjwq -and (Test-Path $wxjwq)) {
  Write-Host '-- wxjwq cross-target (mid success) --'
  Invoke-6NLive $stdoutWx $stderrWx 'gwy/wxjwq.mrp' $Seconds
} else {
...
python (Join-Path $Root 'tools\phase6n_reports.py') @reportArgs
if ($LASTEXITCODE -ne 0) { throw 'phase6n_reports failed' }
...
Copy-Item -Force (Join-Path $reportDir 'phase6n_verdict.md') (Join-Path $Root 'phase6n_verdict.md')

$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$pack = Join-Path $Root "JJFB_phase6n_restore_extchunk_publication_pack_$stamp.zip"
$items = @()
$items += Get-ChildItem (Join-Path $logDir 'phase6n_*') -File -ErrorAction SilentlyContinue
$items += Get-ChildItem (Join-Path $reportDir 'phase6n_*.md') -File -ErrorAction SilentlyContinue
...
$items += Get-Item (Join-Path $Root 'phase6n_verdict.md') -ErrorAction SilentlyContinue
Compress-Archive -Path ($items | ForEach-Object FullName) -DestinationPath $pack -Force

$report = Join-Path $logDir 'phase6n_extchunk_report.txt'
@(
  "phase=6N",
  "jjfb_hash=$hashAfter",
  "pack=$pack",
  "",
  (Get-Content (Join-Path $reportDir 'phase6n_verdict.md') -Raw)
) | Set-Content -Encoding utf8 $report

Write-Host "Phase 6N done. pack=$pack"
Write-Host "==== CONCLUSION ===="
Get-Content (Join-Path $reportDir 'phase6n_verdict.md')
exit 0
