# Phase 6P: Shell Core Continue After Init
param(
  [int]$Seconds = 70,
  [switch]$SkipBuild
)
...
$unitTargets = @(
  'test_ext_chunk_provider',
  'test_ext_er_rw_bind_restore',
  'test_ext_mrpgcmap_entry_order',
  'test_ext_gwy_shell_shim'
)
...
function Invoke-6PLive([string]$outLog, [string]$errLog, [string]$nmrpname, [int]$secs) {
  ...
  $env:GWY_MODULE_SNAPSHOT = (Join-Path $logDir 'module_registry_phase6p.json')
  ...
  while (-not $p.HasExited -and (Get-Date) -lt $deadline) {
    ...
    if (Select-String -Path $outLog -Pattern '\[JJFB_GAMELIST_STARTED\]|\[JJFB_SHELL_CORE_CONTINUE\]|mythroad exit|UC_MEM_READ_UNMAPPED|\[JJFB_6P_EXIT_SOURCE\]' -Quiet) {
      # Allow gamelist a bit more time after continue.
      if (Select-String -Path $outLog -Pattern '\[JJFB_GAMELIST_STARTED\]|mythroad exit|UC_MEM_READ_UNMAPPED' -Quiet) {
...
Write-Host "== Phase 6P Shell Core Continue Seconds=$Seconds =="

$stdoutJj = Join-Path $logDir 'phase6p_shell_core_continue_stdout.txt'
$stderrJj = Join-Path $logDir 'phase6p_shell_core_continue_stderr.txt'
Invoke-6PLive $stdoutJj $stderrJj 'gwy/jjfb.mrp' $Seconds

python (Join-Path $Root 'tools\phase6p_reports.py') $stdoutJj $reportDir
if ($LASTEXITCODE -ne 0) { throw 'phase6p_reports failed' }
...
Copy-Item -Force (Join-Path $reportDir 'phase6p_verdict.md') (Join-Path $Root 'phase6p_verdict.md')

$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$pack = Join-Path $packDir "JJFB_phase6p_shell_core_continue_pack_$stamp.zip"
$items = @()
$items += Get-ChildItem (Join-Path $logDir 'phase6p_*') -File -ErrorAction SilentlyContinue
$items += Get-ChildItem (Join-Path $reportDir 'phase6p_*.md') -File -ErrorAction SilentlyContinue
...
$items += Get-Item (Join-Path $Root 'phase6p_verdict.md') -ErrorAction SilentlyContinue
Compress-Archive -Path ($items | ForEach-Object FullName) -DestinationPath $pack -Force

$report = Join-Path $logDir 'phase6p_shell_core_continue_report.txt'
@(
  "phase=6P",
  "jjfb_hash=$hashAfter",
  "pack=$pack",
  "",
  (Get-Content (Join-Path $reportDir 'phase6p_verdict.md') -Raw)
) | Set-Content -Encoding utf8 $report

Write-Host "Phase 6P done. pack=$pack"
Write-Host "==== CONCLUSION ===="
Get-Content (Join-Path $reportDir 'phase6p_verdict.md')
exit 0
