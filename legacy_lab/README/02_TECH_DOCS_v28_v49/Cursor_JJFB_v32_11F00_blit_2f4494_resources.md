...
> 当前最新状态：v31 已经推进到 present 中层。  
> 现在不是 loader、robotol、timer、registerAPP、12340 的问题了；当前主线是：**0x11F00 图形函数池接口、2f4494 不返回、错误 drawFP seed、缺图野读防护**。
...
```text
2e87ac -> 2f284c -> 2f4494 不返回
```

目前绕过 `2f284c` 能推进到 `0x11F00`，但长期不能永久 bypass。  
需要查清：
