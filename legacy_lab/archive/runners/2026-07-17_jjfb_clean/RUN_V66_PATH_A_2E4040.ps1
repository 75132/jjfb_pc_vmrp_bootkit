# JJFB v66 - 2F68E4 body terminator so 2E4040 reaches 2E4066/2DADC4
# Ref: v65 pushed code=5 into 2E2520/2E4040 but 2F68E4 looped on nonzero body u32.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v66_path_a_stdout.txt"
$errOut = Join-Path $logDir "v66_path_a_stderr.txt"
...
$reportOut = Join-Path $reportDir "v66_path_a_run_result.md"
...
Write-Host "=== JJFB v66 Path A (2F68E4 terminate → 2DADC4) ==="
Write-Host "project=$root"
Write-Host "target: body BE0 end-marker; 2E4040 → 2F68E4 → 2E4066 → 2DADC4"
...
if (-not $bridgeText.Contains("[JJFB_V66_101AB]") -or
    -not $bridgeText.Contains("[JJFB_V66_PATH_A]") -or
  ...
  throw "v66 overlay missing (need V66_101AB + V66_PATH_A + V64_ENQ + FAMILY_APP2)."
}
...
Write-Host "[4/6] Build v66 Path A"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v66 run: before=$runtimeHash after=$afterHash"
}
...
$analyzer = Join-Path $root "scripts\v66_analyze_path_a_log.py"
& $python.Source $analyzer $out $reportOut
if ($LASTEXITCODE -ne 0) { throw "analyzer failed" }

$pattern = "JJFB_V66_101AB|JJFB_V66_PATH_A|JJFB_V64_ENQ|JJFB_V56_EVENT|caller_2E4066|gate_init_2DADC4|uimode_writer|TARGETS_2DADC4|FORCE state"
