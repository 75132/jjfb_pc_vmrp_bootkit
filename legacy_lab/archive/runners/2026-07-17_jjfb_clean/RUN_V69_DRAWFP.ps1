# JJFB v69 - plat 0x10113 writes real _DrawBitmap into ERW+1508/150C/1510
# Fixes 0x270F sentinel so 2EC6B8 can BLX (original bmp draw, not fake UI).
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v69_drawfp_stdout.txt"
$errOut = Join-Path $logDir "v69_drawfp_stderr.txt"
...
$reportOut = Join-Path $reportDir "v69_drawfp_run_result.md"
...
Write-Host "=== JJFB v69 drawFP (10113 -> _DrawBitmap) ==="
Write-Host "project=$root"
Write-Host "target: heal 150C; original bmp blit; keep Path A dequeue"
...
if (-not $bridgeText.Contains("[JJFB_V69_DRAWFP]") -or
    -not $bridgeText.Contains("[JJFB_V69_10113]") -or
    -not $bridgeText.Contains("br__DrawBitmap") -or
  ...
  throw "v69 overlay missing (need V69_DRAWFP + V69_10113 + br__DrawBitmap + V68)."
}
...
Write-Host "[4/6] Build v69 drawFP"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v69 run: before=$runtimeHash after=$afterHash"
}
...
$pattern = "JJFB_V69_|_DrawBitmap|fp150C|V67_DRAWFP|leave_2FC26C|after_2E2520|312C0C|enter 0x2f449c|B70=|2FEBBC|uimode_writer"
