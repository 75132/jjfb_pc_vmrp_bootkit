# JJFB v65 - plat 0x101AB fill so 30D24C unpack >c>i yields r1!=0 for 2E4D6C→B54
# Ref: v64 called 10165 handler but 2E4D6C early-exited (r1=0) because 101AB unhandled.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v65_101ab_stdout.txt"
$errOut = Join-Path $logDir "v65_101ab_stderr.txt"
...
$reportOut = Join-Path $reportDir "v65_101ab_run_result.md"
...
Write-Host "=== JJFB v65 101AB Fill + Enqueue PROBE ==="
Write-Host "project=$root"
Write-Host "target: 101AB fill → unpack r1!=0 → 2E4D6C → 312A60 → B54 → 2E2520"
...
if (-not $bridgeText.Contains("[JJFB_V65_101AB]") -or
  ...
  throw "v65 overlay missing (need V65_101AB + V64_ENQ + V62_FLAG + V61_NEST + FAMILY_APP2)."
}
...
Write-Host "[4/6] Build v65 101AB enqueue"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v65 run: before=$runtimeHash after=$afterHash"
}
...
$analyzer = Join-Path $root "scripts\v65_analyze_enqueue_log.py"
& $python.Source $analyzer $out $reportOut
if ($LASTEXITCODE -ne 0) { throw "analyzer failed" }

$pattern = "JJFB_V65_101AB|JJFB_V65_ENQ|JJFB_V64_ENQ|JJFB_V56_EVENT|caller_2E4066|gate_init_2DADC4|uimode_writer|TARGETS_2DADC4|FORCE state|unhandled plat/msg 0x101AB"
