# JJFB v64 - call 0x10165 enqueue handler once (30D2F8→B54→2E2520 Path A)
# Ref: v63 mrc_event(5) is no-op; real producer is 0x10165→30D2F9→2E4D6C.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v64_enqueue_stdout.txt"
$errOut = Join-Path $logDir "v64_enqueue_stderr.txt"
...
$reportOut = Join-Path $reportDir "v64_enqueue_run_result.md"
...
Write-Host "=== JJFB v64 Enqueue (10165 once PROBE) ==="
Write-Host "project=$root"
Write-Host "target: call 10165 handler once; B54 non-empty → 2E2520 → 2DADC4"
...
if (-not $bridgeText.Contains("[JJFB_V64_ENQ]") -or
  ...
  throw "v64 overlay missing (need V64_ENQ + V62_FLAG + V61_NEST + FAMILY_APP2)."
}
...
Write-Host "[4/6] Build v64 enqueue"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v64 run: before=$runtimeHash after=$afterHash"
}
...
$analyzer = Join-Path $root "scripts\v64_analyze_enqueue_log.py"
& $python.Source $analyzer $out $reportOut
if ($LASTEXITCODE -ne 0) { throw "analyzer failed" }

$pattern = "JJFB_V64_ENQ|JJFB_V56_EVENT|caller_2E4066|gate_init_2DADC4|uimode_writer|TARGETS_2DADC4|FORCE state"
