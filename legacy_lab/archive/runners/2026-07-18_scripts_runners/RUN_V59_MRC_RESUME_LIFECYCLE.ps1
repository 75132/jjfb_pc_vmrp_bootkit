# JJFB v59 - platform mrc_resume (ext code=5) after init (NO FORCE ui_mode / NO C0 inject)
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v59_mrc_resume_lifecycle_stdout.txt"
$errOut = Join-Path $logDir "v59_mrc_resume_lifecycle_stderr.txt"
...
$reportOut = Join-Path $reportDir "v59_mrc_resume_lifecycle_run_result.md"
...
Write-Host "=== JJFB v59 mrc_resume lifecycle (code=5 after init) ==="
Write-Host "project=$root"
Write-Host "target: resume -> 304B5A -> 2F5390 register (observe + lifecycle)"
...
if (-not $vmrpText.Contains("[JJFB_V59_LIFECYCLE] mrc_resume") -or
    -not $bridgeText.Contains("[JJFB_V58_FN] coverage installed") -or
  ...
  throw "v59 source overlay missing (need V59_LIFECYCLE resume + V58_FN hooks)."
}
...
Write-Host "[4/6] Build v59 mrc_resume lifecycle"
...
Write-Host "[5/6] Run with mrc_resume after init (no FORCE / no C0 inject)"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v59 run: before=$runtimeHash after=$afterHash"
}
...
Write-Host "[6/6] Analyze resume + fn-entry coverage"
$analyzer = Join-Path $root "scripts\v59_analyze_mrc_resume_log.py"
if (-not (Test-Path $analyzer)) { $analyzer = Join-Path $root "scripts\v58_analyze_fn_entry_log.py" }
if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
...
$pattern = "JJFB_V59_LIFECYCLE|JJFB_V58_FN|JJFB_V57_SRC|JJFB_V56_CALLBACK|JJFB_V56_FAMILY|uimode_writer|natural_mode|FORCE state|host mrc_init"
