# JJFB v60 - family app=2 (timerCreate) then mrc_resume (NO FORCE / NO C0 inject)
# Ref: JJFB_v57_LIFECYCLE_SOURCE_COVERAGE_COMPLETE — method5 needs timer at ERW+0x8C4 first.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v60_family_app2_timer_stdout.txt"
$errOut = Join-Path $logDir "v60_family_app2_timer_stderr.txt"
...
$reportOut = Join-Path $reportDir "v60_family_app2_timer_run_result.md"
...
Write-Host "=== JJFB v60 family app=2 timerCreate + mrc_resume ==="
Write-Host "project=$root"
Write-Host "target: app2->30CBBC->timerCreate then resume->2F5390 (no timer err:1000)"
...
if (-not $vmrpText.Contains("[JJFB_V60_LIFECYCLE] family app") -and
    -not $vmrpText.Contains("JJFB_FAMILY_APP2_AFTER_INIT")) {
  throw "v60 overlay missing (need FAMILY_APP2_AFTER_INIT in vmrp.c)."
}
if (-not $bridgeText.Contains("jjfb_lifecycle_family_app") -or
    -not $bridgeText.Contains("[JJFB_V60_TIMER]") -or
    -not $vmrpText.Contains("[JJFB_V59_LIFECYCLE] mrc_resume")) {
  throw "v60 source overlay missing (lifecycle family app2 + timer hooks + resume)."
}
...
if (Test-Path $auditReport) { Get-Content $auditReport -Encoding UTF8 | Write-Host }
...
Write-Host "[4/6] Build v60 family app2 + resume"
...
Write-Host "[5/6] Run (app2 then resume; no FORCE / no C0)"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v60 run: before=$runtimeHash after=$afterHash"
}
...
$analyzer = Join-Path $root "scripts\v60_analyze_family_app2_timer_log.py"
& $python.Source $analyzer $out --markdown $reportOut

$pattern = "JJFB_V60_|JJFB_V59_LIFECYCLE|timer err|ERW8C4|CALLBACK|entry_2F5404|uimode_writer|gate_init|TARGETS_2FEBBC|natural_mode|FORCE state"
