# JJFB v56 - trace natural upstream triggers of 0x2DADC4 (NO FORCE)
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v56_upstream_trigger_stdout.txt"
$errOut = Join-Path $logDir "v56_upstream_trigger_stderr.txt"
...
$reportOut = Join-Path $reportDir "v56_upstream_trigger_run_result.md"
...
Write-Host "=== JJFB v56 Upstream Trigger Coverage (natural, no FORCE) ==="
Write-Host "project=$root"
Write-Host "target: event queue / family app=0xC0 / callback scheduler upstream of 2DADC4"
...
if (-not $bridgeText.Contains("jjfb_install_uimode_writer_hooks") -or
    -not $bridgeText.Contains("[JJFB_V56_UPSTREAM] coverage installed") -or
    -not $bridgeText.Contains("[JJFB_GAME_SELF] natural_mode=1") -or
    -not $vmrpText.Contains("JJFB_ACCEPT_START_IGNORE_AFTER_ROBOTOL")) {
  throw "v56 source overlay missing (need v56 upstream hooks + v54 natural_mode)."
}
...
Write-Host "[4/6] Build v56 upstream trigger coverage"
...
Write-Host "[5/6] Run natural (no FORCE/no synthetic event) with upstream coverage"
...
Write-Host "[6/6] Analyze upstream trigger coverage"
$analyzer = Join-Path $root "scripts\v56_analyze_upstream_log.py"
if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
...
$pattern = "JJFB_GAME_SELF|JJFB_START_HANDOFF|JJFB_801|robotol timer|FORCE|upstream_trigger|gate_|caller_|FILEOPEN_MISS"
