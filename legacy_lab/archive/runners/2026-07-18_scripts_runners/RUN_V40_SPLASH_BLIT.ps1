# JJFB v40 — splash draw @0x2EC6B0 + guest-coord host blit
param(
  [string]$Mode = "45",
  [int]$Seconds = 18
)
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw "compile bridge.c failed" }
...
Select-String -Path $out -Pattern "2EC6B0_BLIT|2EC6B0_GUARD|drawFP@150C|SPLASH_ENTER|DEBUG_PRESENT from 2EC" |
