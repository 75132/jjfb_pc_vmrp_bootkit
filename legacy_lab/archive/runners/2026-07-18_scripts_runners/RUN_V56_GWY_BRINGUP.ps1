# JJFB v56 - GWY bring-up: fix white screen (guest writer + DispUpEx + C44)
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v56_gwy_bringup_stdout.txt"
$errOut = Join-Path $logDir "v56_gwy_bringup_stderr.txt"
...
$reportOut = Join-Path $reportDir "v56_gwy_bringup_run_result.md"
...
Write-Host "=== JJFB v56 GWY Bring-up (white-screen fix) ==="
Write-Host "project=$root"
Write-Host "bringup: dims + C44 + guest 2FC03C then 2FC418 + DispUpEx present"
...
if (-not $bridgeText.Contains("jjfb_gwy_bringup_first_screen") -or
    -not $bridgeText.Contains("[JJFB_GWY_BRINGUP]") -or
    -not $bridgeText.Contains("call 0x2FC418 natural writer") -or
    -not $bridgeText.Contains("DispUpEx #%u") -or
    -not $bridgeText.Contains("presented dark baseline")) {
  throw "v56 bring-up / DispUpEx overlay missing in bridge.c"
}
...
$analyzer = Join-Path $root "scripts\v56_analyze_gwy_bringup_log.py"
if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
...
Select-String -Path $out -Pattern "JJFB_GWY_BRINGUP|JJFB_GAME_SELF|uimode_writer|FORCE state|host mrc_init|robotol timer|DispUpEx|SPLASH_ENTER|C44=" |
