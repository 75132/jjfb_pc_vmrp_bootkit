# JJFB v54 - post-handoff natural game-self (no FORCE ui_mode=0x45)
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v54_natural_game_self_stdout.txt"
$errOut = Join-Path $logDir "v54_natural_game_self_stderr.txt"
$keyManifest = Join-Path $logDir "v54_sdk_key_manifest.json"
$auditJson = Join-Path $logDir "v54_start_handoff_audit.json"
$auditReport = Join-Path $reportDir "v54_start_handoff_static_audit.md"
$reportOut = Join-Path $reportDir "v54_natural_game_self_run_result.md"
...
Write-Host "=== JJFB v54 Natural Game-Self (post start-handoff) ==="
Write-Host "project=$root"
Write-Host "keep: br_log alias + gated MR_IGNORE host 801"
Write-Host "change: GWY mode defaults NO FORCE ui_mode=0x45"

$bridgeSource = Join-Path $src "bridge.c"
$vmrpSource = Join-Path $src "vmrp.c"
foreach ($required in @($bridgeSource, $vmrpSource)) {
  if (-not (Test-Path $required)) { throw "v54 source missing: $required" }
}
...
if (-not $bridgeText.Contains("jjfb_v53_probe_guest_log") -or
  ...
  throw "v54 source overlay is not present (need v53 handoff + GAME_SELF natural_mode)."
}
...
if (-not $python) { throw "Python 3 is required for v54 preflight and analysis" }
...
if ($LASTEXITCODE -ne 0) { throw "v54 static audit failed" }
...
# Explicit off (belt + suspenders with GWY default in bridge.c)
...
Write-Host "[4/6] Build v54 GWY natural-mode + v53 handoff"
...
if (-not $SkipBuild) {
  ...
  if (-not $exeAscii.Contains("method=ext_base_0xD4") -or
      ...
      -not $exeAscii.Contains("[JJFB_GAME_SELF] natural_mode=1")) {
    throw "-SkipBuild main.exe does not contain all v54 markers"
  }
  Write-Host "build skipped; existing main.exe contains v54 markers"
}

Write-Host "[5/6] Run original gwy/jjfb.mrp with natural ui_mode (no FORCE 0x45)"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v54 run: before=$runtimeHash after=$afterHash"
}
...
Write-Host "[6/6] Analyze handoff + natural game-self progression"
$analyzer = Join-Path $root "scripts\v54_analyze_post_handoff.py"
if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
  & $python.Source -3 $analyzer $out --markdown $reportOut
} else {
  & $python.Source $analyzer $out --markdown $reportOut
}
if ($LASTEXITCODE -ne 0) { Write-Warning "v54 log analyzer failed" }

$pattern = "JJFB_GAME_SELF|JJFB_FIRST_SCREEN|JJFB_MRP_ALIAS|JJFB_ROBOTOL_LOAD|JJFB_START_HANDOFF|JJFB_801_GUARD|mr_get_method|bridge_dsm_mr_start_dsm|JJFB_801|robotol timer|FORCE|FILEOPEN_MISS|JJFB_NET|JJFB_STRCOM|mr_exit"
