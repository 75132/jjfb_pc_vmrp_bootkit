# JJFB v55 - locate natural ui_mode=0x45 writer chain (NO FORCE)
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v55_uimode_writer_stdout.txt"
$errOut = Join-Path $logDir "v55_uimode_writer_stderr.txt"
$keyManifest = Join-Path $logDir "v55_sdk_key_manifest.json"
$auditJson = Join-Path $logDir "v55_start_handoff_audit.json"
$auditReport = Join-Path $reportDir "v55_start_handoff_static_audit.md"
$reportOut = Join-Path $reportDir "v55_uimode_writer_run_result.md"
...
Write-Host "=== JJFB v55 ui_mode Writer Coverage (natural, no FORCE) ==="
Write-Host "project=$root"
Write-Host "target: 2FC418 writer + 2DADC4/B70 gates + callers"

$bridgeSource = Join-Path $src "bridge.c"
$vmrpSource = Join-Path $src "vmrp.c"
foreach ($required in @($bridgeSource, $vmrpSource)) {
  if (-not (Test-Path $required)) { throw "v55 source missing: $required" }
}
...
if (-not $bridgeText.Contains("jjfb_install_uimode_writer_hooks") -or
    -not $bridgeText.Contains("uimode_writer ENTER") -or
    -not $bridgeText.Contains("[JJFB_GAME_SELF] natural_mode=1") -or
    -not $vmrpText.Contains("JJFB_ACCEPT_START_IGNORE_AFTER_ROBOTOL")) {
  throw "v55 source overlay missing (need uimode writer hooks + v54 natural_mode)."
}
...
if (-not $python) { throw "Python 3 is required for v55 preflight and analysis" }
...
if ($LASTEXITCODE -ne 0) { throw "v55 static audit failed" }

$staticMap = Join-Path $root "scripts\v55_map_uimode_writers.py"
if (Test-Path $staticMap) {
  $mapOut = Join-Path $reportDir "v55_uimode_writer_static_map.md"
  if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
...
Write-Host "[4/6] Build v55 uimode writer coverage"
...
Write-Host "[5/6] Run natural (no FORCE) with writer coverage"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v55 run: before=$runtimeHash after=$afterHash"
}
...
Write-Host "[6/6] Analyze writer coverage"
$analyzer = Join-Path $root "scripts\v55_analyze_uimode_writer_log.py"
if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
...
$pattern = "JJFB_GAME_SELF|JJFB_START_HANDOFF|JJFB_801|robotol timer|FORCE|uimode_writer|gate_|caller_|FILEOPEN_MISS"
