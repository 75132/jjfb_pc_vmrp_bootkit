# JJFB v57 - observe producers of family 0xC0 / callback 2F5405 (NO INJECT/FORCE)
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v57_c0_callback_source_stdout.txt"
$errOut = Join-Path $logDir "v57_c0_callback_source_stderr.txt"
...
$reportOut = Join-Path $reportDir "v57_c0_callback_source_run_result.md"
...
Write-Host "=== JJFB v57 C0/Callback Source Coverage (natural, no FORCE) ==="
Write-Host "project=$root"
Write-Host "target: who calls 2F5390/BL 3054A4 / CMP|MOVS 0xC0 (observe only)"
...
if (-not $bridgeText.Contains("[JJFB_V57_SRC] coverage installed") -or
    -not $bridgeText.Contains("BL_3054A4") -or
    -not $bridgeText.Contains("call_2F5390_prep") -or
    -not $bridgeText.Contains("[JJFB_GAME_SELF] natural_mode=1") -or
    -not $vmrpText.Contains("JJFB_ACCEPT_START_IGNORE_AFTER_ROBOTOL")) {
  throw "v57 source overlay missing (need V57_SRC hooks + natural_mode)."
}
...
Write-Host "[4/6] Build v57 C0/callback source coverage"
...
Write-Host "[6/6] Analyze C0/callback source coverage"
$analyzer = Join-Path $root "scripts\v57_analyze_c0_callback_source_log.py"
if ($python.Name -eq "py.exe" -or $python.Name -eq "py") {
...
$pattern = "JJFB_V57_SRC|JJFB_V56_FAMILY|JJFB_V56_CALLBACK|natural_mode|FORCE state|gate_init|host mrc_init"
