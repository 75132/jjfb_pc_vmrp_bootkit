# JJFB v61 - continue after v60: defer nested 1E209 during ext_call (timer)
# Ref: JJFB_v57_LIFECYCLE_SOURCE_COVERAGE_COMPLETE + v60 entry_2F5404 hung on app=7 nest.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v61_callback_nest_fix_stdout.txt"
$errOut = Join-Path $logDir "v61_callback_nest_fix_stderr.txt"
...
$reportOut = Join-Path $reportDir "v61_callback_nest_fix_run_result.md"
...
Write-Host "=== JJFB v61 nest-fix (defer 1E209 during ext_call) ==="
Write-Host "project=$root"
Write-Host "target: 2F5404 continues; multiple code=2; reach 2F5734/305EB8 if possible"
...
if (-not $bridgeText.Contains("[JJFB_V61_NEST]") -or
    -not $bridgeText.Contains("jjfb_lifecycle_family_app") -or
    -not $vmrpText.Contains("JJFB_FAMILY_APP2_AFTER_INIT")) {
  throw "v61 overlay missing (need V61_NEST in bridge.c + FAMILY_APP2 in vmrp.c)."
}
...
Write-Host "[4/6] Build v61 nest-fix"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v61 run: before=$runtimeHash after=$afterHash"
}
...
$analyzer = Join-Path $root "scripts\v61_analyze_callback_nest_fix_log.py"
& $python.Source $analyzer $out --markdown $reportOut

$pattern = "JJFB_V61_NEST|JJFB_V60_|entry_2F5404|tail_call_305EB8|entry_305EB8|gate_init|uimode_writer|TARGETS_2FEBBC|timer err|FORCE state|ext_call code=2"
