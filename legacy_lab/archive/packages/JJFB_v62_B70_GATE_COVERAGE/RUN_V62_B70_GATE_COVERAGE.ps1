# JJFB v62 - observe 305EB8 gates (15D/B71/B70); find who writes B71
# Ref: v61 nest-fix got continuous 305EB8 with B70=0; static shows real gates are 15D/B71.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v62_b70_gate_stdout.txt"
$errOut = Join-Path $logDir "v62_b70_gate_stderr.txt"
...
$reportOut = Join-Path $reportDir "v62_b70_gate_run_result.md"
...
Write-Host "=== JJFB v62 b70-gate (15D/B71/305EB8) ==="
Write-Host "project=$root"
Write-Host "target: confirm 15D/B71 gates; who writes B71; reach 305EF4 if possible"
...
if (-not $bridgeText.Contains("[JJFB_V62_FLAG]") -or
  ...
  throw "v62 overlay missing (need V62_FLAG + V61_NEST in bridge.c + FAMILY_APP2 in vmrp.c)."
}
...
Write-Host "[4/6] Build v62 b70-gate"
...
$header = @(
  ...
  "[JJFB_V62_FLAG] contract=observe_15D_B71_B70_305EB8_gates no_FORCE no_C0_inject",
...
$analyzer = Join-Path $root "scripts\v62_analyze_b70_gate_log.py"
& $python.Source $analyzer $out $reportOut
if ($LASTEXITCODE -ne 0) { throw "analyzer failed" }

$pattern = "JJFB_V62_FLAG|fail_15D|fail_B71|ok_to_2DADC4|entry_305EB8|gate_init|uimode_writer|FORCE state|after family app=2 15D"
