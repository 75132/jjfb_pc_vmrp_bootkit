...
临时探针（**非正式方案**，仅用于诊断联网/splash 卡点）：

```text
JJFB_SKIP_NET_LOGIN=1      # 默认开：splash ~2s 后种子 AC8/B6C/134D 成功门并尝试进 2EFC0A
JJFB_SKIP_NET_LOGIN_MS=2000
```

关闭探针：启动前 `$env:JJFB_SKIP_NET_LOGIN="0"` 或删掉该环境变量。
