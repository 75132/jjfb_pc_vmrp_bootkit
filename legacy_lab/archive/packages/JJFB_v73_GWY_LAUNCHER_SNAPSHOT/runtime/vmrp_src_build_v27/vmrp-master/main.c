...
void guiDrawBitmap(uint16_t *bmp, int32_t x, int32_t y, int32_t w, int32_t h) {
    ...
    int win_w = surface ? surface->w : WINDOW_WIDTH;
    int win_h = surface ? surface->h : WINDOW_HEIGHT;
    (void)x;
    (void)y;
    (void)w;
    (void)h;
    if (!bmp || !surface) return;
    if (SDL_MUSTLOCK(surface)) {
        if (SDL_LockSurface(surface) != 0) printf("SDL_LockSurface err\n");
    }
    /* v84: guest LCD is SCREEN_WxH; SDL window may be WINDOW_WxH.
     * Stretch full guest frame to fill the window (platform present, not fake UI). */
    if (win_w == SCREEN_WIDTH && win_h == SCREEN_HEIGHT) {
        for (int32_t j = 0; j < SCREEN_HEIGHT; j++) {
            for (int32_t i = 0; i < SCREEN_WIDTH; i++) {
                uint16_t color = *(bmp + (i + j * SCREEN_WIDTH));
                Uint32 *p = (Uint32 *)(((Uint8 *)surface->pixels) + surface->pitch * j) + i;
                *p = SDL_MapRGB(surface->format, PIXEL565R(color), PIXEL565G(color), PIXEL565B(color));
            }
        }
    } else {
        for (int32_t wy = 0; wy < win_h; wy++) {
            int32_t gy = (wy * SCREEN_HEIGHT) / win_h;
            if (gy < 0) gy = 0;
            if (gy >= SCREEN_HEIGHT) gy = SCREEN_HEIGHT - 1;
            Uint32 *row = (Uint32 *)(((Uint8 *)surface->pixels) + surface->pitch * wy);
            for (int32_t wx = 0; wx < win_w; wx++) {
                int32_t gx = (wx * SCREEN_WIDTH) / win_w;
                if (gx < 0) gx = 0;
                if (gx >= SCREEN_WIDTH) gx = SCREEN_WIDTH - 1;
                uint16_t color = *(bmp + (gx + gy * SCREEN_WIDTH));
                row[wx] = SDL_MapRGB(surface->format, PIXEL565R(color), PIXEL565G(color),
                                     PIXEL565B(color));
            }
        }
    }
...
uint32_t timerCb(uint32_t interval, void *param) {
    (void)param;
    timer();
    /* Robotol path: keep ticking so EXT code=2 (mrc_timerTimeout) runs continuously.
     * DSM/Lua path still expects one-shot + guest mr_timerStart re-arm. */
    if (jjfb_robotol_timer_active()) {
        uint32_t ms = interval ? interval : 50;
        return ms;
    }
    timeId = 0;
    return 0;
...
#endif
        {
            ...
            switch (ev.type) {
                ...
                case SDL_MOUSEMOTION:
                    if (isMouseDown) {
                        int32_t gx = ev.motion.x * SCREEN_WIDTH / WINDOW_WIDTH;
                        int32_t gy = ev.motion.y * SCREEN_HEIGHT / WINDOW_HEIGHT;
                ...
                case SDL_MOUSEBUTTONDOWN:
                    isMouseDown = true;
                    event(MR_MOUSE_DOWN,
                          ev.motion.x * SCREEN_WIDTH / WINDOW_WIDTH,
                          ev.motion.y * SCREEN_HEIGHT / WINDOW_HEIGHT);
                    break;
...
int main(int argc, char *args[]) {
    ...
    freopen("jjfb_loader_stdout.txt", "w", stdout);
    freopen("jjfb_loader_stderr.txt", "w", stderr);
    setvbuf(stdout, NULL, _IONBF, 0);
    setvbuf(stderr, NULL, _IONBF, 0);
    printf("[JJFB_LOADER] main start\n");
#ifdef __x86_64__
    printf("__x86_64__\n");
#elif __i386__
    printf("__i386__\n");
    ...
    window = SDL_CreateWindow("vmrp", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                              WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_OPENGL);
    if (window == NULL) {
        printf("Window could not be created! SDL_Error: %s\n", SDL_GetError());
        return -1;
    }
    printf("[JJFB_V84_SCREEN] guest=%dx%d window=%dx%d resource via JJFB_MYTHROAD_ROOT "
           "(stretch present; no layout force)\n",
