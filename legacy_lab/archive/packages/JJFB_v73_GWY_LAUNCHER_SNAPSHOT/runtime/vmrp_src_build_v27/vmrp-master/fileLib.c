...
int32_t my_open(const char *filename, uint32_t mode) {
    ...
    /* Mythroad often opens download/cache paths like "0@s0.map" with RDWR
     * alone (mode=4). Without O_CREAT the host open fails and the game never
     * gets a writable cache fd. Auto-create when @ is present and writeable. */
    if (filename && strchr(filename, '@') &&
        (mode & (MR_FILE_RDWR | MR_FILE_WRONLY | MR_FILE_CREATE)) &&
        !(mode & MR_FILE_RDONLY)) {
        new_mode |= O_CREAT;
