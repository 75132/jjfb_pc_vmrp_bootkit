...
/* Host RGB565 framebuffer for DrawRect / splash host-blit / DispUpEx present. */
...
/* mr_table._DispUpEx — refresh dirty rect from screen buffer to SDL.
 * Previously a no-op, which left the window permanently white even when the
 * guest "refreshed". Present jjfb_screen565 (host RGB565 used by DrawRect /
 * splash host-blit). If mr_table.mr_screenBuf is a valid guest buffer, prefer it. */
static void br__DispUpEx(BridgeMap *o, uc_engine *uc) {
    uint32_t x, y, w, h;
    uint16_t *buf = jjfb_screen565;
    static uint32_t n;
    (void)o;
    ...
    n++;
    if (mr_table) {
        uint32_t slot = toMrpMemAddr(mr_table) + 0x16C;
        uint32_t guest_buf = 0;
        if (uc_mem_read(uc, slot, &guest_buf, 4) == UC_ERR_OK &&
            guest_buf && getMrpMemPtr(guest_buf))
            buf = (uint16_t *)getMrpMemPtr(guest_buf);
    }
    ...
    if (n <= 8 || (n % 60) == 0) {
        printf("[JJFB_DRAW] DispUpEx #%u %u,%u %ux%u buf=%s\n",
               n, x, y, w, h, (buf == jjfb_screen565) ? "host565" : "guest");
        fflush(stdout);
    }
    guiDrawBitmap(buf, (int32_t)x, (int32_t)y, (int32_t)w, (int32_t)h);
    SET_RET_V(MR_SUCCESS);
}
...
static void br_DrawRect(BridgeMap *o, uc_engine *uc) {
    ...
    iw = (int16_t)(w & 0xFFFF);
    ih = (int16_t)(h & 0xFFFF);
    /* docx: SCRW=240. Some chrome right-aligns with stored width 320
     * (ERW+0x830 natural H used as W) → x≈270. Remap 320-layout into 240 canvas. */
    if (SCREEN_WIDTH == 240 && ix >= SCREEN_WIDTH && ix < 320) {
        int32_t oldx = ix;
        ix -= (320 - SCREEN_WIDTH);
        n_axis_remap++;
        if (n_axis_remap <= 8 || (n_axis_remap % 60) == 0) {
            printf("[JJFB_DRAW] axis_remap x %d -> %d (320-layout -> SCRW=240)\n",
                   oldx, ix);
            fflush(stdout);
        }
    }
...
static int jjfb_310bb4_host_blit = 1; /* default on for splash bring-up */
...
static void jjfb_install_ptr_store_watch(uc_engine *uc) {
    ...
    err = uc_hook_add(uc, &jjfb_ptr_store_hook, UC_HOOK_MEM_WRITE,
                      jjfb_hook_ptr_store, NULL, 0x600000, 0x7F0000);
    if (err) printf("[JJFB_PTR_STORE] heap watch fail err=%u\n", err);
    else printf("[JJFB_PTR_STORE] watching heap stores @0x600000..0x7F0000\n");
...
static void jjfb_hook_erw_band_write(uc_engine *uc, uc_mem_type type,
    ...
    if (off == JJFB_ERW_STATE_EARLY || off == JJFB_ERW_SPLASH_CNT_AC8 ||
        off == JJFB_ERW_PROGRESS_BA0 + 0x2C || off == 0xB6C ||
        off == 0x134D || off == 0x1350 || off == 0xA98 ||
        n <= 80 || (n % 50) == 0) {
...
/* v57: 0x10134 often arrives as app=W*H*2 without a prior 2d92dc name.
 * Map locked splash sizes (docx SCRW=240 + v37 evidence) so we load real assets. */
static int jjfb_bmp_req_from_bytes(uint32_t bytes, uint32_t lr) {
    static const struct {
        uint32_t bytes;
        const char *name;
    } known[] = {
        { 0x2D8A, "loadingbar!201!29.bmp" }, /* 201*29*2 */
        { 0x0240, "bar!16!18.bmp" },         /* 16*18*2 */
        { 0x1C20, "textbar!120!30.bmp" },    /* 120*30*2 */
        { 0x4714, "slogo!157!58.bmp" },      /* 157*58*2 */
    };
    unsigned i;
    for (i = 0; i < sizeof(known) / sizeof(known[0]); i++) {
        if (known[i].bytes == bytes) {
            printf("[JJFB_10134] size-map bytes=0x%X -> %s\n", bytes, known[i].name);
            fflush(stdout);
            jjfb_bmp_req_set(known[i].name, 0, lr);
            return 1;
        }
    }
    return 0;
...
/* Load RGB565 pixels for name!W!H.bmp from jjfb.mrp / dsm_gm.mrp (gzip members). */
static int jjfb_mrp_load_rgb565(const char *full_name, int expect_bytes,
    ...
    /* Fast path: pre-extracted cache next to main.exe */
    snprintf(cache_path, sizeof(cache_path), "jjfb_bmp_cache/%s.rgb565", full_name);
    ...
    for (pi = 0; packs[pi]; pi++) {
        if (strcmp(packs[pi], "jjfb_bmp_cache") == 0)
            continue;
        fp = fopen(packs[pi], "rb");
        if (!fp) continue;
            ...
            if (memcmp(p, full_name, nlen) == 0 && p[nlen] == 0) {
                ...
                if (off + len <= (uint32_t)fsz && len > 0 && len < 2 * 1024 * 1024) {
                    ...
                    if (uncompress((Bytef *)out, &destLen, (const Bytef *)(file + off), len) == Z_OK &&
                        ...
                        /* gzip wrapper: inflateInit2 windowBits=15+16 */
                        z_stream strm;
                        int zret;
...
static uint32_t jjfb_plat_handler_1e200; /* from 0x10102 code=0x30D2F9 */
 ...
 * Splash: x=(2f9968()-bmp.w)/2 ??2f9968 must return WIDTH on 240 canvas. */
#define JJFB_ERW_SCR_W824 0x824
#define JJFB_ERW_SCR_Y828 0x828
#define JJFB_ERW_SCR_H830 0x830
#define JJFB_ERW_SCR_W834 0x834

/* Host-side RGB565 framebuffer declared near br_mr_drawBitmap (jjfb_screen565). */
...
static void jjfb_screen_blit_rgb565_key(const uint16_t *src, int src_w, int src_h,
    ...
    if (x0 >= x1 || y0 >= y1) return;
    for (y = y0; y < y1; y++) {
        const uint16_t *srow = src + (y - dst_y) * src_w + (x0 - dst_x);
 ...
 * code object (observed):
 *   +0x14 = payload length
 *   +0x18 = payload (often base64-like ASCII)
 * Stage: ASCII font + block for non-ASCII; DEBUG present only.
...
static void jjfb_plat_11f00(uc_engine *uc, uint32_t app, uint32_t code,
    ...
    uint16_t bg = jjfb_rgb565(0, 0, 80);
    uint32_t nchars = 0, len_field = 0, p0_n = 0;
    uint8_t payload[128];
    uint8_t decoded[96];
    uint32_t ascii_n = 0, block_n = 0, dec_n = 0;
    const uint8_t *draw_bytes;
    uint32_t draw_n;
    int32_t cx, cy, drawn_w;
    static uint32_t n11;
    static int cleared_bg;
    uint32_t i;
    n11++;
    memset(payload, 0, sizeof(payload));
    memset(decoded, 0, sizeof(decoded));

    /* Once: dark fill so window is not pure white (from game buffer path). */
    if (!cleared_bg) {
        jjfb_screen_fill_rect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, jjfb_rgb565(8, 12, 40));
        cleared_bg = 1;
        printf("[JJFB_FIRST_SCREEN] cleared guest RGB565 to dark (no host overlay)\n");
        fflush(stdout);
    }
    ...
    if (code && getMrpMemPtr(code)) {
        ...
        /* Follow likely string/object pointers inside code blob. */
    ...
    /* Prefer decoded base64 when it looks more like text; else draw payload ASCII. */
    draw_bytes = payload;
    draw_n = nchars;
    ...
    if (dec_n > 0) {
        ...
        /* If decode is binary (glyph tokens), keep drawing raw ASCII markers for now. */
    }
    ...
    for (i = 0; i < draw_n; i++) {
        ...
        if (ch >= 0x20 && ch <= 0x7E) {
            jjfb_screen_draw_ascii(cx, cy, ch, fg, bg);
            ascii_n++;
        } else if (ch >= 0x81 && i + 1 < draw_n) {
            /* GBK lead byte: 16x16 placeholder until real font wired. */
            jjfb_screen_draw_glyph_block(cx, cy, fg, bg);
            jjfb_screen_draw_glyph_block(cx + 8, cy, fg, bg);
            block_n += 2;
            cx += 8; /* extra advance; loop also adds 8 */
            i++;
        } else {
            jjfb_screen_draw_glyph_block(cx, cy, fg, bg);
            block_n++;
    ...
    if (n11 <= 12 || (n11 % 40) == 0) {
        printf("[JJFB_DRAW_TEXT] 11F00 #%u app=0x%X code=0x%X xy=%d,%d glyphs=%u "
               "ascii=%u block=%u %dx16 p1=0x%X p2=0x%X p3=0x%X dirty=%u\n",
               n11, app, code, (int)x, (int)y, draw_n, ascii_n, block_n,
               drawn_w, param1, param2, param3, jjfb_screen_dirty);
        if (ascii_n)
    ...
    /* DEBUG present only ??not pretending to be mrc_refreshScreen. */
    if (jjfb_screen_dirty) {
        guiDrawBitmap(jjfb_screen565,
                       (int32_t)jjfb_screen_dirty_x, (int32_t)jjfb_screen_dirty_y,
                       (int32_t)jjfb_screen_dirty_w, (int32_t)jjfb_screen_dirty_h);
        if (n11 <= 8 || (n11 % 40) == 0) {
            printf("[JJFB_DEBUG_PRESENT] from 11F00 dirty %u,%u %ux%u "
                   "(host debug; DispUpEx/mrc_refreshScreen untouched)\n",
                   jjfb_screen_dirty_x, jjfb_screen_dirty_y,
                   jjfb_screen_dirty_w, jjfb_screen_dirty_h);
            fflush(stdout);
...
static uc_hook jjfb_uimode_writer_hooks[12];
...
static void jjfb_hook_splash_cov(uc_engine *uc, uint64_t address, uint32_t size, void *user_data) {
    ...
    /* At LDR *[ERW+0xB6C]: log the gate pointer. */
    if (pc == 0x2EFAEA || pc == 0x2EFAEE) {
    ...
    if (force && force[0] && force[0] != '0' && !jjfb_force_b6c_done &&
        (pc == 0x2EFAEC || pc == 0x2EFAEE)) {
        uint32_t nr0 = 1;
        jjfb_force_b6c_done = 1;
        uc_reg_read(uc, UC_ARM_REG_R0, &r0);
        if (force[0] == '1' && !force[1])
            nr0 = 1;
        else
            nr0 = (uint32_t)strtoul(force, NULL, 0);
        if (nr0 == 0) nr0 = 1;
        uc_reg_write(uc, UC_ARM_REG_R0, &nr0);
        printf("[JJFB_FORCE_B6C] probe pc=0x%X old_r0=0x%X -> r0=0x%X (BNE -> 0x2EFB06)\n",
               pc, r0, nr0);
        fflush(stdout);
    }
    ...
    if (force && force[0] && force[0] != '0' && !jjfb_force_134d_done) {
        uint32_t nr0 = 1;
        if (!(force[0] == '1' && !force[1]))
            nr0 = (uint32_t)strtoul(force, NULL, 0);
        if (nr0 == 0) nr0 = 1;
        if (pc == 0x2EFBA4 && jjfb_guest_ext_erw) {
            ...
            printf("[JJFB_FORCE_134D] seed *(ERW+0x134D)=0x%X before LDR @0x2EFBA4\n", nr0);
            fflush(stdout);
        } else if (pc == 0x2EFBA6) {
            jjfb_force_134d_done = 1;
            uc_reg_read(uc, UC_ARM_REG_R0, &r0);
            uc_reg_write(uc, UC_ARM_REG_R0, &nr0);
            printf("[JJFB_FORCE_134D] probe pc=0x%X old_r0=0x%X -> r0=0x%X (CMP)\n",
                   pc, r0, nr0);
            fflush(stdout);
        }
    }
    ...
    if (force && force[0] && force[0] != '0') {
        ...
        if (!s_ac8gate_done && pc == 0x2EFBFA && jjfb_guest_ext_erw) {
            ...
            s_ac8gate_done = 1;
            printf("[JJFB_FORCE_AC8_GATE] seed *(ERW+AC8)=0x%X before LDR @0x2EFBFA\n", nr1);
            fflush(stdout);
        } else if (!s_ac8gate_done && pc == 0x2EFBFC) {
            s_ac8gate_done = 1;
            uc_reg_read(uc, UC_ARM_REG_R1, &r1);
            uc_reg_write(uc, UC_ARM_REG_R1, &nr1);
            printf("[JJFB_FORCE_AC8_GATE] probe pc=0x%X old_r1=0x%X -> r1=0x%X (CMP)\n",
                   pc, r1, nr1);
            fflush(stdout);
        }
    }
    ...
    if (force && force[0] == '1' && !jjfb_2efc_tail_forced &&
        jjfb_2efc40_hits == 0 && pc >= 0x2EFAE8 && pc <= 0x2EFB10) {
        /* Probe: after progress loop epilogue, divert into literal-pool neighborhood
         * code at 0x2EFC40 (guest executes; do not host-write progress/AC8). */
...
static void jjfb_install_uimode_writer_hooks(uc_engine *uc) {
    static const uint32_t addrs[] = {
        0x2FC418, /* writer entry: ends with store ui_mode=0x45 */
        0x2FC448, /* STR r0,[r5,#0] ui_mode store */
        0x2FC03C, /* init helper that BL writer */
        0x2DADC4, /* gated init */
        0x2DAE00, /* B70 gate check */
        0x2DAE24, /* BL 2FC03C after B70!=0 */
        0x30EE50, /* alternate writer caller */
        0x2FECA2, /* BL 2DADC4 */
        0x2E4066, /* BL 2DADC4 */
        0x305EF4  /* BL 2DADC4 (near 134D gate) */
    };
    uint32_t i;
    uc_err err;
    ...
    printf("[JJFB_GAME_SELF] uimode_writer coverage installed "
           "(2FC418/2FC03C/2DADC4/B70/callers)\n");
    fflush(stdout);
}
...
static void jjfb_hook_ui_code(uc_engine *uc, uint64_t address, uint32_t size, void *user_data) {
    ...
    if ((uint32_t)address == 0x2E87AC) {
    ...
    } else if ((uint32_t)address == 0x2F4494) {
        ...
        /* Side-channel ctx dump while chrome stays nop'd. */
        ...
        if (n4494 <= 12 || (n4494 % 40) == 0)
            printf("[JJFB_UI] enter 0x2f4494 #%u lr=0x%X y=%d w=%d mode=%u flagA64=0x%X "
                   "(nop chrome; blit loop blocks present)\n",
        ...
        if (!jjfb_allow_chrome || jjfb_chrome_allow_calls == 0) {
            /*
             * Default: bypass chrome so 305bf4->0x11F00 continues.
             * Use env JJFB_ALLOW_CHROME=1 to temporarily allow entering 2f4494.
             */
            uc_reg_write(uc, UC_ARM_REG_PC, &lr);
            return;
        }
        /* Defensive: avoid allow_calls corruption causing runaway allow. */
        ...
        printf("[JJFB_2F4494_ALLOW] #%u allow_left=%u mode=%u lr=0x%X\n",
               n4494, jjfb_chrome_allow_calls, mode, lr);
        fflush(stdout);
        /*
         * 2f4494 = decorative frame (wy_jiao/wy_xian/jiantou bmps via 2d92dc),
         * mode 0xa path. Objects load OK, but 303d94's FP was CODE junk
         * (0x80278); even after DrawRect redirect, 310bb4 blit loop still
         * prevents return to 2e885a. Chrome is optional for drawText present
         * ??return immediately so 305bf4->0x11F00 runs. Not a fake refresh.
         */
    } else if ((uint32_t)address == 0x2F46D6) {
        /* ldrh r0,[r4,#0xa] ??first use of layout bmp object */
        ...
        if (n46d6 <= 12 || (n46d6 % 40) == 0)
            printf("[JJFB_UI] 2f4494@46d6 #%u r4=0x%X r5=0x%X obj+0xa=%u (before 303d94)\n",
    ...
    } else if ((uint32_t)address == 0x2EC6B0) {
        /*
         * Splash/bmp draw wrapper (called from 0x2EFA92):
         *   r0=obj  r1=x  r2=y  r3=flags
         *   sp: src_x, w, h, ...
         * Then blx *(ERW+0x150C). When that FP is 0x270F/junk, guest never blits.
         * JJFB_2EC6B0_RET: 0|1|obj|pixels ??A/B for state advance (not cosmetics).
         */
        ...
        dst_x = (int16_t)r1;
        dst_y = (int16_t)r2;
        if (n_c6b0 <= 40 || (n_c6b0 % 40) == 0 || (req && jjfb_is_firstscreen_name(req->name))) {
        ...
        if (host_blit && req && req->guest_pixels && req->w > 0 && req->h > 0 &&
            ...
            /* Skip broken guest FP (0x270F sentinel / unmapped) after host blit. */
            if (fp150c == 0 || fp150c == 0x270F || fp150c < 0x1000 ||
                ...
                if (rv && rv[0]) {
                    if (strcmp(rv, "obj") == 0)
                        r0ret = r0;
                    else if (strcmp(rv, "pixels") == 0)
                        r0ret = req->guest_pixels;
                    else
                        r0ret = (uint32_t)strtoul(rv, NULL, 0);
                }
                if (n_c6b0 <= 24 || (n_c6b0 % 40) == 0) {
                    printf("[JJFB_2EC6B0_GUARD] bad fp150C=0x%X -> host blit, r0=0x%X (%s) ret lr\n",
                           fp150c, r0ret, rv && rv[0] ? rv : "1");
                    fflush(stdout);
                }
                uc_reg_write(uc, UC_ARM_REG_R0, &r0ret);
                uc_reg_write(uc, UC_ARM_REG_PC, &ret);
        ...
        uc_reg_read(uc, UC_ARM_REG_R6, &r6);
        uc_reg_read(uc, UC_ARM_REG_R7, &r7);
        /* r4 holds FP about to be blx'd (loaded from ERW+0x150C). */
        fp = r4;
        req = jjfb_bmp_cache_find_pixels(r0);
        if (n_c712 <= 24 || (n_c712 % 40) == 0 || req) {
            printf("[JJFB_2EC712_BLX] #%u lr=0x%X fp=0x%X px=0x%X x=%d y=%d w_r3=0x%X name=%s\n",
        ...
        if ((int32_t)prog > (int32_t)r6) {
            jjfb_progress_drew = 1;
            if (n_prog <= 64 || (n_prog % 20) == 0 || (uint32_t)address == 0x2EFADA) {
        ...
        } else if (n_prog <= 48 || (n_prog % 40) == 0 || (uint32_t)address == 0x2EFADA) {
        ...
        /* If this looks like a splash bmp draw, host-blit with guest coords. */
        if (req && req->guest_pixels && req->w > 0 && req->h > 0) {
            int dst_y = (int32_t)r1;
            int dst_x = (int32_t)r2;
            /* Prefer r1=y r2=x; if clearly out of canvas, swap once. */
    ...
    } else if ((uint32_t)address == 0x303D94) {
        static uint32_t n3d94;
        uint32_t tbl = 0, fp = 0, slot = 0;
        n3d94++;
        /* Guest: ldr tbl,[ext_base]; r7=*(tbl+0x1E8). Dump chain (2f4494 still nop). */
        if (jjfb_guest_ext_base) {
            slot = jjfb_guest_ext_base; /* [GOT-ish+0x38] resolves here */
            uc_mem_read(uc, slot, &tbl, 4);
            if (tbl && getMrpMemPtr(tbl + 0x1E8))
                uc_mem_read(uc, tbl + 0x1E8, &fp, 4);
        }
        if (n3d94 <= 12 || (n3d94 % 40) == 0) {
            printf("[JJFB_UI] enter 0x303d94 #%u lr=0x%X r0=0x%X r1=%d r2=%d r3=0x%X\n",
                   n3d94, lr, r0, (int32_t)r1, (int32_t)r2, r3);
            printf("[JJFB_303D94] ctx_slot=0x%X tbl=0x%X fp@+1E8=0x%X "
    ...
    } else if ((uint32_t)address == 0x303DB4) {
        /* ldr r7,[r0,#0x28] ??r0 already = tbl+0x1C0 */
        static uint32_t n3db4;
        uint32_t r7pre = 0, tbl = 0, fp = 0;
        n3db4++;
        tbl = r0 - 0x1C0;
        if (getMrpMemPtr(r0 + 0x28))
            uc_mem_read(uc, r0 + 0x28, &fp, 4);
        uc_reg_read(uc, UC_ARM_REG_R7, &r7pre);
        if (n3db4 <= 12 || (n3db4 % 40) == 0) {
            printf("[JJFB_303D94] ldr r7 prep #%u tbl=0x%X (r0-1C0) slot=+1E8 -> 0x%X "
                   "r0=0x%X (expect mr_table)\n",
                   n3db4, tbl, fp, r0);
            fflush(stdout);
        }
        (void)r7pre;
    } else if ((uint32_t)address == 0x303DBA) {
        /* blx r7 ??intended mr_table draw op (DrawRect @+0x1E8). */
        static uint32_t n3dba;
        uint32_t r7 = 0;
        n3dba++;
        uc_reg_read(uc, UC_ARM_REG_R7, &r7);
        if (n3dba <= 16 || (n3dba % 40) == 0)
            printf("[JJFB_UI] 303d94 blx r7=#%u r7=0x%X r0=0x%X r1=%d r2=%d\n",
                   n3dba, r7, r0, (int32_t)r1, (int32_t)r2);
        /*
         * Bug: graphics ctx often holds CODE_ADDRESS-based junk, so r7 lands
         * in 0x80xxx (e.g. 0x80278) with no bridge hook. Executing there
         * ends the emu (PC hits stop) and 2f4494/2f284c never return to
         * 2e885a/305bf4. Keep 2f4494 nop for main UI; this path is side diag.
         * Do NOT hard-redirect r7 here while chrome is nop'd ??log only.
         */
        if (r7 >= CODE_ADDRESS && r7 < CODE_ADDRESS + 0x10000) {
            if (n3dba <= 16 || (n3dba % 40) == 0)
                printf("[JJFB_303D94] BAD FP r7=0x%X (CODE junk; chrome still nop)\n", r7);
            fflush(stdout);
        } else if (r7 < 0x1000 || !getMrpMemPtr(r7 & ~1u)) {
            printf("[JJFB_UI] 303d94 BAD FP r7=0x%X -> skip blx, ret0\n", r7);
            fflush(stdout);
            {
                uint32_t ret = lr;
                uint32_t zero = 0;
...
static void jjfb_install_ui_hooks(uc_engine *uc) {
    static const uint32_t addrs[] = {
        ...
        0x2EC6B0, /* splash bmp draw wrapper: (obj,x,y,...) ??blx *(ERW+0x150C) */
        0x2EC6E2, /* inside 2EC6B0: prepare args / path when sp flag==0 */
        0x2EC712, /* blx draw FP */
        ...
        0x2E87AC, 0x2F284C, 0x2EA180, 0x2F4494, 0x2F46D6,
        0x303D94, 0x303DB4, 0x303DBA,
    ...
    printf("[JJFB_UI] installed probes through 2f4494/303d94/310bb4/305bf4/30680c\n");
    fflush(stdout);
}
...
/* v56 GWY bring-up: unlock first pixels without host FORCE mem-write of ui_mode.
 * 1) seed 240x320 clip/dims (platform)
 * 2) call guest 0x2FC8B8 → C44 refresh gate
 * 3) call guest 0x2FC03C (may only leave ui_mode=0x3 without BL writer)
 * 4) if still not 0x45, call guest 0x2FC418 natural writer directly
 * 5) re-assert C44 if cleared; dark-fill + present (not pure white)
 * Disable with JJFB_GWY_BRINGUP=0.
 * Probe only: still not host uc_mem_write of ui_mode. */
static int jjfb_gwy_bringup_done;
static void jjfb_gwy_bringup_first_screen(uc_engine *uc) {
    const char *env;
    uint32_t mode_before = 0, mode_mid = 0, mode_after = 0;
    uint32_t z = 0, w = SCREEN_WIDTH, h = SCREEN_HEIGHT;
    uint16_t one16 = 1;
    uint8_t c44 = 0;
    if (!uc || !jjfb_guest_ext_erw || jjfb_gwy_bringup_done)
        return;
    env = getenv("JJFB_GWY_BRINGUP");
    if (env && env[0] == '0' && (!env[1] || env[1] == '\0')) {
        printf("[JJFB_GWY_BRINGUP] skipped (JJFB_GWY_BRINGUP=0)\n");
        fflush(stdout);
        jjfb_gwy_bringup_done = 1;
        return;
    }
    jjfb_gwy_bringup_done = 1;
    uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_UI_MODE, &mode_before, 4);
    printf("[JJFB_GWY_BRINGUP] contract=dims+C44+guest_2FC03C_then_2FC418+DispUpEx "
           "no_FORCE_memwrite\n");
    printf("[JJFB_GWY_BRINGUP] start ui_mode=0x%X (call guest writer, not FORCE write)\n",
           mode_before);
    fflush(stdout);
    ...
    uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x58, &h, 4);
    uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x5C, &w, 4);
    uc_mem_write(uc, jjfb_guest_ext_erw + 0xD14, &one16, 2);
    printf("[JJFB_GWY_BRINGUP] seed screen 240x320 + hwD14=1\n");
    fflush(stdout);

    printf("[JJFB_GWY_BRINGUP] call 0x2FC8B8 enable C44\n");
    fflush(stdout);
    jjfb_run_guest_thumb(uc, 0x2FC8B9, 0, 0);
    uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_FLAG_C44, &c44, 1);
    printf("[JJFB_GWY_BRINGUP] C44=%u after 2FC8B8\n", c44);
    fflush(stdout);

    printf("[JJFB_GWY_BRINGUP] call 0x2FC03C (may not BL writer; expect probe)\n");
    fflush(stdout);
    jjfb_run_guest_thumb(uc, 0x2FC03D, 0, 0);
    uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_UI_MODE, &mode_mid, 4);
    printf("[JJFB_GWY_BRINGUP] ui_mode 0x%X -> 0x%X after 2FC03C\n",
           mode_before, mode_mid);
    fflush(stdout);

    mode_after = mode_mid;
    if (mode_mid != JJFB_UI_MODE_SPLASH) {
        /* v56 evidence: bare 2FC03C left ui_mode=0x3 and never entered 2FC418.
         * Call the natural 0x45 writer itself (guest code, not host FORCE). */
        printf("[JJFB_GWY_BRINGUP] call 0x2FC418 natural writer (ui_mode!=0x45)\n");
        fflush(stdout);
        jjfb_run_guest_thumb(uc, 0x2FC419, 0, 0);
        uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_UI_MODE, &mode_after, 4);
        printf("[JJFB_GWY_BRINGUP] ui_mode 0x%X -> 0x%X after 2FC418\n",
               mode_mid, mode_after);
        fflush(stdout);
    }

    /* 2FC03C / timer may clear C44; re-open refresh gate for first paint. */
    uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_FLAG_C44, &c44, 1);
    if (!c44) {
        printf("[JJFB_GWY_BRINGUP] C44 cleared; re-call 0x2FC8B8\n");
        fflush(stdout);
        jjfb_run_guest_thumb(uc, 0x2FC8B9, 0, 0);
        uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_FLAG_C44, &c44, 1);
    }
    printf("[JJFB_GWY_BRINGUP] final ui_mode=0x%X C44=%u\n", mode_after, c44);
    fflush(stdout);

    jjfb_screen_fill_rect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, jjfb_rgb565(8, 12, 40));
    guiDrawBitmap(jjfb_screen565, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    printf("[JJFB_GWY_BRINGUP] presented dark baseline (DispUpEx also live)\n");
    fflush(stdout);
}

/* v57: present original splash RGB565 already constructed by 0x10134.
 * docx: draw then refresh. Uses game assets + locked 240x320 layout — not fake UI.
 * Disable with JJFB_GWY_SPLASH_BLIT=0. */
static int jjfb_gwy_splash_blit_done;
static void jjfb_gwy_present_splash_assets(uc_engine *uc) {
    const char *env;
    int i, n = 0;
    if (!uc || jjfb_gwy_splash_blit_done)
        return;
    env = getenv("JJFB_GWY_SPLASH_BLIT");
    if (env && env[0] == '0' && (!env[1] || env[1] == '\0')) {
        jjfb_gwy_splash_blit_done = 1;
        printf("[JJFB_GWY_SPLASH] blit skipped (JJFB_GWY_SPLASH_BLIT=0)\n");
        fflush(stdout);
        return;
    }
    for (i = 0; i < jjfb_bmp_cache_n; i++) {
        JjfbBmpReq *req = &jjfb_bmp_cache[i];
        int dx, dy;
        if (!req->valid || !req->guest_pixels || !jjfb_is_firstscreen_name(req->name))
            continue;
        dx = (SCREEN_WIDTH - req->w) / 2;
        dy = 80;
        if (strstr(req->name, "loadingbar")) {
            dx = (SCREEN_WIDTH - req->w) / 2; /* expect 19 */
            dy = 220;
        } else if (strstr(req->name, "textbar")) {
            dx = (SCREEN_WIDTH - req->w) / 2;
            dy = 240;
        } else if (strstr(req->name, "bar!")) {
            dx = (SCREEN_WIDTH - req->w) / 2;
            dy = 210;
        } else if (strstr(req->name, "slogo")) {
            dx = (SCREEN_WIDTH - req->w) / 2;
            dy = 80;
        }
        if (dx < 0) dx = 0;
        if (dy < 0) dy = 0;
        jjfb_host_blit_req(uc, req, dx, dy, "GWY_SPLASH");
        n++;
    }
    if (n > 0) {
        jjfb_gwy_splash_blit_done = 1;
        guiDrawBitmap(jjfb_screen565, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
        printf("[JJFB_GWY_SPLASH] presented %d original assets (refresh after draw)\n", n);
        fflush(stdout);
    }
}
...
void jjfb_on_robotol_timer_tick(uc_engine *uc) {
    ...
    /* After splash 10134 constructs assets, present originals (docx refresh). */
    if (jjfb_handler_tick >= 12 && jjfb_handler_tick <= 40)
        jjfb_gwy_present_splash_assets(uc);
    ...
    if (!jjfb_forced_state && jjfb_handler_tick == 10 && jjfb_guest_ext_erw) {
        ...
        if (skip_force) {
            ...
            jjfb_install_ac8_watch(uc);
            jjfb_install_uimode_writer_hooks(uc);
            /* Platform bring-up: dims + C44 + guest writer + baseline present. */
            jjfb_gwy_bringup_first_screen(uc);
        } else {
        uc_mem_read(uc, jjfb_guest_ext_erw + JJFB_ERW_STATE, &st, 4);
        if (st == 0 || st == 1) {
             ...
             * made x+w > 0 always fail ??negative width ??never reached 0x11F00. */
            {
                ...
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x0C, &w, 4);
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x10, &h, 4);
                /* +0x4C must != +0x10 (320); use screen width for clip too. */
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x4C, &w, 4);
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x50, &z, 4);
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x58, &h, 4);
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x7D8 + 0x5C, &w, 4);
    ...
    /* After a few ticks, poke mrc_event once to leave idle splash. */
    if (jjfb_handler_tick == 5) {
        printf("[JJFB_SEND] inject mrc_event(0,0,0) via helper code=1\n");
        fflush(stdout);
        /* Build mini event in guest mem: code,p0,p1,p2,p3 */
        int32_t ev[5] = {0, 0, 0, 0, 0};
        void *emem = my_mallocExt(sizeof(ev));
        if (emem) {
            memcpy(emem, ev, sizeof(ev));
            /* Direct helper call would re-enter mutex; use raw run instead. */
            {
                uint32_t Paddr = jjfb_guest_ext_P;
                uint32_t helper = jjfb_guest_ext_helper;
                uint32_t erw = jjfb_guest_ext_erw;
                uint32_t in_addr = toMrpMemAddr(emem);
                uint32_t code_u = 1, il = (uint32_t)sizeof(ev);
                uint32_t r9, sp;
                uint32_t *oh = NULL;
                int32_t *ol = NULL;
                uint32_t oh_a, ol_a;
                    ...
                    if (oh && ol) {
                        *oh = 0; *ol = 0;
                        ...
                        runCode(uc, helper & ~1u, CODE_ADDRESS, (helper & 1u) != 0);
                        {
                            uint32_t rv = 0;
                            uc_reg_read(uc, UC_ARM_REG_R0, &rv);
                            printf("[JJFB_SEND] inject event ret=%d\n", (int32_t)rv);
                            fflush(stdout);
                        }
                        sp += 8;
                        uc_reg_write(uc, UC_ARM_REG_SP, &sp);
...
static void br_jjfb_sendAppEvent(BridgeMap *o, uc_engine *uc) {
    ...
    if (extCode == 0x10102 || extCode == 0x10162 || extCode == 0x10165) {
         ...
         * Return value is ignored by caller; still alloc for compatibility. */
    ...
    if (extCode == 0x10113) {
        /* Likely getProc/capability ??returning bare 1 makes guest blx to 0x1.
         * Return a Thumb stub that returns 0/1 instead. */
    ...
    if (extCode == 0x10132) {
        ...
        if (sz >= 4 && sz < 0x800000) {
            ...
            if (p) {
                memset(p, 0, sz);
                /* Store payload size in header word. */
                *(uint32_t *)p = (sz >= 4) ? (sz - 4) : 0;
                uint32_t addr = toMrpMemAddr(p);
                static uint32_t jjfb_10132_n;
                jjfb_10132_n++;
                if (jjfb_10132_n <= 6 || (jjfb_10132_n % 100) == 0 || sz > 64) {
    ...
    if (extCode == 0x10134) {
        ...
        JjfbBmpReq *req = NULL;
        uint32_t site_lr_early = 0;
        n10134++;
        uc_reg_read(uc, UC_ARM_REG_LR, &site_lr_early);
        if (jjfb_bmp_last.valid && jjfb_bmp_last.bytes == (int)app)
            req = &jjfb_bmp_last;
        if (!req)
            req = jjfb_bmp_cache_find_bytes((int)app);
        /* v57: no 2d92dc name yet — bootstrap from locked splash sizes. */
        if (!req && jjfb_bmp_req_from_bytes(app, site_lr_early)) {
            if (jjfb_bmp_last.valid && jjfb_bmp_last.bytes == (int)app)
                req = &jjfb_bmp_last;
            if (!req)
                req = jjfb_bmp_cache_find_bytes((int)app);
        ...
        if (req && req->bytes == (int)app && app >= 16 && app < 0x200000) {
            ...
            if (p) {
                ...
                /* v38: optional bitmap object return (mode B). */
                {
    ...
    printf("[JJFB_SEND] unhandled plat/msg 0x%X app=0x%X code=0x%X p0=0x%X p1=0x%X\n",
           extCode, app, code, param0, param1);
    fflush(stdout);
    /* Only treat as alloc when app looks like a size, not a pointer. */
    if (app >= 0x100 && app < 0x100000 && extCode >= 0x10000 &&
