# JJFB v63 - Path A PROBE: one MR_MENU_RETURN(5) after lifecycle → 2E4040 → 2DADC4
# Ref: v62 B71 blocks Path C; MOUSE_UP wrong; MENU_RETURN/MOUSE_MOVE is Path A.
param(
  [int]$Seconds = 25,
  [switch]$SkipBuild,
...
$out = Join-Path $logDir "v63_path_a_event_stdout.txt"
$errOut = Join-Path $logDir "v63_path_a_event_stderr.txt"
...
$reportOut = Join-Path $reportDir "v63_path_a_event_run_result.md"
...
Write-Host "=== JJFB v63 Path A (MENU_RETURN once PROBE) ==="
Write-Host "project=$root"
Write-Host "target: Path A event 5 once; reach 2DADC4/writer if possible"
...
if (-not $bridgeText.Contains("[JJFB_V63_PATH_A]") -or
    -not $bridgeText.Contains("[JJFB_V62_FLAG]") -or
    -not $bridgeText.Contains("[JJFB_V61_NEST]") -or
    -not $bridgeText.Contains("jjfb_lifecycle_family_app") -or
    -not $vmrpText.Contains("JJFB_FAMILY_APP2_AFTER_INIT")) {
  throw "v63 overlay missing (need V63_PATH_A + V62_FLAG + V61_NEST + FAMILY_APP2)."
}
...
$env:JJFB_PATH_A_EVENT_ONCE = "5"  # PROBE: single MR_MENU_RETURN → 2DADC4
...
Write-Host "[4/6] Build v63 path-a"
...
if ($afterHash -ne $runtimeHash) {
  throw "jjfb.mrp changed during v62 run: before=$runtimeHash after=$afterHash"
}
...
$analyzer = Join-Path $root "scripts\v63_analyze_path_a_event_log.py"
& $python.Source $analyzer $out $reportOut
if ($LASTEXITCODE -ne 0) { throw "analyzer failed" }

$pattern = "JJFB_V63_PATH_A|caller_2E4066|gate_init_2DADC4|gate_B70|uimode_writer|ok_to_2DADC4|TARGETS_2DADC4|FORCE state"
Select-String -Path $out -Pattern $pattern | Select-Object -First 140 | ForEach-Object { $_.Line }
Write-Host "---"
