...
int startVmrp() {
    ...
    if (bridge_dsm_init(uc) == MR_SUCCESS) {
        ...
        char *target_env = getenv("JJFB_GWY_TARGET");
        char startParamBuf[384];

        if (gwy_launcher_mode && target_env && target_env[0]) {
            /* Allow smoke-testing other GWY titles: gwy/tlbb.mrp, gwy/ssjx.mrp, ... */
            filename = target_env;
        }
        ...
        if (!startParam || !*startParam) {
            if (gwy_launcher_mode && target_env && target_env[0] &&
                strcmp(target_env, "gwy/jjfb.mrp") != 0) {
                /* Generic cfg-style param; per-game nextid/ncode unknown — keep
                 * structure, swap nmrpname only. */
                snprintf(startParamBuf, sizeof(startParamBuf),
                         "napptype=12_nextid=0_ncode=0_narg=0_narg1=1_nmrpname=%s_gwyblink",
                         target_env);
                startParam = startParamBuf;
            } else {
        ...
        if (gwy_launcher_mode) {
            ...
            my_resolve_path(filename, target_host, sizeof(target_host));
            printf("[JJFB_GWY_LAUNCH] cfg_index=36 target=%s\n", filename);
