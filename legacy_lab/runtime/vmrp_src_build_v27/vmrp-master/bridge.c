...
static void br_mr_free(BridgeMap *o, uc_engine *uc) {
    ...
    if (p && getMrpMemPtr(p)) {
        if (!my_safe_freeExt(getMrpMemPtr(p))) {
            /* Foreign / guest-heap ptr: ignore (do not corrupt host freelist). */
            static uint32_t n_ign;
            n_ign++;
            if (n_ign <= 8 || (n_ign % 50) == 0) {
                printf("[JJFB_MR_FREE] ignore non-host ptr=0x%X len=%u\n", p, len);
                fflush(stdout);
            }
        }
    }
...
/* 0x10132 sz<4: guest writes at block+4 past the 3-byte payload into a
 * neighbor object / freelist → BAD_SIZE storm. Snapshot + rewrite-restore. */
static uint8_t jjfb_10132_oob_snap[32];
static uint32_t jjfb_10132_oob_guest;
static int jjfb_10132_oob_active;
static uc_hook jjfb_10132_oob_hook;
static int jjfb_10132_oob_hook_on;
static uint32_t jjfb_10132_oob_hits;
...
static void jjfb_hook_10132_oob_write(uc_engine *uc, uc_mem_type type,
                                      uint64_t address, int size, int64_t value,
                                      void *user_data) {
    (void)type;
    (void)user_data;
    (void)value;
    if (!jjfb_10132_oob_active || !jjfb_10132_oob_guest)
        return;
    if (address + (uint64_t)size <= jjfb_10132_oob_guest ||
        address >= jjfb_10132_oob_guest + sizeof(jjfb_10132_oob_snap))
        return;
    /* Undo OOB store into neighbor / freelist (host poke — avoids re-entry). */
    {
        void *host = getMrpMemPtr(jjfb_10132_oob_guest);
        if (host)
            memcpy(host, jjfb_10132_oob_snap, sizeof(jjfb_10132_oob_snap));
    }
    jjfb_10132_oob_hits++;
    if (jjfb_10132_oob_hits <= 8 || (jjfb_10132_oob_hits % 40) == 0) {
        uint32_t pc = 0, lr = 0;
        uc_reg_read(uc, UC_ARM_REG_PC, &pc);
        uc_reg_read(uc, UC_ARM_REG_LR, &lr);
        printf("[JJFB_10132_OOB] restore #%u addr=0x%llX size=%d pc=0x%X lr=0x%X\n",
               jjfb_10132_oob_hits, (unsigned long long)address, size, pc, lr);
        fflush(stdout);
    }
}

static void jjfb_10132_oob_arm(uc_engine *uc, void *user, uint32_t user_addr, uint32_t sz) {
    char *oob_host;
    uint32_t oob_guest;
    uint32_t off;
    uc_err err;

    if (!uc || !user || !user_addr || !sz)
        return;
    /* First byte past the guest-visible payload (may be chunk padding). */
    off = sz;
    oob_host = (char *)user + off;
    oob_guest = user_addr + off;
    if (!getMrpMemPtr(oob_guest))
        return;
    memcpy(jjfb_10132_oob_snap, oob_host, sizeof(jjfb_10132_oob_snap));
    jjfb_10132_oob_guest = oob_guest;
    jjfb_10132_oob_active = 1;
    jjfb_10132_oob_hits = 0;
    if (jjfb_10132_oob_hook_on) {
        uc_hook_del(uc, jjfb_10132_oob_hook);
        jjfb_10132_oob_hook_on = 0;
    }
    err = uc_hook_add(uc, &jjfb_10132_oob_hook, UC_HOOK_MEM_WRITE,
                      jjfb_hook_10132_oob_write, NULL,
                      oob_guest, oob_guest + sizeof(jjfb_10132_oob_snap) - 1);
    if (err) {
        printf("[JJFB_10132_OOB] hook fail err=%u @0x%X\n", err, oob_guest);
        jjfb_10132_oob_active = 0;
    } else {
        jjfb_10132_oob_hook_on = 1;
        printf("[JJFB_10132_OOB] armed @0x%X..0x%X off=%u peek=0x%02X%02X%02X%02X\n",
               oob_guest, oob_guest + (uint32_t)sizeof(jjfb_10132_oob_snap) - 1, off,
               jjfb_10132_oob_snap[0], jjfb_10132_oob_snap[1],
               jjfb_10132_oob_snap[2], jjfb_10132_oob_snap[3]);
    }
    fflush(stdout);
}
...
static const char *jjfb_erw_off_tag(uint32_t off) {
    ...
    if (off == 0xBE8) return "BE8_netphase"; /* 2F5404: 3|4 → BL 3122d8 while ui_mode=0x44 */
...
static void jjfb_hook_erw_band_write(uc_engine *uc, uc_mem_type type,
    ...
    if (off == JJFB_ERW_STATE_EARLY || off == JJFB_ERW_SPLASH_CNT_AC8 ||
        off == JJFB_ERW_PROGRESS_BA0 + 0x2C || off == 0xBE8 ||
        (jjfb_verbose_logs() &&
...
static void br_mr_open(BridgeMap *o, uc_engine *uc) {
    ...
    if (ret) {
        ...
        /* 0@s0.map etc.: member@pack / download cache.
         * RDONLY miss may auto-create empty host file (see fileLib my_open). */
        if (filenameStr && strchr(filenameStr, '@'))
            printf("[JJFB_FILEOPEN_AT] guest=\"%s\" miss mode=%u "
                   "(member@pack / download cache)\n",
                   filenameStr, mode);
    }
    fflush(stdout);
...
static void br_mr_getLen(BridgeMap *o, uc_engine *uc) {
    ...
    char *filenameStr = getMrpMemPtr(filename);
    LOG("ext call %s(%s)\n", o->name, filenameStr);
    len = my_getLen(filenameStr);
    if (filenameStr && strchr(filenameStr, '@')) {
        printf("[JJFB_FILEIO] getLen guest=\"%s\" len=%d\n", filenameStr, (int)len);
        fflush(stdout);
    }
    SET_RET_V(len);
}
...
static void br_mem_free(BridgeMap *o, uc_engine *uc) {
    ...
    if (mem && getMrpMemPtr(mem))
        my_safe_freeExt(getMrpMemPtr(mem));
    SET_RET_V(MR_SUCCESS);
}
...
static void br_mr_getNetworkID(BridgeMap *o, uc_engine *uc) {
    /* Match mythroad/dsm.c: MR_NET_ID_MOBILE. nettype.sav also says cmnet. */
    (void)o;
    (void)uc;
    printf("[JJFB_NET] getNetworkID -> MR_NET_ID_MOBILE(0)\n");
    fflush(stdout);
    SET_RET_V(0); /* MR_NET_ID_MOBILE */
}
...
static void br_mr_connectWAP(BridgeMap *o, uc_engine *uc) {
    ...
    s = wap ? (char *)getMrpMemPtr(wap) : NULL;
    printf("[JJFB_NET] connectWAP(%s) -> ok (platform stub)\n",
           s ? s : "(null)");
    fflush(stdout);
    SET_RET_V(MR_SUCCESS);
}
...
static void runCode(uc_engine *uc, uint32_t startAddr, uint32_t stopAddr, bool isThumb) {
    ...
    if (err) {
        ...
        if (err == UC_ERR_READ_UNMAPPED || err == UC_ERR_FETCH_UNMAPPED ||
            err == UC_ERR_WRITE_UNMAPPED || err == UC_ERR_EXCEPTION) {
            static uint32_t unmap_n;
            unmap_n++;
            if (unmap_n <= 12 || (unmap_n % 50) == 0)
                printf("[JJFB_UNMAP] survive #%u err=%u pc=0x%X lr=0x%X r0=0x%X r4=0x%X r9=0x%X\n",
                       unmap_n, (unsigned)err, pc, lr, r0, r4, r9);
            if (err == UC_ERR_EXCEPTION && getMrpMemPtr(pc & ~1u)) {
                uint8_t op[8];
                if (uc_mem_read(uc, pc & ~1u, op, sizeof(op)) == UC_ERR_OK) {
                    printf("[JJFB_EXCEPTION] bytes@pc %02X %02X %02X %02X %02X %02X %02X %02X\n",
                           op[0], op[1], op[2], op[3], op[4], op[5], op[6], op[7]);
                    fflush(stdout);
                    /* ARM bx lr (aeabi_idiv0 tail after "Divide By Zero" print). */
                    if (op[0] == 0x1E && op[1] == 0xFF && op[2] == 0x2F && op[3] == 0xE1 &&
                        lr > CODE_ADDRESS && lr < END_ADDRESS) {
                        uint32_t retpc = lr;
                        uc_reg_write(uc, UC_ARM_REG_PC, &retpc);
                        printf("[JJFB_EXCEPTION] recover ARM bx lr -> 0x%X\n", lr);
                        fflush(stdout);
                        err = uc_emu_start(uc, retpc, stopAddr, 0, 0);
                        if (!err)
                            return;
                        uc_reg_read(uc, UC_ARM_REG_PC, &pc);
                        uc_reg_read(uc, UC_ARM_REG_LR, &lr);
                    }
                    /* Common Thumb epilogue: pop {r3}; bx r3 — recover via LR. */
                    else if (op[0] == 0x08 && op[1] == 0xBC && op[2] == 0x18 && op[3] == 0x47 &&
                        lr > CODE_ADDRESS && lr < END_ADDRESS) {
                        uint32_t retpc = lr | 1u;
                        uc_reg_write(uc, UC_ARM_REG_PC, &retpc);
                        printf("[JJFB_EXCEPTION] recover bx r3 -> lr=0x%X\n", lr);
                        fflush(stdout);
                        err = uc_emu_start(uc, retpc, stopAddr, 0, 0);
                        if (!err)
                            return;
                        uc_reg_read(uc, UC_ARM_REG_PC, &pc);
                        uc_reg_read(uc, UC_ARM_REG_LR, &lr);
                    } else if (op[0] == 0x01 && op[1] == 0xB0 && op[2] == 0x08 &&
                               op[3] == 0xBC && op[4] == 0x18 && op[5] == 0x47) {
                        /* add sp,#4; pop {r3}; bx r3 — ret addr is on stack, not LR. */
                        uint32_t sp = 0, ret = 0, new_sp;
                        uc_reg_read(uc, UC_ARM_REG_SP, &sp);
                        if (uc_mem_read(uc, sp + 4, &ret, 4) == UC_ERR_OK &&
                            ret > CODE_ADDRESS && ret < END_ADDRESS) {
                            new_sp = sp + 8;
                            uc_reg_write(uc, UC_ARM_REG_SP, &new_sp);
                            uc_reg_write(uc, UC_ARM_REG_R3, &ret);
                            {
                                uint32_t retpc = ret | 1u;
                                uint32_t r0ok = 0;
                                /* sz=3 open-miss path: treat as soft fail so caller
                                 * can fall through to download/net instead of abort. */
                                if (r0 == 3)
                                    uc_reg_write(uc, UC_ARM_REG_R0, &r0ok);
                                uc_reg_write(uc, UC_ARM_REG_PC, &retpc);
                            }
                            printf("[JJFB_EXCEPTION] recover epilogue via stack ret=0x%X r0_was=0x%X (lr was 0x%X)\n",
                                   ret, r0, lr);
                            fflush(stdout);
                            err = uc_emu_start(uc, ret | 1u, stopAddr, 0, 0);
                            if (!err)
                                return;
                            uc_reg_read(uc, UC_ARM_REG_PC, &pc);
                            uc_reg_read(uc, UC_ARM_REG_LR, &lr);
                        } else if (lr > CODE_ADDRESS && lr < END_ADDRESS) {
                            uint32_t retpc = lr | 1u;
                            uc_reg_write(uc, UC_ARM_REG_PC, &retpc);
                            printf("[JJFB_EXCEPTION] recover epilogue fallback lr=0x%X\n", lr);
                            fflush(stdout);
                            err = uc_emu_start(uc, retpc, stopAddr, 0, 0);
                            if (!err)
                                return;
...
/* v74: optional second code=5 enqueue (game-lifecycle inject). Default OFF.
 * Opt-in only: JJFB_V74_B58_SECOND=1. Not part of official run path. */
...
static int jjfb_v74_second_enq_enable = 0; /* opt-in: JJFB_V74_B58_SECOND=1 */
/* v75: family app=0xC0 after B71 (same class as APP2: call registered 1E200
 * handler → 30DC44 → 2FEBBC). Default ON with APP2; opt out:
 * JJFB_FAMILY_C0_AFTER_B71=0. Not a raw B70/ui_mode force. */
static int jjfb_v75_c0_enable = 1; /* default ON; set JJFB_FAMILY_C0_AFTER_B71=0 */
static int jjfb_v75_c0_done;
/* v79/v80 PROBES (default OFF). Writing guest ERW / skipping game BL is
 * game-state forcing — not GWY launcher shim. Keep observe logs only.
 * Opt-in: JJFB_V79_CLEAR_B70=1, JJFB_V80_HOLD_44=1, JJFB_V80_BE8_READY=1. */
static int jjfb_v79_clear_b70_enable = 0;
static int jjfb_v80_hold_44_enable = 0;
static int jjfb_v80_be8_ready_enable = 0;
static int jjfb_v80_be8_ready_done;
...
static void jjfb_hook_uimode_writer(uc_engine *uc, uint64_t address, uint32_t size, void *user_data) {
    ...
    case 0x2FB2C4: {
        uint32_t be8 = 0;
        uint8_t b70e = 0;
        /* Observe connecting entry; re-heal shadow net/gfx GOT (platform). */
        jjfb_heal_shadow_gfx(uc, "enter_0x44");
        if (jjfb_guest_ext_erw) {
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xBE8, &be8, 4);
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xB70, &b70e, 1);
            /* PROBE only: JJFB_V79_CLEAR_B70=1. Do not clear B70 by default. */
            if (jjfb_v79_clear_b70_enable && b70e) {
                uint8_t z = 0;
                uc_mem_write(uc, jjfb_guest_ext_erw + 0xB70, &z, 1);
                printf("[JJFB_V79_B70] PROBE clear B70 on enter_0x44\n");
            }
        }
        printf("[JJFB_NET_GATE] enter_0x44 ui_mode=0x%X BE8=%u B70_was=%u "
               "(observe; no guest write by default)\n",
               mode, be8, b70e);
        fflush(stdout);
        break;
    }
    case 0x2DAE00: {
        ...
        fflush(stdout);
        /* C0 inject is opt-in only (JJFB_FAMILY_C0_AFTER_B71=1). */
    ...
    case 0x2DAE24:
        n_2dae24++;
        printf("[JJFB_GAME_SELF] BL_2FC03C_site #%u lr=0x%X B70=%u ui_mode=0x%X\n",
               n_2dae24, lr, b70, mode);
        break;
    case 0x311FE0:
        /* observe-only: 311FD4 may store ui_mode=3 */
        break;
    case 0x2DAE1A: {
        ...
        /* PROBE only: JJFB_V80_HOLD_44=1 skips game BL — not official shim. */
        if (jjfb_v80_hold_44_enable && mode == 0x44u) {
            uint32_t skip = 0x2DAE1E | 1u;
            printf("[JJFB_V80_HOLD44] PROBE skip BL 2FC26C while ui_mode=0x44\n");
            fflush(stdout);
            uc_reg_write(uc, UC_ARM_REG_PC, &skip);
        }
        break;
    }
    case 0x2FB2F0: {
        /* After enter-0x44 may store BE8 from sp+8 and/or force BE8=2. */
        uint32_t be8 = 0, sp8 = 0, sp = 0;
        int8_t flag = 0;
        if (jjfb_guest_ext_erw) {
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xBE8, &be8, 4);
            uc_mem_read(uc, jjfb_guest_ext_erw + 0x12D8, &flag, 1);
        }
        uc_reg_read(uc, UC_ARM_REG_SP, &sp);
        if (sp)
            uc_mem_read(uc, sp + 8, &sp8, 4);
        printf("[JJFB_NET_GATE] post_enter_0x44 BE8=%u sp8=%u flag12D8=%d "
               "ui_mode=0x%X (BE8 writers: only 2FB2E0/2FB2EE in robotol)\n",
               be8, sp8, (int)flag, mode);
        fflush(stdout);
        break;
    }
    case 0x2FC26C: {
    ...
    case 0x2F5404: {
        ...
        if (jjfb_guest_ext_erw)
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xBE8, &be8, 4);
        printf("[JJFB_V56_CALLBACK] entry_2F5404 #%u lr=0x%X "
               "ui_mode=0x%X BE8=%u r0-r3=0x%X,0x%X,0x%X,0x%X\n",
               n_cb_entry, lr, mode, be8, r0, r1, r2, r3);
        if (mode == 0x44u) {
            /* PROBE only: JJFB_V80_BE8_READY=1 writes BE8/f1AE/FE8 — not official. */
            if (jjfb_v80_be8_ready_enable && !jjfb_v80_be8_ready_done &&
                jjfb_guest_ext_erw && be8 == 2u) {
                uint32_t ready = 3;
                uint8_t net_on = 1;
                uint32_t tclear = 0;
                jjfb_v80_be8_ready_done = 1;
                uc_mem_write(uc, jjfb_guest_ext_erw + 0xBE8, &ready, 4);
                uc_mem_write(uc, jjfb_guest_ext_erw + 0x1AE, &net_on, 1);
                uc_mem_write(uc, jjfb_guest_ext_erw + 0xFE8, &tclear, 4);
                be8 = ready;
                printf("[JJFB_V80_BE8] PROBE write BE8=3 f1AE=1 FE8=0\n");
            }
            printf("[JJFB_NET_GATE] 2F5404 while connecting BE8=%u "
                   "(%s)\n",
                   be8,
                   (be8 == 3u || be8 == 4u) ? "will BL 3122d8" : "skip net kick");
            fflush(stdout);
        }
        break;
    }
    case 0x3122D8: {
        uint8_t f1ae = 0, fcae = 0, fc95 = 0, f166 = 0;
        uint32_t tfe8 = 0;
        if (jjfb_guest_ext_erw) {
            uc_mem_read(uc, jjfb_guest_ext_erw + 0x1AE, &f1ae, 1);
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xCAE, &fcae, 1);
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xC95, &fc95, 1);
            uc_mem_read(uc, jjfb_guest_ext_erw + 0x166, &f166, 1);
            uc_mem_read(uc, jjfb_guest_ext_erw + 0xFE8, &tfe8, 4);
        }
        printf("[JJFB_NET_GATE] enter_3122d8 lr=0x%X ui_mode=0x%X "
               "f1AE=%d fCAE=%d fC95=%d f166=%d (path) tFE8=0x%X\n",
               lr, mode, (int)(int8_t)f1ae, (int)(int8_t)fcae,
               (int)(int8_t)fc95, (int)(int8_t)f166, tfe8);
        fflush(stdout);
        break;
    }
    case 0x2F5734:
...
static void jjfb_install_uimode_writer_hooks(uc_engine *uc) {
    static const uint32_t addrs[] = {
        ...
        0x2FB2C4, /* observe connecting entry + platform GOT heal */
        0x2FB2F0, /* post-enter: log BE8 after 2FB2E0/2FB2EE stores */
        0x311FE0, /* observe 311FD4 ui_mode store */
        ...
        0x2F5404, 0x2F5734, 0x305EB8, 0x3122D8, /* +net kick from connecting */
           ...
           "+10130_large_alloc_noop_ok NO_FORCE NO_C0\n");
    {
        ...
        }
        {
            ...
            if (b58 && b58[0] == '1')
                jjfb_v74_second_enq_enable = 1;
            printf("[JJFB_V74_B58] contract=second_101AB_one_record_after_2FC26C "
                   "enable=%d (default OFF; JJFB_V74_B58_SECOND=1) NO_FORCE\n",
        ...
        }
        {
            ...
            if (c0 && c0[0] == '0')
                ...
                jjfb_v75_c0_enable = 1;
            printf("[JJFB_V75_B70] contract=family_C0_after_B71_via_1E200 "
                   "enable=%d (default ON like APP2; JJFB_FAMILY_C0_AFTER_B71=0) "
                   "NO_FORCE_B70_BYTE\n",
                   jjfb_v75_c0_enable);
        }
        {
            const char *c79 = getenv("JJFB_V79_CLEAR_B70");
            if (c79 && c79[0] == '1')
                jjfb_v79_clear_b70_enable = 1;
            printf("[JJFB_V79_B70] PROBE clear_B70_on_enter_0x44 enable=%d "
                   "(default OFF; JJFB_V79_CLEAR_B70=1)\n",
                   jjfb_v79_clear_b70_enable);
        }
        {
            const char *h44 = getenv("JJFB_V80_HOLD_44");
            if (h44 && h44[0] == '1')
                jjfb_v80_hold_44_enable = 1;
            printf("[JJFB_V80_HOLD44] PROBE skip_2FC26C_in_0x44 enable=%d "
                   "(default OFF; JJFB_V80_HOLD_44=1)\n",
                   jjfb_v80_hold_44_enable);
            {
                const char *br = getenv("JJFB_V80_BE8_READY");
                if (br && br[0] == '1')
                    jjfb_v80_be8_ready_enable = 1;
                printf("[JJFB_V80_BE8] PROBE write_BE8/f1AE/FE8 enable=%d "
                       "(default OFF; JJFB_V80_BE8_READY=1)\n",
                       jjfb_v80_be8_ready_enable);
            }
        }
    }
    fflush(stdout);
...
static void jjfb_heal_shadow_gfx(uc_engine *uc, const char *why) {
    ...
    static const uint32_t net_off[] = {
        0x144, /* mr_initNetwork */
        0x148, /* mr_closeNetwork */
        0x14C, /* mr_getHostByName */
        0x150, /* mr_socket */
        0x154, /* mr_connect */
        0x158, /* mr_closeSocket */
        0x15C, /* mr_recv */
        0x164, /* mr_send */
        0x168  /* mr_sendto */
    };
    uint32_t gi, healed = 0, net_healed = 0;
    ...
    for (gi = 0; gi < (uint32_t)(sizeof(net_off) / sizeof(net_off[0])); gi++) {
        uint32_t src = 0, dst = 0;
        uc_mem_read(uc, host + net_off[gi], &src, 4);
        uc_mem_read(uc, shadow + net_off[gi], &dst, 4);
        if (src && dst != src &&
            (dst < 0x1000 || !dst ||
             (dst >= CODE_ADDRESS && dst < CODE_ADDRESS + 0x10000) ||
             dst == 0x270F || dst >= END_ADDRESS)) {
            uc_mem_write(uc, shadow + net_off[gi], &src, 4);
            net_healed++;
        }
    }
    if (healed)
    ...
    if (net_healed) {
        uint32_t init_fp = 0;
        uc_mem_read(uc, shadow + 0x144, &init_fp, 4);
        printf("[JJFB_V77_NET] heal(%s) shadow@0x%X fixed %u net slots "
               "initNetwork_fp=0x%X\n",
               why, shadow, net_healed, init_fp);
    }
    fflush(stdout);
}

static void jjfb_hook_clamp_94e0c(uc_engine *uc, uint64_t address, uint32_t size,
                                  void *user_data) {
    ...
    uc_reg_read(uc, UC_ARM_REG_R2, &r2);
    uc_reg_read(uc, UC_ARM_REG_R3, &r3);
    /* Thumb clear loop: stop absurd remaining counts before END_ADDRESS walk. */
    if (r2 > 0x100000u) {
        uint32_t z = 0;
        static uint32_t n_cl;
        n_cl++;
        if (n_cl <= 6 || (n_cl % 200) == 0) {
            printf("[JJFB_CLAMP] @0x94E0C #%u r0=0x%X r1=0x%X r2=0x%X -> r2=0\n",
                   n_cl, r0, r1, r2);
            fflush(stdout);
        }
        uc_reg_write(uc, UC_ARM_REG_R2, &z);
    } else if (r1 > 0x100000u && r1 < 0xFF000000u && r0 < END_ADDRESS &&
               r1 > END_ADDRESS - 0x1000u) {
        uint32_t z = 0;
        printf("[JJFB_CLAMP] @0x94E0C r1=0x%X near end -> r1=r0\n", r1);
        fflush(stdout);
        uc_reg_write(uc, UC_ARM_REG_R1, &r0);
        uc_reg_write(uc, UC_ARM_REG_R2, &z);
    }
    (void)r3;
}

/* After first absurd 0x10132, skip further plat wrapper calls with huge r1
 * so robotol 0x2D99EB alloc/clear loop can unwind toward net/download. */
static int jjfb_10132_storm_active;
static uint32_t jjfb_10132_storm_skips;

/* robotol 0x2D99AC: heap helper, alloc_size = r0+4, then memset via FP.
 * Callers pass getLen+1 / strlen sum+1. Garbage r0 (e.g. 0x8EB31FAC) starts
 * the BAD_SIZE storm. Reject at entry — return NULL to caller. */
static void jjfb_hook_robotol_2d99ac(uc_engine *uc, uint64_t address,
                                    uint32_t size, void *user_data) {
    uint32_t r0 = 0, lr = 0, need;
    (void)address;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
    need = r0 + 4u;
    if (need < 0x100000u && (r0 & 0xFF000000u) == 0)
        return;
    uc_reg_read(uc, UC_ARM_REG_LR, &lr);
    {
        static uint32_t n_rej;
        uint32_t z = 0;
        uint32_t retpc = (lr & ~1u) | 1u;
        n_rej++;
        if (n_rej <= 12 || (n_rej % 100) == 0) {
            printf("[JJFB_2D99AC] reject #%u r0=0x%X need=0x%X lr=0x%X -> NULL\n",
                   n_rej, r0, need, lr);
            fflush(stdout);
        }
        jjfb_10132_storm_active = 1;
        uc_reg_write(uc, UC_ARM_REG_R0, &z);
        uc_reg_write(uc, UC_ARM_REG_PC, &retpc);
    }
}

/* robotol 0x308d98 / 0x308db8: read big-endian u32 from buffer.
 * Only clamp when the caller is the 0@s0 map-parse block (0x309B80..0x309DE0);
 * elsewhere BE32 is reused for non-count data and must not be zeroed. */
static void jjfb_hook_robotol_be32_ret(uc_engine *uc, uint64_t address,
                                      uint32_t size, void *user_data) {
    uint32_t r0 = 0, lr = 0;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_LR, &lr);
    lr &= ~1u;
    if (lr < 0x309B80u || lr > 0x309DE0u)
        return;
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
    if (r0 <= 0x10000u)
        return;
    {
        static uint32_t n_clamp;
        uint32_t z = 0;
        n_clamp++;
        if (n_clamp <= 16 || (n_clamp % 50) == 0) {
            printf("[JJFB_308D98] clamp be32 #%u addr=0x%llX val=0x%X lr=0x%X -> 0\n",
                   n_clamp, (unsigned long long)address, r0, lr);
            fflush(stdout);
        }
        uc_reg_write(uc, UC_ARM_REG_R0, &z);
    }
...
/* robotol 0x305E30: calloc-ish — r0*r1 bytes via 2D99AC.
 * Backup if a bad count slips past 308d98 clamp. */
static uint32_t jjfb_305e30_rej;
static void jjfb_hook_robotol_305e30(uc_engine *uc, uint64_t address,
                                    uint32_t size, void *user_data) {
    ...
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
    uc_reg_read(uc, UC_ARM_REG_R1, &r1);
    prod = (uint64_t)r0 * (uint64_t)r1;
    if (prod < 0x100000ull && (r0 & 0xFF000000u) == 0 && (r1 & 0xFF000000u) == 0)
        return;
    uc_reg_read(uc, UC_ARM_REG_LR, &lr);
    jjfb_305e30_rej++;
    if (jjfb_305e30_rej <= 12 || (jjfb_305e30_rej % 100) == 0) {
        printf("[JJFB_305E30] reject #%u r0=0x%X r1=0x%X prod=0x%llX lr=0x%X -> NULL\n",
               jjfb_305e30_rej, r0, r1, (unsigned long long)prod, lr);
        fflush(stdout);
    }
    {
        uint32_t z = 0;
...
/* robotol putchar: movs r0,#3; mov r1,sp; svc #0xab
 * Unicorn has no Mythroad SVC → UC_ERR_EXCEPTION and corrupts state.
 * Emulate as host putchar and skip the svc. */
static void jjfb_hook_robotol_svc_ab(uc_engine *uc, uint64_t address,
                                    uint32_t size, void *user_data) {
    uint32_t r0 = 0, r1 = 0, pc;
    uint8_t ch = 0;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
    uc_reg_read(uc, UC_ARM_REG_R1, &r1);
    if (r0 == 3 && r1 && uc_mem_read(uc, r1, &ch, 1) == UC_ERR_OK) {
        static uint32_t n;
        n++;
        if (n <= 80 || (n % 200) == 0) {
            printf("[JJFB_SVC_AB] putchar #%u ch=0x%02X\n", n, ch);
            fflush(stdout);
        }
        fputc((int)ch, stdout);
        if ((n & 0x3F) == 0)
    ...
    /* Thumb svc is 2 bytes — skip it. */
    pc = ((uint32_t)address + 2u) | 1u;
    uc_reg_write(uc, UC_ARM_REG_PC, &pc);
}

/* ARM __aeabi_uidiv/idiv @0x2D8EDC: missing/empty 0@s0 yields divisor 0 →
 * "Arithmetic exception: Divide By Zero" then stuck ui_mode=0x3. Return 0. */
static void jjfb_hook_aeabi_idiv0(uc_engine *uc, uint64_t address,
                                 uint32_t size, void *user_data) {
    uint32_t r1 = 0, lr = 0, z = 0;
    (void)address;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_R1, &r1);
    ...
    uc_reg_read(uc, UC_ARM_REG_LR, &lr);
    {
        ...
        n++;
        if (n <= 12 || (n % 50) == 0) {
            printf("[JJFB_IDIV0] #%u lr=0x%X -> quot=0 (skip div0 trap)\n", n, lr);
            fflush(stdout);
        }
    }
    uc_reg_write(uc, UC_ARM_REG_R0, &z);
    /* LR from Thumb blx has thumb bit; keep it for correct mode return. */
    uc_reg_write(uc, UC_ARM_REG_PC, &lr);
}

/* 2F6BA0 file→buffer: before alloc at 0x2F6BB8, r4=getLen.
 * size 0/@-miss must return NULL so 309AE4 does not parse garbage. */
static int jjfb_map_load_missed;
static void jjfb_hook_2f6bb8_empty_load(uc_engine *uc, uint64_t address,
                                       uint32_t size, void *user_data) {
    uint32_t r4 = 0, sp = 0, ret = 0, z = 0, new_sp;
    (void)address;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_R4, &r4);
    if (r4 >= 1u && r4 < 0x200000u)
        return;
    uc_reg_read(uc, UC_ARM_REG_SP, &sp);
    /* Frame: push {r3,r4,r5,r6,r7,lr} only so far. LR at sp+20. */
    if (uc_mem_read(uc, sp + 20, &ret, 4) != UC_ERR_OK ||
        ret < CODE_ADDRESS || ret >= END_ADDRESS)
        return;
    {
        static uint32_t n;
        n++;
        if (n <= 12 || (n % 40) == 0) {
            printf("[JJFB_2F6BA0] empty/miss size=%d -> NULL (skip map parse)\n",
                   (int)r4);
            fflush(stdout);
        }
    }
    jjfb_map_load_missed = 1;
    new_sp = sp + 24;
    uc_reg_write(uc, UC_ARM_REG_SP, &new_sp);
    uc_reg_write(uc, UC_ARM_REG_R0, &z);
    ret |= 1u;
...
/* After 2F6BA0 in map parse 309AE4: if buffer NULL, jump to epilogue. */
static void jjfb_hook_309b00_null_buf(uc_engine *uc, uint64_t address,
                                     uint32_t size, void *user_data) {
    uint32_t r0 = 0, r4z = 0, pc;
    (void)address;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
        ...
        return;
    {
        ...
        n++;
        if (n <= 8) {
            printf("[JJFB_309AE4] null buffer after load -> epilogue (force miss)\n");
            fflush(stdout);
        }
    }
    jjfb_map_load_missed = 1;
    /* Ensure free-path r4 is NULL before epilogue. */
    uc_reg_write(uc, UC_ARM_REG_R4, &r4z);
    /* 0x309DDE: add sp,#0x3c; pop — skip free. */
    pc = 0x309DDE | 1u;
    uc_reg_write(uc, UC_ARM_REG_PC, &pc);
}

/* 2FE50E/2FE526: layout idiv after map parse. After map-miss ERW scale
 * bytes are 0; some aeabi paths still trap. Skip blx and return quot=0. */
static void jjfb_hook_2fe50e_idiv_guard(uc_engine *uc, uint64_t address,
                                       uint32_t size, void *user_data) {
    uint32_t r0 = 0, r1 = 0, z = 0, next;
    (void)size;
    (void)user_data;
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
    uc_reg_read(uc, UC_ARM_REG_R1, &r1);
    if (!jjfb_map_load_missed && r0 != 0 && r1 != 0)
        return;
    {
        static uint32_t n;
        n++;
        if (n <= 12) {
            printf("[JJFB_2FE50E] skip idiv @0x%llX r0=%d r1=%d miss=%d -> 0\n",
                   (unsigned long long)address, (int)r0, (int)r1, jjfb_map_load_missed);
            fflush(stdout);
        }
    }
    uc_reg_write(uc, UC_ARM_REG_R0, &z);
    uc_reg_write(uc, UC_ARM_REG_R1, &z);
    /* Thumb blx is 4 bytes. */
    next = ((uint32_t)address + 4u) | 1u;
    uc_reg_write(uc, UC_ARM_REG_PC, &next);
}

static void jjfb_hook_plat_wrapper_304550(uc_engine *uc, uint64_t address,
                                          uint32_t size, void *user_data) {
    uint32_t r0 = 0, r1 = 0, lr = 0;
    (void)address;
    (void)size;
    (void)user_data;
    if (!jjfb_10132_storm_active)
        return;
    uc_reg_read(uc, UC_ARM_REG_R0, &r0);
    uc_reg_read(uc, UC_ARM_REG_R1, &r1);
    if (r0 != 0x10132)
        return;
    if (!(r1 >= 0x200000u || (r1 & 0xFF000000u) != 0))
        return;
    uc_reg_read(uc, UC_ARM_REG_LR, &lr);
    if (lr < CODE_ADDRESS || lr >= END_ADDRESS)
        return;
    {
        uint32_t z = 0;
        uint32_t retpc = lr | 1u;
        uc_reg_write(uc, UC_ARM_REG_R0, &z);
        uc_reg_write(uc, UC_ARM_REG_PC, &retpc);
        jjfb_10132_storm_skips++;
        if (jjfb_10132_storm_skips <= 8 || (jjfb_10132_storm_skips % 200) == 0) {
            printf("[JJFB_10132_STORM] skip wrapper #%u r1=0x%X -> ret lr=0x%X\n",
                   jjfb_10132_storm_skips, r1, lr);
            fflush(stdout);
        }
    }
}
...
static void jjfb_install_ui_hooks(uc_engine *uc) {
    ...
    /* Clamp mythroad clear-loop at 0x94E0C: after bad 10132 size, guest
     * memset-walks from a heap ptr to END_ADDRESS (UC_ERR_WRITE_UNMAPPED). */
    {
        static uc_hook h94e;
        static int h94e_on;
        if (!h94e_on) {
            err = uc_hook_add(uc, &h94e, UC_HOOK_CODE, jjfb_hook_clamp_94e0c, NULL,
                              0x94E0C, 0x94E0C, 0);
            if (!err) {
                h94e_on = 1;
                printf("[JJFB_CLAMP] installed memset-guard @0x94E0C\n");
            }
        }
    }
    /* Skip absurd 0x10132 through plat wrapper (thumb may enter mid-range). */
    {
        static uc_hook h550;
        static int h550_on;
        if (!h550_on) {
            err = uc_hook_add(uc, &h550, UC_HOOK_CODE, jjfb_hook_plat_wrapper_304550,
                              NULL, 0x304550, 0x3045C0, 0);
            if (!err) {
                h550_on = 1;
                printf("[JJFB_10132_STORM] wrapper skip hook @0x304550..0x3045C0\n");
            }
        }
    }
    {
        static uc_hook h99ac;
        static int h99ac_on;
        if (!h99ac_on) {
            /* Thumb entry: hook both even/odd. */
            err = uc_hook_add(uc, &h99ac, UC_HOOK_CODE, jjfb_hook_robotol_2d99ac,
                              NULL, 0x2D99AC, 0x2D99AF, 0);
            if (!err) {
                h99ac_on = 1;
                printf("[JJFB_2D99AC] reject-absurd-size hook @0x2D99AC\n");
            }
        }
    }
    {
        static uc_hook h5e30;
        static int h5e30_on;
        if (!h5e30_on) {
            err = uc_hook_add(uc, &h5e30, UC_HOOK_CODE, jjfb_hook_robotol_305e30,
                              NULL, 0x305E30, 0x305E33, 0);
            if (!err) {
                h5e30_on = 1;
                printf("[JJFB_305E30] reject-absurd-product hook @0x305E30\n");
            }
        }
    }
    {
        static uc_hook hbe32a, hbe32b;
        static int hbe32_on;
        if (!hbe32_on) {
            /* After BE assemble: first fn sets r0 at 0x308DB0; second keeps r0
             * through the loop — clamp on pop/bx so r0 is the value. */
            err = uc_hook_add(uc, &hbe32a, UC_HOOK_CODE, jjfb_hook_robotol_be32_ret,
                              NULL, 0x308DB2, 0x308DB5, 0);
            if (!err)
                err = uc_hook_add(uc, &hbe32b, UC_HOOK_CODE, jjfb_hook_robotol_be32_ret,
                                  NULL, 0x308DDA, 0x308DDD, 0);
            if (!err) {
                hbe32_on = 1;
                printf("[JJFB_308D98] clamp-be32 hooks @0x308DB2 / 0x308DDA\n");
            }
        }
    }
    {
        ...
        static int hsvc_on;
        if (!hsvc_on) {
            err = uc_hook_add(uc, &hsvc, UC_HOOK_CODE, jjfb_hook_robotol_svc_ab,
                              NULL, 0x2D92AE, 0x2D92AF, 0);
            if (!err) {
                hsvc_on = 1;
                printf("[JJFB_SVC_AB] putchar hook @0x2D92AE\n");
            }
        }
    }
    {
        static uc_hook hidiv;
        static int hidiv_on;
        if (!hidiv_on) {
            err = uc_hook_add(uc, &hidiv, UC_HOOK_CODE, jjfb_hook_aeabi_idiv0,
                              NULL, 0x2D8EDC, 0x2D8EDF, 0);
            if (!err) {
                hidiv_on = 1;
                printf("[JJFB_IDIV0] guard @0x2D8EDC (aeabi div)\n");
            }
        }
    }
    {
        static uc_hook hload, hparse, hdiva, hdivb;
        static int hload_on;
        if (!hload_on) {
            err = uc_hook_add(uc, &hload, UC_HOOK_CODE, jjfb_hook_2f6bb8_empty_load,
                              NULL, 0x2F6BB8, 0x2F6BBB, 0);
            if (!err)
                err = uc_hook_add(uc, &hparse, UC_HOOK_CODE, jjfb_hook_309b00_null_buf,
                                  NULL, 0x309B00, 0x309B03, 0);
            if (!err)
                err = uc_hook_add(uc, &hdiva, UC_HOOK_CODE, jjfb_hook_2fe50e_idiv_guard,
                                  NULL, 0x2FE50E, 0x2FE511, 0);
            if (!err)
                err = uc_hook_add(uc, &hdivb, UC_HOOK_CODE, jjfb_hook_2fe50e_idiv_guard,
                                  NULL, 0x2FE526, 0x2FE529, 0);
            if (!err) {
                hload_on = 1;
                printf("[JJFB_MAP_MISS] empty-load@0x2F6BB8 + null-buf@0x309B00 + idiv@2FE50E/26\n");
            }
        }
    }
...
static void br_jjfb_sendAppEvent(BridgeMap *o, uc_engine *uc) {
    ...
    if (extCode == 0x10133) {
        /* Free companion to 0x10132/0x10134.
         * Guest may pass user ptr or user-4; never corrupt host freelist. */
        if (app && getMrpMemPtr(app)) {
            if (!my_safe_freeExt(getMrpMemPtr(app))) {
                static uint32_t n_ign;
                n_ign++;
                if (n_ign <= 8 || (n_ign % 40) == 0) {
                    printf("[JJFB_SEND] plat 0x10133 ignore non-host block=0x%X\n", app);
                    fflush(stdout);
                }
            } else if ((jjfb_handler_tick % 40) == 1) {
    ...
    if (extCode == 0x10132) {
        /* Guest treats return as "block": data at block+4, optional hdr at block[0]. */
        uint32_t sz = app;
        uint32_t lr = 0;
        uc_reg_read(uc, UC_ARM_REG_LR, &lr);
        /* Fast-reject post-storm huge sizes (avoid stub/clamp flood). */
        if (jjfb_10132_storm_active &&
            (sz >= 0x200000u || (sz & 0xFF000000u) != 0)) {
            static uint32_t n_fast;
            n_fast++;
            if (n_fast <= 8 || (n_fast % 200) == 0) {
                printf("[JJFB_10132_STORM] fast-reject #%u sz=0x%X\n", n_fast, sz);
                fflush(stdout);
            }
            SET_RET_V(0);
            return;
        }
        if (sz >= 1 && sz < 0x200000 && (sz & 0xFF000000u) == 0) {
            void *p = my_mallocExt(sz);
            if (p) {
                memset(p, 0, sz);
                if (sz >= 4)
                    *(uint32_t *)p = sz - 4;
                {
                    ...
                    jjfb_10132_n++;
                    if (sz < 4)
                        jjfb_10132_oob_arm(uc, p, addr, sz);
                    if (jjfb_10132_n <= 6 || (jjfb_10132_n % 100) == 0 ||
                        sz > 64 || sz < 4) {
                        printf("[JJFB_SEND] plat 0x10132 alloc #%u sz=%u -> block 0x%X lr=0x%X\n",
                               jjfb_10132_n, sz, addr, lr);
                        fflush(stdout);
                    }
                    SET_RET_V(addr);
                    return;
        ...
        } else if (sz >= 0x200000 || (sz & 0xFF000000u) != 0) {
            /* Absorbed bad size (was 0x8EB31FB0 after 0@s0 / 12340).
             * Return NULL — a stub still gets memset with the huge count. */
            static uint32_t n_bad;
            uint32_t r0 = 0, r1 = 0, r2 = 0, r3 = 0;
            n_bad++;
            uc_reg_read(uc, UC_ARM_REG_R0, &r0);
            uc_reg_read(uc, UC_ARM_REG_R1, &r1);
            uc_reg_read(uc, UC_ARM_REG_R2, &r2);
            uc_reg_read(uc, UC_ARM_REG_R3, &r3);
            if (n_bad <= 8 || (n_bad % 100) == 0) {
                printf("[JJFB_SEND] plat 0x10132 BAD_SIZE #%u sz=%u (0x%X) lr=0x%X "
                       "r0-r3=0x%X,0x%X,0x%X,0x%X oob_hits=%u\n",
                       n_bad, sz, sz, lr, r0, r1, r2, r3, jjfb_10132_oob_hits);
                if (n_bad == 1) {
                    uint32_t spv = 0, site_lr = 0, i;
                    uint32_t stk[12];
                    uc_reg_read(uc, UC_ARM_REG_SP, &spv);
                    uc_mem_read(uc, spv + 0x24, &site_lr, 4);
                    printf("[JJFB_10132_BAD] sp=0x%X site_lr(sp+24)=0x%X\n", spv, site_lr);
                    for (i = 0; i < 12; i++)
                        uc_mem_read(uc, spv + i * 4, &stk[i], 4);
                    printf("[JJFB_10132_BAD] stack[0..11]=");
                    for (i = 0; i < 12; i++)
                        printf(" %08X", stk[i]);
                    printf("\n");
                    if (jjfb_10132_oob_guest) {
                        uint8_t cur[16];
                        void *host = getMrpMemPtr(jjfb_10132_oob_guest);
                        uint32_t blk = jjfb_10132_oob_guest - 3; /* sz=3 → user */
                        uint8_t hdr[8];
                        void *bh = getMrpMemPtr(blk - 4);
                        if (host) {
                            memcpy(cur, host, sizeof(cur));
                            printf("[JJFB_10132_OOB] at-BAD snap=%02X%02X%02X%02X cur=%02X%02X%02X%02X "
                                   "guest=0x%X changed=%d\n",
                                   jjfb_10132_oob_snap[0], jjfb_10132_oob_snap[1],
                                   jjfb_10132_oob_snap[2], jjfb_10132_oob_snap[3],
                                   cur[0], cur[1], cur[2], cur[3],
                                   jjfb_10132_oob_guest,
                                   memcmp(cur, jjfb_10132_oob_snap, sizeof(cur)) != 0);
                        }
                        if (bh) {
                            memcpy(hdr, bh, sizeof(hdr));
                            printf("[JJFB_10132_BAD] tiny hdr@0x%X = %02X%02X%02X%02X %02X%02X%02X%02X\n",
                                   blk - 4, hdr[0], hdr[1], hdr[2], hdr[3],
            ...
            if (n_bad == 1 && jjfb_10132_oob_active) {
                jjfb_10132_oob_active = 0;
                printf("[JJFB_10132_OOB] disarm after first BAD_SIZE\n");
                fflush(stdout);
            }
            /* Absurd size: return NULL. Non-null stub made 0x2D99EC take the
             * success path and memset/clear with the huge count forever. */
            if (n_bad == 1) {
                jjfb_10132_storm_active = 1;
                printf("[JJFB_10132_STORM] active — NULL on BAD; skip further huge 0x10132\n");
                fflush(stdout);
            }
            SET_RET_V(0);
            return;
        }
        printf("[JJFB_SEND] plat 0x10132 alloc FAIL sz=%u (0x%X) lr=0x%X\n",
               sz, sz, lr);
        fflush(stdout);
        SET_RET_V(0);
        return;
     ...
     */
    if (extCode == 0x101AB) {
        /* Platform fill for Path A. Deliver local downVersion once (real file
         * state), then empty BE(-1). Re-injecting the same record every enqueue
         * re-arms AC8/gate and kicks the game out of connecting (0x44). */
        static int jjfb_101ab_downver_delivered;
        uint32_t body_size;
        ...
        int with_rec = 0;
        {
            const char *empty = getenv("JJFB_101AB_EMPTY");
            if (empty && empty[0] == '1') {
                with_rec = 0;
            } else if (jjfb_101ab_fill_records) {
                with_rec = 1; /* V74 force one-shot still allowed */
            } else if (!jjfb_101ab_downver_delivered) {
                with_rec = 1;
            }
        }
        ...
        if (with_rec) {
            ...
            /* field C/D: match local downVersion.v BE 0x3EE (2F6C44 compare). */
            payload[o++] = 0;
            payload[o++] = 0;
            payload[o++] = 0;
            payload[o++] = 0;
        ...
        if (with_rec) {
            jjfb_101ab_downver_delivered = 1;
            printf("[JJFB_101AB] plat fill ONCE buf=0x%X out=0x%X type=%u "
                   "payload_len=%u body_sz=%u record=\"%s\"+BE(-1) "
                   "(later 101AB → empty)\n",
                   app, code, param0, payload_len, body_size, rec_name);
        } else {
            printf("[JJFB_101AB] plat fill EMPTY buf=0x%X out=0x%X type=%u "
                   "payload_len=%u body=BE(-1) (delivered=%d)\n",
                   app, code, param0, payload_len, jjfb_101ab_downver_delivered);
        }
        fflush(stdout);
        SET_RET_V(0);
        return;
    ...
    if (extCode == 0x12340) {
        ...
        /* Measure-ish: write font metrics into out pointers.
         * If before1 looks like a pointer (stack/heap), it is not a height —
         * force a sane glyph height to avoid later BAD_SIZE alloc storms. */
        if (param1 && getMrpMemPtr(param1)) {
            int force = (before1 == 0) || (before1 > 0x10000u);
            if (force)
        ...
        if (n12340 <= 8 || (n12340 % 100) == 0) {
            printf("[JJFB_12340] n=%u app=0x%X code=0x%X p0=0x%X p1=0x%X p2=0x%X "
                   ...
                   before1, before2,
                   (before1 == 0 || before1 > 0x10000u) ? out_h : 0);
    ...
    /* v73: 0x10170(app=0xE200, p0=0x1C) — ack SUCCESS.
     * Evidence: unhandled→timer ret=-11/UC_ERR; SUCCESS leaves splash
     * 0x45→0x14→0x44 (connecting). Alloc(p0) regresses (bad event ptr). */
    if (extCode == 0x10170) {
        static uint32_t n_10170;
        n_10170++;
        printf("[JJFB_V73_10170] #%u app=0x%X code=0x%X p0=0x%X p1=0x%X -> SUCCESS\n",
               n_10170, app, code, param0, param1);
        fflush(stdout);
        SET_RET_V(MR_SUCCESS);
        return;
    }
