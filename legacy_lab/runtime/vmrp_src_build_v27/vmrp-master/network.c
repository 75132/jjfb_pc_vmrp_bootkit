...
static int jjfb_skip_net_login(void) {
    const char *e = getenv("JJFB_SKIP_NET_LOGIN");
    return e && e[0] == '1';
}
...
int32 my_connect(int32 s, int32 ip, uint16 port, int32 type) {
    ...
    if (jjfb_skip_net_login()) {
        data->state = MR_SUCCESS;
        data->realState = MR_SUCCESS;
        printf("[JJFB_PROBE_SKIP_NET] connect s=%d ip=0x%08X:%u -> SUCCESS (TEMPORARY)\n",
               s, ip, port);
        return MR_SUCCESS;
    }
...
int32 my_initNetwork(uc_engine* uc, MR_INIT_NETWORK_CB cb, const char* mode, void* userData) {
    ...
    if (jjfb_skip_net_login()) {
        /* Sync success — no async cb (Mythroad MR_SUCCESS contract). */
        printf("[JJFB_PROBE_SKIP_NET] initNetwork mode=%s -> SUCCESS (TEMPORARY)\n",
               mode ? mode : "(null)");
        (void)uc; (void)cb; (void)userData;
        return MR_SUCCESS;
    }
...
#else
    /* Win32 host: Winsock path lives in my_initNetworkSync even without
     * full NETWORK_SUPPORT (posix sockets / cmwap). Return sync SUCCESS so
     * robotol connecting can proceed; opt-in skip via JJFB_SKIP_NET_LOGIN. */
#ifdef WIN_PLAT
    printf("my_initNetwork(WIN) cb=%p mode='%s'\n", (void *)cb,
           mode ? mode : "(null)");
    if (jjfb_skip_net_login()) {
        printf("[JJFB_PROBE_SKIP_NET] initNetwork mode=%s -> SUCCESS (TEMPORARY)\n",
               mode ? mode : "(null)");
    ...
    return my_initNetworkSync();
#else
    (void)uc; (void)cb; (void)mode; (void)userData;
    return MR_FAILED;
#endif
#endif
}
...
int32 my_getHostByName(uc_engine* uc, const char* name, MR_GET_HOST_CB cb, void* userData) {
    ...
    if (jjfb_skip_net_login()) {
        /* 127.0.0.1 — sync IP return, no cb. */
        int32 ip = (127 << 24) | 1;
        printf("[JJFB_PROBE_SKIP_NET] getHostByName '%s' -> 127.0.0.1 (TEMPORARY)\n",
               name ? name : "(null)");
        (void)uc; (void)cb; (void)userData;
        return ip;
    }
