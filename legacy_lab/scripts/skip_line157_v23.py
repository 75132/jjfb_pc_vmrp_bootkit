...
def patch_main_param():
    ...
    bak=exe.with_name("main.before_skip_line157_v23.exe")
    if not bak.exists():
    ...
    mo=data.find(marker)
    if mo >= 0:
        data[mo+marker.find(b"NULL"):mo+marker.find(b"NULL")+4] = b"L23P"

    patched=exe.with_name("main.skip_line157_v23.exe")
    ...
    log("skip_line157_v23_main_patch_report.json", json.dumps(report, ensure_ascii=False, indent=2))
...
def patch_line_to_next(buf, f, line, min_target_line=None):
    pcs=[i for i,l in enumerate(f["lineinfo"], start=1) if l == line]
    if not pcs:
        raise RuntimeError(f"line {line} not found")
    pc=pcs[0]
    # target is first PC with line > line, or >= min_target_line if given.
    target_pc=None
    for i,l in enumerate(f["lineinfo"], start=1):
        if i <= pc:
            continue
        if min_target_line is not None:
            if l >= min_target_line:
                target_pc=i; break
        else:
            if l > line:
                target_pc=i; break
    if target_pc is None:
        raise RuntimeError(f"no target after line {line}")
    skip=target_pc - pc - 1
    if skip <= 0 or skip > 255:
        raise RuntimeError(f"bad jump skip for line {line}: pc={pc} target={target_pc} skip={skip}")
    off=f["code_off"]+(pc-1)*4
    old=bytes(buf[off:off+4])
    new=struct.pack("<I", encode_jmp(skip))
    buf[off:off+4]=new
    return {"line":line,"pc":pc,"target_pc":target_pc,"target_line":f["lineinfo"][target_pc-1],"skip":skip,"off":hex(off),"old":old.hex(),"new":new.hex()}

def patch_start_mr(decomp):
    ...
    report={"main_code_off":hex(f["code_off"]),"ncode":len(f["code"]),"patches":[]}
    # v22 sdk errors: line 143 and 147 jump to line >148.
    report["patches"].append(patch_line_to_next(buf, f, 143, min_target_line=149))
    report["patches"].append(patch_line_to_next(buf, f, 147, min_target_line=149))
    # v23: skip line157 to first subsequent line.
    report["patches"].append(patch_line_to_next(buf, f, 157, min_target_line=158))
...
def patch_jjfb(src, out):
    ...
    patched_decomp, bc_report=patch_start_mr(decomp)
    ...
    report={"src":str(src),"out":str(out),"start_off":hex(start_off),"old_comp_size":old_size,"new_comp_size":len(new_comp),"old_decomp_len":len(decomp),"new_decomp_len":len(patched_decomp),"size_field_updated_to":psize,"sanity_decomp_len":len(test),"bytecode_patch":bc_report,"sha1":sha1_file(out)}
    return report
...
def prepare_runtime(exe):
    ...
    patched_jjfb=LOGS/"jjfb_skip_line157_v23.mrp"
    mrp_report=patch_jjfb(src_jjfb, patched_jjfb)
    log("skip_line157_v23_mrp_patch_report.json", json.dumps(mrp_report, ensure_ascii=False, indent=2))
    print(json.dumps(mrp_report, ensure_ascii=False, indent=2))
    dsm=dst/"dsm_gm.mrp"
    bak=dst/"dsm_gm.before_skip_line157_v23.mrp"
    ...
    log("skip_line157_v23_prepare.txt","\n".join(rep))

def collect_zip():
    rc,tl=run_cmd(["cmd","/c","tasklist"], timeout=20)
    log("skip_line157_v23_tasklist.txt", tl)
    zipname=LOGS/f"skip_line157_v23_feedback_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
...
def live():
    ...
    print("")
    print("Launching v23 skip line157. Watch vmrp window.")
    print("Param:", START_PARAM_STR)
    p=subprocess.Popen([str(exe)], cwd=str(exe.parent))
    log("skip_line157_v23_launch.txt", f"pid={p.pid}\nexe={exe}\nparam={START_PARAM_STR}\n")
    snapshots=[]
    for i in range(30):
        time.sleep(10)
        rc,ns=run_cmd(["cmd","/c","netstat -ano"], timeout=20)
        name=f"skip_line157_v23_netstat_{i:02d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        log(name, ns)
        ...
        if has:
            ...
            break
    log("skip_line157_v23_monitor.json", json.dumps(snapshots, ensure_ascii=False, indent=2))
    collect_zip()
...
def restore():
    exe=find_main()
    if exe:
        for name in ["main.before_skip_line157_v23.exe","main.before_skip_sdk_error_v22.exe","main.before_sdk_branch_variants_v21.exe","main.before_sdk_bypass_v20.exe","main.before_direct_arg_patch_v15.exe"]:
            bak=exe.with_name(name)
            if bak.exists():
                shutil.copy2(bak, exe); print(f"restored main.exe from {bak}"); break
        dst=exe.parent/"mythroad"
        for name in ["dsm_gm.before_skip_line157_v23.mrp","dsm_gm.before_skip_sdk_error_v22.mrp","dsm_gm.before_sdk_branch_variants_v21.mrp","dsm_gm.before_sdk_bypass_v20.mrp","dsm_gm.before_direct_arg_patch_v15.mrp"]:
            bak=dst/name
            if bak.exists():
