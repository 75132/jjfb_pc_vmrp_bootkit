...
def patch_main_param():
    ...
    bak=exe.with_name("main.before_skip_loader_variants_v24.exe")
    if not bak.exists():
    ...
    mo=data.find(marker)
    if mo >= 0:
        data[mo+marker.find(b"NULL"):mo+marker.find(b"NULL")+4] = b"V24P"
    patched=exe.with_name("main.skip_loader_variants_v24.exe")
...
def first_pc_at_or_after_line(f, min_line):
    for i,l in enumerate(f["lineinfo"], start=1):
        if l >= min_line:
            return i, l
    return None, None
def patch_pc_to_target(buf, f, pc, target_pc):
    skip=target_pc-pc-1
    if skip <= 0 or skip > 255:
        raise RuntimeError(f"bad skip pc={pc} target={target_pc} skip={skip}")
    off=f["code_off"]+(pc-1)*4
    old=bytes(buf[off:off+4])
    new=struct.pack("<I", encode_jmp(skip))
    buf[off:off+4]=new
    return {"pc":pc,"line":f["lineinfo"][pc-1],"target_pc":target_pc,"target_line":f["lineinfo"][target_pc-1],"skip":skip,"off":hex(off),"old":old.hex(),"new":new.hex()}
def patch_start_mr_variant(decomp, target_pc):
    ...
    # error lines 143/147: first PC of each line.
    patches=[]
    for line in [143,147,157]:
        pcs=[i for i,l in enumerate(f["lineinfo"], start=1) if l == line]
        if not pcs:
            raise RuntimeError(f"line {line} not found")
        patches.append(patch_pc_to_target(buf, f, pcs[0], target_pc))
    return bytes(buf), {"target_pc":target_pc,"target_line":f["lineinfo"][target_pc-1],"patches":patches,"line_groups":line_groups(f),"main_code_off":hex(f["code_off"]),"ncode":len(f["code"])}
def line_groups(f):
    groups=[]
    last=None
    for i,l in enumerate(f["lineinfo"], start=1):
...
def find_source_jjfb(runtime_dst=None):
    ...
    return None
def make_variant_mrp(src, out, target_pc):
    ...
    patched_decomp, bc_report=patch_start_mr_variant(decomp, target_pc)
    ...
    return {"src":str(src),"out":str(out),"old_comp_size":old_size,"new_comp_size":len(new_comp),"bytecode_patch":bc_report,"sha1":sha1_file(out)}
def extract_line_groups_from_src(src):
    data=src.read_bytes()
    marker=b"start.mr\x00"
    pos=data.find(marker)
    off_field=pos+len(marker)
    start_off=struct.unpack_from("<I", data, off_field)[0]
    size=struct.unpack_from("<I", data, off_field+4)[0]
    decomp=zlib.decompress(data[start_off:start_off+size], wbits=31)
    f,end=parse_func(decomp,0,top=True)
    return line_groups(f)
...
def prepare_base_runtime(exe):
    ...
    return dst
def install_mrp(exe, mrp):
    dst=exe.parent/"mythroad"
    dsm=dst/"dsm_gm.mrp"
    bak=dst/"dsm_gm.before_skip_loader_variants_v24.mrp"
    if dsm.exists() and not bak.exists(): shutil.copy2(dsm,bak)
    shutil.copy2(mrp,dsm)
    shutil.copy2(mrp,dst/"jjfb.mrp")
    (dst/"gwy").mkdir(parents=True, exist_ok=True)
    shutil.copy2(mrp,dst/"gwy"/"jjfb.mrp")
def run_one(exe, tag, wait=15):
    ts=datetime.now().strftime("%Y%m%d_%H%M%S")
    stdout=LOGS/f"skip_loader_{tag}_{ts}.txt"
    ...
    ns_path=LOGS/f"skip_loader_{tag}_netstat_{ts}.txt"
    ...
    has_target_net=bool(TARGET_NET_RE.search(ns))
    interesting = has_target_net or any(k in txt for k in ["mrc_loader", "robotol", "connect", "mr_socket", "my_connect"])
    return {"tag":tag,"stdout":str(stdout),"netstat":str(ns_path),"alive":alive,"stdout_len":len(txt),"tail":txt[-2000:],"has_target_net":has_target_net,"interesting":interesting}
def run_variants():
    exe=patch_main_param()
    dst=prepare_base_runtime(exe)
    src=find_source_jjfb(dst)
    if not src: raise RuntimeError("jjfb.mrp not found")
    groups=extract_line_groups_from_src(src)
    log("skip_loader_line_groups.json", json.dumps(groups, ensure_ascii=False, indent=2))
    # Candidate targets: first PCs after line157, max 10.
    candidates=[g for g in groups if g["line"] > 157][:10]
    patch_reports=[]; results=[]
    for g in candidates:
        tag=f"pc{g['pc']}_line{g['line']}"
        print("== candidate", tag, "==")
        mrp=LOGS/f"jjfb_skip_loader_{tag}.mrp"
        pr=make_variant_mrp(src, mrp, g["pc"])
        patch_reports.append(pr)
        install_mrp(exe, mrp)
        res=run_one(exe, tag, wait=15)
        results.append(res)
        print(json.dumps({"tag":tag,"stdout_len":res["stdout_len"],"has_target_net":res["has_target_net"],"interesting":res["interesting"],"tail":res["tail"][-200:]}, ensure_ascii=False))
        if res["interesting"]:
            print("INTERESTING candidate found; stopping.")
            break
    log("skip_loader_variants_patch_reports.json", json.dumps(patch_reports, ensure_ascii=False, indent=2))
    log("skip_loader_variants_results.json", json.dumps(results, ensure_ascii=False, indent=2))
    collect_zip()
def collect_zip():
    rc,tl=run_cmd(["cmd","/c","tasklist"], timeout=20)
    log("skip_loader_variants_tasklist.txt", tl)
    zipname=LOGS/f"skip_loader_variants_v24_feedback_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
...
def restore():
    exe=find_main()
    if exe:
        for name in ["main.before_skip_loader_variants_v24.exe","main.before_skip_line157_v23.exe","main.before_skip_sdk_error_v22.exe","main.before_sdk_branch_variants_v21.exe","main.before_sdk_bypass_v20.exe","main.before_direct_arg_patch_v15.exe"]:
            bak=exe.with_name(name)
            if bak.exists():
                shutil.copy2(bak, exe); print(f"restored main.exe from {bak}"); break
        dst=exe.parent/"mythroad"
        for name in ["dsm_gm.before_skip_loader_variants_v24.mrp","dsm_gm.before_skip_line157_v23.mrp","dsm_gm.before_skip_sdk_error_v22.mrp","dsm_gm.before_sdk_branch_variants_v21.mrp","dsm_gm.before_sdk_bypass_v20.mrp","dsm_gm.before_direct_arg_patch_v15.mrp"]:
            bak=dst/name
            if bak.exists():
