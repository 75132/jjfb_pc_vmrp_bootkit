#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import argparse, struct, re, shutil, zipfile, hashlib, subprocess, time, json, zlib, gzip, io, binascii
...
# Replace serialized string constant:
# old: string length 7, "_error\0"
# new: string length 4, "_gc\0"
# This shrinks start.mr by 3 bytes but keeps bytecode structurally valid.
...
def patch_main_param():
    ...
    bak=exe.with_name("main.before_sdk_bypass_v19.exe")
    if not bak.exists():
    ...
    mo=data.find(marker)
    if mo >= 0:
        data[mo+marker.find(b"NULL"):mo+marker.find(b"NULL")+4] = b"S19P"

    patched=exe.with_name("main.sdk_bypass_v19.exe")
    ...
    log("sdk_bypass_v19_main_patch_report.json", json.dumps(report, ensure_ascii=False, indent=2))
...
def patch_jjfb_start_mr(src, out):
    ...
    decomp2=decomp[:idx] + GC_CONST_VALID + decomp[idx+len(ERROR_CONST_PATTERN):]

    new_comp=gzip_bytes(bytes(decomp2), level=9)
    if len(new_comp) > old_size:
        # fallback try other levels
        best=None
        for level in range(1,10):
            c=gzip_bytes(bytes(decomp2), level=level)
    ...
    # IMPORTANT FIX: update MRP file table compressed size to real new gzip size.
    struct.pack_into("<I", data, off_field+4, len(new_comp))
    data[start_off:start_off+len(new_comp)] = new_comp
    # Clear leftover old gzip bytes so there is no fake footer/trailing garbage.
    if len(new_comp) < old_size:
    ...
    # Sanity: reload from patched MRP using new size, decompress.
    ...
    report={
        ...
        "sanity_contains_error": b"cann`t find sdk key!" in test,
...
def prepare_runtime(exe):
    ...
    patched_jjfb=LOGS/"jjfb_sdk_bypass_v19.mrp"
    mrp_report=patch_jjfb_start_mr(src_jjfb, patched_jjfb)
    log("sdk_bypass_v19_mrp_patch_report.json", json.dumps(mrp_report, ensure_ascii=False, indent=2))
    print(json.dumps(mrp_report, ensure_ascii=False, indent=2))

    dsm=dst/"dsm_gm.mrp"
    bak=dst/"dsm_gm.before_sdk_bypass_v19.mrp"
    ...
    log("sdk_bypass_v19_prepare.txt","\n".join(rep))

def collect_zip():
    rc,tl=run_cmd(["cmd","/c","tasklist"], timeout=20)
    log("sdk_bypass_v19_tasklist.txt", tl)
    zipname=LOGS/f"sdk_bypass_v19_feedback_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
...
def live():
    ...
    print("")
    print("Launching v19 SDK-bypass with fixed MRP size. Watch vmrp window.")
    print("Param:", START_PARAM_STR)
    p=subprocess.Popen([str(exe)], cwd=str(exe.parent))
    log("sdk_bypass_v19_launch.txt", f"pid={p.pid}\nexe={exe}\nparam={START_PARAM_STR}\n")
    snapshots=[]
    for i in range(30):
        time.sleep(10)
        rc,ns=run_cmd(["cmd","/c","netstat -ano"], timeout=20)
        name=f"sdk_bypass_v19_netstat_{i:02d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        log(name, ns)
        ...
        if has:
            ...
            break
    log("sdk_bypass_v19_monitor.json", json.dumps(snapshots, ensure_ascii=False, indent=2))
    collect_zip()
...
def restore():
    exe=find_main()
    if exe:
        for name in ["main.before_sdk_bypass_v19.exe","main.before_sdk_bypass_v18.exe","main.before_live_boot_v17.exe","main.before_direct_arg_patch_v15.exe","main.before_fixed_key_v9.exe"]:
            bak=exe.with_name(name)
            if bak.exists():
                shutil.copy2(bak, exe)
                ...
                break
        dst=exe.parent/"mythroad"
        for name in ["dsm_gm.before_sdk_bypass_v19.mrp","dsm_gm.before_sdk_bypass_v18.mrp","dsm_gm.before_live_boot_v17.mrp","dsm_gm.before_param_variants_v16.mrp","dsm_gm.before_direct_arg_patch_v15.mrp","dsm_gm.before_fixed_key_v9.mrp"]:
            bak=dst/name
            if bak.exists():
