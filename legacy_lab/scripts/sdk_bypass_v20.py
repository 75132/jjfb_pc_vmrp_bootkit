...
# Serialized constants in start.mr:
# string "_error" -> string "_gc"
# string "cann`t find sdk key!" -> integer 0
ERROR_CONST_PATTERN = b"\x04\x07\x00\x00\x00_error\x00"
GC_CONST_VALID = b"\x04\x04\x00\x00\x00_gc\x00"
MSG_CONST_PATTERN = b"\x04\x15\x00\x00\x00cann`t find sdk key!\x00"
INT0_CONST = b"\x03\x00\x00\x00\x00"
...
def patch_main_param():
    ...
    bak=exe.with_name("main.before_sdk_bypass_v20.exe")
    if not bak.exists():
    ...
    mo=data.find(marker)
    if mo >= 0:
        data[mo+marker.find(b"NULL"):mo+marker.find(b"NULL")+4] = b"S20P"

    patched=exe.with_name("main.sdk_bypass_v20.exe")
    ...
    log("sdk_bypass_v20_main_patch_report.json", json.dumps(report, ensure_ascii=False, indent=2))
...
def patch_jjfb_start_mr(src, out):
    ...
    if start_off <= 0 or old_size <= 0 or start_off+old_size > len(data):
        raise RuntimeError(f"bad start.mr offset/size off=0x{start_off:x} size=0x{old_size:x}")

    comp=bytes(data[start_off:start_off+old_size])
    decomp=bytearray(zlib.decompress(comp, wbits=31))
    old_decomp_len=len(decomp)

    idx_err=decomp.find(ERROR_CONST_PATTERN)
    idx_msg=decomp.find(MSG_CONST_PATTERN)
    if idx_err < 0:
        raise RuntimeError("_error constant pattern not found in start.mr")
    if idx_msg < 0:
        raise RuntimeError("sdk error message constant pattern not found in start.mr")
    if not (idx_err < idx_msg < idx_err + 128):
        raise RuntimeError(f"unexpected constant layout idx_err=0x{idx_err:x} idx_msg=0x{idx_msg:x}")

    # Patch from end to start so offsets remain valid.
    decomp2 = bytes(decomp)
    decomp2 = decomp2[:idx_msg] + INT0_CONST + decomp2[idx_msg+len(MSG_CONST_PATTERN):]
    decomp2 = decomp2[:idx_err] + GC_CONST_VALID + decomp2[idx_err+len(ERROR_CONST_PATTERN):]

    new_comp=gzip_bytes(decomp2, level=9)
    if len(new_comp) > old_size:
        best=None
        for level in range(1,10):
            c=gzip_bytes(decomp2, level=level)
            if len(c) <= old_size and (best is None or len(c) < len(best)):
                best=c
        if best is None:
            raise RuntimeError(f"new gzip too large for old slot; old={old_size}")
        new_comp=best

    # Update size field to real size.
    ...
    report={
        ...
        "old_comp_size": old_size,
        "new_comp_size": len(new_comp),
        "old_decomp_len": old_decomp_len,
        "new_decomp_len": len(decomp2),
        "error_const_offset": hex(idx_err),
        "message_const_offset": hex(idx_msg),
        "size_field_updated_to": psize,
        "sanity_decomp_len": len(test),
        "sanity_contains_message": b"cann`t find sdk key!" in test,
        "sanity_error_const_present": ERROR_CONST_PATTERN in test,
        "sanity_gc_const_present": GC_CONST_VALID in test,
        "sanity_int0_after_gc": INT0_CONST in test[idx_err:idx_err+128],
        "sha1": sha1_file(out),
    }
    return report
...
def prepare_runtime(exe):
    ...
    patched_jjfb=LOGS/"jjfb_sdk_bypass_v20.mrp"
    mrp_report=patch_jjfb_start_mr(src_jjfb, patched_jjfb)
    log("sdk_bypass_v20_mrp_patch_report.json", json.dumps(mrp_report, ensure_ascii=False, indent=2))
    print(json.dumps(mrp_report, ensure_ascii=False, indent=2))

    dsm=dst/"dsm_gm.mrp"
    bak=dst/"dsm_gm.before_sdk_bypass_v20.mrp"
    ...
    log("sdk_bypass_v20_prepare.txt","\n".join(rep))

def collect_zip():
    rc,tl=run_cmd(["cmd","/c","tasklist"], timeout=20)
    log("sdk_bypass_v20_tasklist.txt", tl)
    zipname=LOGS/f"sdk_bypass_v20_feedback_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
...
def live():
    ...
    print("")
    print("Launching v20 SDK-bypass _gc(0). Watch vmrp window.")
    print("Param:", START_PARAM_STR)
    p=subprocess.Popen([str(exe)], cwd=str(exe.parent))
    log("sdk_bypass_v20_launch.txt", f"pid={p.pid}\nexe={exe}\nparam={START_PARAM_STR}\n")
    snapshots=[]
    for i in range(30):
        time.sleep(10)
        rc,ns=run_cmd(["cmd","/c","netstat -ano"], timeout=20)
        name=f"sdk_bypass_v20_netstat_{i:02d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        log(name, ns)
        ...
        if has:
            ...
            break
    log("sdk_bypass_v20_monitor.json", json.dumps(snapshots, ensure_ascii=False, indent=2))
    collect_zip()
...
def restore():
    exe=find_main()
    if exe:
        for name in ["main.before_sdk_bypass_v20.exe","main.before_sdk_bypass_v19.exe","main.before_sdk_bypass_v18.exe","main.before_live_boot_v17.exe","main.before_direct_arg_patch_v15.exe"]:
            bak=exe.with_name(name)
            if bak.exists():
                shutil.copy2(bak, exe)
                ...
                break
        dst=exe.parent/"mythroad"
        for name in ["dsm_gm.before_sdk_bypass_v20.mrp","dsm_gm.before_sdk_bypass_v19.mrp","dsm_gm.before_sdk_bypass_v18.mrp","dsm_gm.before_live_boot_v17.mrp","dsm_gm.before_param_variants_v16.mrp","dsm_gm.before_direct_arg_patch_v15.mrp","dsm_gm.before_fixed_key_v9.mrp"]:
            bak=dst/name
            if bak.exists():
