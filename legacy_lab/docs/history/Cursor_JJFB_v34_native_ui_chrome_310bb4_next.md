...
## 7. v34 实施进度（2026-07-17）

### 7.1 受控放行 2f4494 + 310Bxx/310BB4 guard

在 `bridge.c` 增加环境变量开关（默认不改变现状）：

```text
JJFB_ALLOW_CHROME=1           才放行 2f4494
JJFB_CHROME_ALLOW_CALLS=N     限制放行次数（防止卡死）
JJFB_CHROME_SKIP_310BB4=1     默认：310Bxx/310BB4 入口直接返回（guard）
JJFB_310BB4_DUMP_N=N          dump N 次寄存器/栈/对象窗口
```

### 7.2 310bb4 ABI 抓取（入口+栈窗口+bmp 对象）

证据：`logs/v34_310bb4_dump3_stdout.txt`

```text
[JJFB_310BB4_ENTER] pc=0x310B5C / pc=0x310BB4 ...
[JJFB_DUMP] 310BB4_sp_0x00 @sp 0x40
[JJFB_SEND] dump 310BB4_r5_obj @r5 0x80
[JJFB_310BB4_GUARD] skip body, return to lr=...
```

当前观察：

```text
r5 常为 0x6BD0F0（疑似 bmp/对象）
lr 在 2f48xx~2f4Axx 循环（2f4494 内部贴图序列）
同一路径既会进入 0x310B5C（helper）也会进入 0x310BB4（classic loop）
```

### 7.3 wy_jiao/wy_xian/jiantou 名字链确认（2d92dc）

在 UI hook 里追加 `0x2D92DC` 资源加载点日志。注意：**name 指针在 r1，不在 r0**。

证据：`logs/v34_2d92dc_names_stdout.txt`
...
```text
chrome 资源名格式 = name!w!h.bmp（已确认）
下一步：把 name!w!h.bmp / @vmimage.bmp / *.mrp 路径映射串到实际打开与解码逻辑
```
